...
Classified checkout uses Swirepay's server-minted checkout-session flow. The authenticated SDTV server freezes the approved database amount, calls `POST /v3/checkout-session` with the server-only `SWIREPAY_SECRET_KEY`, and records the returned `paymentSessionGid` in `swirepay_checkout_sessions` before returning the short-lived encrypted token to the browser. The pinned `@swirepay-developer/swirepay-frontend-payment-sdk` renders inline with `accountGid` and `secureToken`; publishable keys, browser-supplied amounts, and the legacy `open()` callback contract are not used. Signed webhooks resolve the session ledger and remain the only authority that activates a classified.
...
## Multi-city site resolution

Multi-city rollout begins at a server-only hostname boundary rather than by changing existing content queries. `resolveCurrentSite()` normalizes the forwarded request hostname, resolves it through `site_domains` and `sites`, and returns a typed site configuration containing identity, locale, timezone, status, primary domain, and non-secret settings. Recognized domains use an active domain mapping; Vercel preview hosts and other unmapped hosts deliberately resolve to `DEFAULT_SITE_CODE` (Seattle by default) so current preview and production behavior remains stable.

If database configuration is unavailable, the resolver returns a built-in Seattle fallback instead of breaking Studio. Studio displays the resolved site, hostname, source, and timezone as a temporary operational diagnostic. The initial resolver phase did not filter or rebrand content; city ownership is added to content tables module by module, with Seattle backfills and indexes applied before each module's hostname-based filtering is enabled.

Events are the first site-owned content module. Every event has a required `site_id`; the migration backfills all pre-existing records to Seattle and provides a Seattle default for legacy insertion paths. The root server layout resolves the request once and supplies the typed site context to browser components. Public event lists, event detail and related-event reads, homepage event feeds, featured-event presentation, SEO entity lookup, sitemap event entries, the mobile Events API, and the primary Studio Events manager apply that site identifier. Event submissions snapshot the resolved site and remain approval-first. Specialized cross-event operational tools remain global during the transition until they receive an explicit market selector.

Local businesses use the same required site-ownership pattern. Existing listings are backfilled to Seattle, while new community submissions snapshot the resolved market. The directory, premium business section, homepage business feed and count, business detail SEO lookup, sitemap, mobile Businesses API, and primary Studio list/editor filter by `site_id`. Sponsorship, offer, claim, and outreach workflows continue to reference the business identity and therefore inherit its market without duplicating site ownership onto every dependent record.

Vercel preview deployments support a temporary market override for isolation testing. `/api/site-preview` validates a requested site code against the registry and stores it in an HTTP-only, secure, same-site cookie for one hour. The resolver honors that cookie only when `VERCEL_ENV=preview` and the request hostname ends in `.vercel.app`; production/custom domains cannot activate it. Clearing the cookie restores normal hostname resolution. Studio identifies this state as `Preview override`.

Community organizations and groups are site-owned directories. Existing records are assigned to Seattle, and both community and administrator creation paths snapshot the resolved site. Public directories, organization profiles and SEO, sitemap entries, mobile organization/group resources, the event form's organization picker, and the shared Studio moderation UI filter by that site. Event-to-organization links remain relational; the UI only offers organizations from the event's current market.
