...
If database configuration is unavailable, the resolver returns a built-in Seattle fallback instead of breaking Studio. Studio displays the resolved site, hostname, source, and timezone as a temporary operational diagnostic. This phase does not filter events, businesses, organizations, or other content and does not change public branding. City ownership will be added to content tables module by module, beginning with events, with Seattle backfills and indexes applied before any hostname-based filtering is enabled.
