...
export default function MyAvailabilityPage() {
  ...
  async function init() {
    ...
    setUser(currentUser);
    if (!currentUser) { setMessage("Please login first."); setLoading(false); return; }
    const nextRole = await resolveUserRole(supabase, currentUser);
    setRole(nextRole);
    if (!isTeamRole(nextRole)) { setMessage(`Team access needed. Current role: ${nextRole}`); setLoading(false); return; }
  ...
  return (
    <main className="min-h-screen bg-slate-950 text-white">
      ...
      <div className="max-w-6xl mx-auto px-6 py-10">
        ...
        <h1 className="text-4xl md:text-5xl font-black mt-3">My Availability</h1>
        <p className="text-slate-300 mt-2">Mark when you can support SDTV coverage.</p>
