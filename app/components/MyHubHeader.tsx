...
export default function MyHubHeader() {
  ...
  const shouldShowJourney =
    Boolean(email) && !canSeeTeamTools && !canSeeStudio && !canSeeVideoTools;
  ...
  const links: HubLink[] = [
    ...
    {
      label: "My Influencer Profile",
      href: "/my-influencer-profile",
      show: Boolean(email),
    },
    {
      label: "My Community Submissions",
    ...
    {
      label: "My Coverage Assignments",
      href: "/my-assignments",
      show: true,
      tone: canSeeTeamTools ? "default" : "team",
    },
    {
      label: "My Video Assignments",
      href: "/my-video-assignments",
      show: canSeeVideoTools,
      tone: "default",
    },
    {
  ...
  function linkClass(tone?: HubLink["tone"], active?: boolean) {
    if (active)
      return "relative bg-pink-600 text-white shadow-lg shadow-pink-900/30 ring-1 ring-pink-300/40 after:absolute after:left-3 after:right-3 after:-bottom-1 after:h-0.5 after:rounded-full after:bg-pink-200";
    if (tone === "primary") return "bg-white/10 hover:bg-pink-600 text-white";
    if (tone === "team")
      return "bg-white/5 hover:bg-white/10 text-slate-400 border border-white/10";
        ...
        <div className="flex flex-col gap-4">
          <div className="flex items-start justify-between gap-3">
            <div className="min-w-0">
              <a href="/" className="text-pink-300 font-bold text-sm">
                ← Public Site
              </a>
            ...
            <div className="flex gap-2 shrink-0 items-center">
              <button
                ...
                className="lg:hidden border border-white/20 px-3 py-2 rounded-lg transition text-sm font-black"
              >
                ☰ Menu
              </button>
              ...
              .map((link) => {
                ...
                return (
                  <a
                    ...
                    className={`${linkClass(link.tone, active)} px-3 py-2 rounded-lg transition`}
                  >
                    {link.label}
                    {link.tone === "team" ? " · Team" : ""}
                  </a>
          ...
          {menuOpen && (
            <div className="lg:hidden fixed inset-0 z-50 bg-slate-950/95 text-white backdrop-blur-md">
              <div className="flex min-h-screen flex-col px-5 py-5">
                <div className="flex items-center justify-between gap-3 border-b border-white/10 pb-4">
                  <div className="min-w-0">
                    ...
                    <h2 className="truncate text-2xl font-black">My Hub</h2>
                    <p className="truncate text-xs font-bold text-slate-400">
                      {email || "SDTV workspace"}
                    </p>
                  </div>
                  <button
                    type="button"
                    ...
                    .map((link) => {
                      ...
                      return (
                        <a
                          ...
                          aria-current={active ? "page" : undefined}
                          onClick={() => setMenuOpen(false)}
                          className={`${active ? "bg-pink-600 text-white" : link.tone === "team" ? "bg-white/5 text-slate-300 border border-white/10" : "bg-white/10 text-white"} rounded-2xl px-4 py-4`}
                        >
                          {link.label}
                          {link.tone === "team" ? " · Team" : ""}
                        </a>
