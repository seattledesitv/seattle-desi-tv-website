...
import HeroCarouselV3, { type HeroItemV3, type HeroLayoutStyle } from "./HeroCarouselV3";
  ...
  useEffect(() => {
    async function load() {
      ...
      const eventItems: HeroItemV3[] = (eventsResult.data || []).map((row: any) => ({
        id: `event-${row.id}`, title: row.title,
        subtitle: `${formatDate(row.date)}${row.location ? ` · ${row.location}` : ""}`,
        image_url: firstImage(row), button_text: "View Event", button_url: `/events/${row.id}`,
        badge: "Featured Event", display_order: Number(row.featured_order || 0), theme: row.hero_theme || "fallback",
      }));
      const bannerItems: HeroItemV3[] = (bannersResult.data || []).filter((row: any) => activeToday(row, today)).map((row: any) => ({
        id: row.id, title: row.title, subtitle: row.subtitle, image_url: row.image_url,
        ...
        display_order: Number(row.display_order || 0), theme: row.theme || "fallback",
      }));
      const festivalItems: HeroItemV3[] = (festivalsResult.data || []).filter((row: any) => activeToday(row, today)).map((row: any) => ({
        id: `festival-${row.id}`, title: row.title || row.festival_name,
        subtitle: row.subtitle || `Celebrating ${row.festival_name} with the Seattle Desi community.`,
        image_url: row.image_url, button_text: "Explore Events", button_url: "/events",
        badge: row.festival_name, display_order: -1, theme: row.theme || "festival",
      }));
      ...
      setLayoutStyle(["image_focus", "classic", "premium", "cinematic"].includes(savedLayout) ? savedLayout : "image_focus");
    }
    load();
  }, []);

  return <HeroCarouselV3 items={items} layoutStyle={layoutStyle} />;
}
