...
const ctaCards = [["Join as Volunteer", "Support SDTV events, production, community outreach, and operations.", "Volunteer"], ["Become an RJ", "Host or support Seattle Desi Radio programs and community segments.", "RJ / Radio Host"], ["Become a VJ", "Anchor interviews, event coverage, reels, and community stories.", "VJ / Anchor"], ["Request Event Coverage", "Invite SDTV to cover your cultural, community, or business event.", "Event Coverage"], ["Sponsor SDTV", "Promote your brand across SDTV web, video, radio, and community channels.", "Sponsorship"]];
...
export default function ContactSection({ compact = false, initialInterest = "" }: { compact?: boolean; initialInterest?: string }) {
  ...
  useEffect(() => { const interest = normalizedInterest(initialInterest); if (interest) setForm((current) => ({ ...current, interest, message: current.message || `Hi SDTV team, I am interested in ${interest}. Please share next steps.` })); }, [initialInterest]);

  function renderTurnstile() {
    if (!siteKey || !turnstileRef.current || !window.turnstile || widgetIdRef.current) return;
    widgetIdRef.current = window.turnstile.render(turnstileRef.current, { sitekey: siteKey, callback: (token: string) => { setCaptchaToken(token); setStatus((current) => current === "Please complete the captcha." ? "" : current); }, "expired-callback": () => setCaptchaToken(""), "error-callback": () => { setCaptchaToken(""); setStatus("Captcha had an error. Please refresh and try again."); } });
  }
