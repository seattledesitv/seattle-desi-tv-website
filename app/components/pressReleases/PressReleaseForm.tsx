...
import DirectoryImageCropper, { type DirectoryImageCrop } from "../DirectoryImageCropper";
import type { PressRelease, PressReleaseDocument, PressReleaseInput, PressReleaseStatus } from "../../lib/pressReleases/types";
...
const blank = (): PressReleaseInput => ({
  ...
  image_position_x: 50, image_position_y: 50, image_zoom: 1, image_display_mode: "cover",
  documents: [],
  contact_name: "", contact_email: "", source_url: "",
});

function inputFromRelease(release?: PressRelease | null): PressReleaseInput {
  if (!release) return blank();
  return {
    title: release.title, summary: release.summary, body: release.body,
    organization_name: release.organization_name, location: release.location,
    release_date: release.release_date, image_urls: [...release.image_urls],
    image_position_x: release.image_position_x ?? 50, image_position_y: release.image_position_y ?? 50,
    image_zoom: release.image_zoom ?? 1, image_display_mode: release.image_display_mode ?? "cover",
    documents: [...(release.documents || [])], contact_name: release.contact_name,
    contact_email: release.contact_email, source_url: release.source_url,
  };
}

export default function PressReleaseForm({ saving, error, admin = false, initialRelease, onCreate, onUpdate, onUpload, onCancel }: {
  ...
  const [form, setForm] = useState(() => inputFromRelease(initialRelease));
  ...
  const crop: DirectoryImageCrop = { x: form.image_position_x, y: form.image_position_y, zoom: form.image_zoom, mode: form.image_display_mode };
  const setCrop = (value: DirectoryImageCrop) => setForm((current) => ({ ...current, image_position_x: value.x, image_position_y: value.y, image_zoom: value.zoom, image_display_mode: value.mode || "cover" }));
  ...
  async function submit() {
    ...
    try {
      ...
      if (primaryFile) {
        ...
        payload = { ...payload, image_urls: [url, ...payload.image_urls.filter((item) => item !== url)] };
      }
      if (initialRelease && onUpdate) {
        await onUpdate(initialRelease.id, payload);
        setMessage(admin ? "Press release changes saved." : "Changes saved and returned to SDTV for review.");
      } else {
  ...
  return <section className="rounded-3xl border bg-white p-6 shadow-sm md:p-8">
    <div className="grid gap-5 md:grid-cols-2">
      ...
      {form.image_urls.length > 0 && <div className="md:col-span-2"><p className="mb-3 text-sm font-bold text-slate-600">The first image is used on cards. Select any image as primary, remove it, or adjust the visible area below.</p><div className="flex flex-wrap gap-3">{form.image_urls.map((url, index) => <div key={url} className={`relative rounded-xl border-2 p-1 ${index === 0 ? "border-pink-600" : "border-transparent"}`}><img src={url} alt={`Upload ${index + 1}`} className="h-24 w-32 rounded-lg object-cover"/><div className="mt-1 flex gap-1">{index === 0 ? <span className="rounded-md bg-pink-50 px-2 py-1 text-xs font-black text-pink-700">Primary</span> : <button type="button" onClick={() => set("image_urls", [url, ...form.image_urls.filter((item) => item !== url)])} className="rounded-md bg-slate-100 px-2 py-1 text-xs font-black">Make primary</button>}<button type="button" aria-label={`Remove image ${index + 1}`} onClick={() => set("image_urls", form.image_urls.filter((item) => item !== url))} className="rounded-md bg-slate-950 px-2 py-1 text-xs font-black text-white">Remove</button></div></div>)}</div></div>}
      <div className="md:col-span-2"><DirectoryImageCropper src={form.image_urls[0]} value={crop} onChange={setCrop} onFileChange={setPrimaryFile} label={form.image_urls[0] ? "Replace primary image and choose its visible card area" : "Upload primary image and choose its visible card area"}/></div>
      ...
      {admin && !editing && <label className="flex items-center gap-3 rounded-xl bg-pink-50 p-4 font-bold md:col-span-2"><input type="checkbox" checked={publishNow} onChange={(e) => setPublishNow(e.target.checked)} />Publish immediately (otherwise send to moderation queue)</label>}
    </div>
    <div className="mt-6 flex flex-col gap-3 sm:flex-row"><button type="button" disabled={saving || uploading} onClick={() => void submit()} className="flex-1 rounded-xl bg-pink-600 px-6 py-4 font-black text-white disabled:opacity-60">{uploading ? "Uploading files..." : saving ? "Saving..." : editing ? admin ? "Save Press Release" : "Save Changes for Review" : admin && publishNow ? "Publish Press Release" : "Submit for Review"}</button>{editing && onCancel && <button type="button" disabled={saving || uploading} onClick={onCancel} className="rounded-xl border px-6 py-4 font-black">Cancel Editing</button>}</div>
