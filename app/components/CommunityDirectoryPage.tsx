...
export default function CommunityDirectoryPage({ kind }: { kind: Kind }) {
  ...
  async function load() {
    ...
    const columns = kind === "groups"
      ...
      : "id,name,organization_type,category,location,website,description,contact_name,contact_email,created_at,image,image_position_x,image_position_y,image_zoom,image_display_mode,manager_verified_at";
    let listingResult: any = await forSite(supabase.from(config.table).select(columns), site.id).eq("status", "approved").eq("approved", true).order("created_at", { ascending: false });
    if (kind === "organizations" && listingResult.error && /manager_verified_at/i.test(listingResult.error.message || "")) {
      const basicColumns = "id,name,organization_type,category,location,website,description,contact_name,contact_email,created_at,image,image_position_x,image_position_y,image_zoom,image_display_mode";
      listingResult = await forSite(supabase.from(config.table).select(basicColumns), site.id).eq("status", "approved").eq("approved", true).order("created_at", { ascending: false });
    }
            ...
            return <article key={item.id} className={`flex h-full flex-col overflow-hidden rounded-3xl border bg-slate-50 transition ${expanded ? "md:col-span-2 border-pink-200 shadow-lg" : ""}`}>
              ...
              <div className="flex flex-1 flex-col p-5">
                <div className="flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
                  ...
                  <div className="flex flex-wrap gap-2">{kind === "organizations" && item.manager_verified_at && <span className="rounded-full bg-emerald-100 px-3 py-1 text-xs font-black text-emerald-800">✓ Claimed</span>}{tag(item.platform || item.organization_type)}{tag(item.category)}{tag(item.language)}</div>
                </div>
                ...
                <div className="mt-4 flex flex-wrap gap-3">
                  ...
                  {kind === "groups" && <a href={`/manage-listing?type=community_group&id=${item.id}&name=${encodeURIComponent(item.name)}`} className="rounded-xl border border-pink-200 bg-pink-50 px-4 py-2 text-sm font-black text-pink-700">Claim, correct, or remove</a>}
                </div>
                {kind === "organizations" && <div data-organization-manage-action="yes" className="mt-auto pt-5"><div className="rounded-2xl border border-pink-100 bg-white p-4"><p className="text-sm font-black text-slate-800">{item.manager_verified_at ? "This organization has a verified manager" : "Are you an authorized representative?"}</p><p className="mt-1 text-xs font-bold text-slate-500">{item.manager_verified_at ? "Additional authorized team members may still request management access." : "Verify your role to keep this organization profile accurate."}</p><a href={`/community-organizations/manage?organization=${item.id}`} className="mt-3 inline-flex rounded-full bg-pink-600 px-4 py-2 text-sm font-black text-white hover:bg-pink-700">{item.manager_verified_at ? "Request Manager Access" : "Claim this Organization"}</a></div></div>}
