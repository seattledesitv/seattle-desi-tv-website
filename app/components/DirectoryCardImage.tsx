...
export default function DirectoryCardImage({ src, alt, positionX = 50, positionY = 50, zoom = 1, badge, heightClass = "h-56" }: Props) {
  ...
  return <>
    ...
    {open && <div role="dialog" aria-modal="true" aria-label={`Full image for ${alt}`} onMouseDown={(event) => { if (event.target === event.currentTarget) setOpen(false); }} className="fixed inset-0 z-[1000] grid place-items-center bg-slate-950/90 p-4 md:p-10">
