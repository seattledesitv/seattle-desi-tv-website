...
export default function PublishingPipelineWorkspace({ supabase, publicationId, publicationStatus }: { supabase: SupabaseClient; publicationId: string; publicationStatus: PublicationStatus }) {
  const pipeline = usePublishingPipeline(supabase, publicationId);
  const [channels, setChannels] = useState<Set<PublishingChannel>>(new Set(["website", "email", "pdf"]));
  const [scheduledAt, setScheduledAt] = useState("");
  const [testEmails, setTestEmails] = useState<Record<string, string>>({});
  const deliveryApproved = ["approved", "scheduled", "published"].includes(publicationStatus);
  ...
  function publish(outputId: string) {
    ...
    const prompt = output?.channel === "website" ? "Publish this edition to the public SDTV website now?" : `Confirm ${output?.status === "failed" ? "retry" : "publishing handoff"} for ${output?.channel}?`;
    if (!output || !window.confirm(prompt)) return;
    ...
    <section className="rounded-3xl border bg-white p-5">
      ...
      {pipeline.loading ? <p className="mt-4 text-slate-500">Loading pipeline…</p> : <div className="mt-4 grid gap-3 md:grid-cols-2">{pipeline.outputs.map((output) => {
        ...
        return <article key={output.id} className="rounded-2xl border p-4">
          <div className="flex items-center justify-between gap-3"><p className="font-black capitalize">{output.channel === "email" ? "Newsletter email" : output.channel === "newsletter" ? "Legacy newsletter package" : output.channel}</p><span className="rounded-full bg-slate-100 px-3 py-1 text-xs font-black uppercase">{output.status}</span></div>
