...
export default function PublicationChannelPreview({ model, channel }: { model: PublicationPreviewModel; channel: PublicationPreviewChannel }) {
  ...
  return <article className={`publication-print-root overflow-hidden rounded-3xl bg-white text-slate-950 shadow-xl ${frame}`}>
    <header className={`publication-cover ${social ? "aspect-square" : channel === "website" ? "min-h-52 md:min-h-60" : "min-h-80"} relative flex flex-col justify-end overflow-hidden bg-gradient-to-br from-slate-950 via-pink-950 to-pink-600 p-6 text-white`}>
    ...
    {!social && <div className={`publication-body ${channel === "mobile" ? "p-4" : channel === "website" ? "p-4 md:p-5" : "p-6 md:p-8"} grid ${channel === "website" ? "gap-4" : "gap-7"}`}>
      {model.sections.filter((section) => section.section_key !== "cover").map((section) => {
        ...
        return <section key={section.id} className="publication-section break-inside-avoid">
          <h2 className={`border-b-2 border-pink-500 pb-2 font-black ${channel === "website" ? "text-xl" : "text-2xl"}`}>{section.title}</h2>
          {section.introduction && <p className={`mt-2 ${channel === "website" ? "text-sm leading-6" : "leading-7"} ${actions ? "text-slate-300" : "text-slate-600"}`}>{section.introduction}</p>}
          <div className={`publication-item-grid mt-3 grid gap-3 ${channel !== "mobile" ? statistics ? "sm:grid-cols-3" : channel === "website" ? "sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4" : "sm:grid-cols-2" : ""}`}>
