  ...
  return <main className="min-h-screen bg-slate-50 text-slate-950"><SiteHeader />{loading ? <section className="mx-auto max-w-5xl px-6 py-16"><div className="rounded-2xl border bg-white p-8">Loading organization...</div></section> : !organization ? <section className="mx-auto max-w-5xl px-6 py-16"><div className="rounded-2xl border bg-white p-8">{message}</div></section> : <>
    <section className="relative overflow-hidden bg-slate-950 text-white">
      ...
      <div className="relative mx-auto grid max-w-6xl gap-8 px-6 py-12 md:px-10 lg:grid-cols-[minmax(0,1fr)_360px] lg:items-center">
        <div><a href="/community-organizations" className="font-bold text-pink-300">← Back to Organizations</a><div className="mt-7 flex flex-wrap items-center gap-2"><p className="text-sm font-black uppercase tracking-[0.2em] text-pink-300">Community Organization</p>{organization.manager_verified_at && <span className="rounded-full bg-emerald-400/20 px-3 py-1 text-xs font-black text-emerald-200 ring-1 ring-emerald-300/30">✓ Claimed · Manager verified</span>}</div><h1 className="mt-3 text-4xl font-black md:text-6xl">{organization.name}</h1><div className="mt-5 flex flex-wrap gap-2">{organization.organization_type && <span className="rounded-full border border-white/20 bg-white/10 px-3 py-1 text-sm font-bold">{organization.organization_type}</span>}{organization.category && <span className="rounded-full border border-white/20 bg-white/10 px-3 py-1 text-sm font-bold">{organization.category}</span>}{organization.location && <span className="rounded-full border border-white/20 bg-white/10 px-3 py-1 text-sm font-bold">{organization.location}</span>}</div><div className="mt-7 flex flex-wrap gap-3">{organization.website && <CheckedExternalLink href={organization.website} notFoundMessage="This organization website is not available." className="inline-flex rounded-xl bg-pink-600 px-5 py-3 font-black text-white">Visit Website</CheckedExternalLink>}<a href={`/community-organizations/manage?organization=${organization.id}`} className="rounded-xl bg-white px-5 py-3 font-black text-slate-950">{organization.manager_verified_at ? "Request Manager Access" : "Claim this Organization"}</a><button type="button" onClick={shareProfile} className="rounded-xl border border-white/20 bg-white/10 px-5 py-3 font-black">Share</button></div>{shareMessage && <p className="mt-3 text-sm font-bold text-emerald-200">{shareMessage}</p>}</div>
