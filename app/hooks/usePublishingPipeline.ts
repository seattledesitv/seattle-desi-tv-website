...
export function usePublishingPipeline(supabase: SupabaseClient, publicationId: string) {
  const [outputs, setOutputs] = useState<PublicationOutputRecord[]>([]); const [attempts, setAttempts] = useState<PublishAttemptRecord[]>([]); const [loading, setLoading] = useState(true); const [busy, setBusy] = useState(false); const [error, setError] = useState(""); const [notice, setNotice] = useState("");
  ...
  return { outputs, attempts, loading, busy, error, notice, refresh, prepare, publish, cancel, download, copy };
}
