...
export function useOfferPricing(admin = false) {
  ...
  const refresh = useCallback(async () => { setLoading(true); setError(""); try { setPricing(admin ? await BusinessOfferService.listAllPricing() : await BusinessOfferService.listPricing()); } catch (cause) { setError(message(cause)); } finally { setLoading(false); } }, [admin]);
  useEffect(() => {
    // The hook owns the initial remote pricing synchronization.
    // eslint-disable-next-line react-hooks/set-state-in-effect
    void refresh();
  }, [refresh]);
  async function update(placement: OfferPlacement, changes: Record<string, unknown>) { setSaving(true); setError(""); try { await BusinessOfferService.updatePricing(placement, changes); await refresh(); } catch (cause) { setError(message(cause)); } finally { setSaving(false); } }
