...
function Step({ done, active, text }: { done?: boolean; active?: boolean; text: string }) {
  return <div className="flex items-center gap-2 text-sm"><span className={`grid h-6 w-6 place-items-center rounded-full text-xs font-black ${done ? "bg-green-600 text-white" : active ? "bg-yellow-400 text-slate-950" : "bg-slate-200 text-slate-500"}`}>{done ? "✓" : active ? "!" : "•"}</span><span className={done ? "font-bold text-slate-900" : active ? "font-bold text-yellow-900" : "text-slate-500"}>{text}</span></div>;
}
...
export default function MyEventsV2Page() {
  ...
  async function loadRows() {
    ...
    const { data, error } = await supabase
      .from("events")
      .select("id,title,date,location,status,created_at,updated_at,poc_email,poc_phone,description,ticket_url")
  ...
  function stats(eventId: string) {
    ...
    return { crewRows, influencerRows, workflow, media, approvedCrew, approvedInfluencers, mediaReceived, mediaRequested, submittedCrewMedia, coverageGreen, coverageYellow, coverageLabel };
  }
  ...
  function EventPortalCard({ event }: { event: any }) {
    ...
    return <button onClick={() => setSelectedEventId(event.id)} className={`rounded-3xl border p-5 text-left text-slate-950 shadow-xl transition hover:-translate-y-1 ${active ? "border-pink-500 bg-pink-50 ring-2 ring-pink-100" : "border-white bg-white"}`}>
      ...
      <p className="mt-3 text-xs font-bold text-slate-500">{ageText(s.workflow?.updated_at || event.updated_at || event.created_at)}</p>
    </button>;
  }
  ...
  return <main className="min-h-screen bg-slate-950 text-white">
    ...
    <section className="mx-auto max-w-7xl px-6 py-10">
      <div className="mb-8 flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
        <div>
          <p className="font-black uppercase tracking-wide text-pink-300">My Hub · Test Page</p>
          <h1 className="mt-2 text-4xl font-black md:text-5xl">My Events v2</h1>
      ...
      {events.length === 0 ? <div className="rounded-3xl border bg-white p-8 text-slate-950"><h2 className="text-2xl font-black">No events found</h2><p className="mt-2 text-gray-600">Your submitted events will appear here.</p></div> : <div className="grid gap-6 lg:grid-cols-[1fr_420px]">
        ...
        <aside className="h-fit rounded-3xl bg-white p-6 text-slate-950 shadow-2xl lg:sticky lg:top-6">
          {!selectedEvent || !selectedStats ? <p className="text-slate-500">Select an event to open the organizer portal.</p> : <div>
            ...
            <section className="mt-6 rounded-2xl bg-slate-50 p-4"><h3 className="text-lg font-black">Current Health</h3><p className={`mt-3 inline-flex rounded-full px-3 py-1 text-sm font-black ${selectedStats.coverageGreen ? "bg-green-100 text-green-700" : selectedStats.coverageYellow ? "bg-yellow-100 text-yellow-800" : "bg-slate-200 text-slate-700"}`}>{selectedStats.coverageLabel}</p><p className="mt-3 text-sm text-slate-600">Video status: <b>{label(selectedStats.workflow?.status)}</b></p></section>

            <section className="mt-5 rounded-2xl border p-4"><h3 className="text-lg font-black">Event Journey</h3><div className="mt-4 grid gap-3">
              <Step done text="Event submitted" />
              <Step done={String(selectedEvent.status).toLowerCase() === "approved"} active={String(selectedEvent.status).toLowerCase() === "pending"} text="Event approved" />
              <Step done={selectedStats.approvedCrew > 0 || selectedStats.approvedInfluencers > 0} active={!selectedStats.coverageGreen} text="Coverage confirmed" />
              <Step done={selectedStats.mediaReceived || selectedStats.submittedCrewMedia > 0} active={selectedStats.mediaRequested} text="Media received" />
              <Step done={Boolean(selectedStats.workflow)} active={!selectedStats.workflow && selectedStats.coverageGreen} text="Video workflow started" />
            ...
            <section className="mt-5 rounded-2xl border p-4"><h3 className="text-lg font-black">Coverage Team</h3><div className="mt-3 grid gap-2 text-sm"><p><b>Crew:</b> {selectedStats.approvedCrew} confirmed</p><p><b>Influencers:</b> {selectedStats.approvedInfluencers} confirmed</p><p><b>Media:</b> {selectedStats.mediaReceived ? "Organizer media received" : selectedStats.mediaRequested ? "Waiting for organizer media" : selectedStats.submittedCrewMedia > 0 ? "SDTV media submitted" : "Not received yet"}</p></div></section>

            {selectedStats.mediaRequested && !selectedStats.mediaReceived && <a href={`/events/media-request/${selectedEvent.id}`} className="mt-5 block rounded-xl bg-pink-600 px-5 py-4 text-center font-black text-white">Provide Media Folder</a>}
            <a href={`/events/${selectedEvent.id}`} className="mt-3 block rounded-xl bg-slate-950 px-5 py-4 text-center font-black text-white">Open Public Event Page</a>
          </div>}
        </aside>
      </div>}
    </section>
