...
export default function OnboardingPage() {
  ...
  async function submitOnboarding() {
    ...
    setMessage("Onboarding submitted. Your request is now awaiting team role access approval.");
  }
  ...
  return <main className="min-h-screen bg-slate-950 text-white px-6 py-10"><div className="max-w-4xl mx-auto"><a href="/login" className="text-pink-300 font-bold">← Back to Account</a><div className="mt-5 bg-white text-slate-950 rounded-3xl p-6 md:p-8 shadow-2xl"><h1 className="text-3xl md:text-4xl font-black">Volunteer Onboarding</h1><p className="text-gray-600 mt-2">Complete this after SDTV orientation. Once submitted, SDTV can approve your final team role access.</p>{loading && <div className="bg-slate-100 rounded-2xl p-5 mt-6 font-bold">{message}</div>}{!loading && message && <div className="bg-yellow-50 border border-yellow-200 text-yellow-900 rounded-2xl p-5 mt-6 font-bold whitespace-pre-line">{message}</div>}{!loading && canSubmit && <div className="grid gap-5 mt-6"><div className="grid md:grid-cols-2 gap-4"><label className="font-bold">Full Name<input className="w-full border rounded-xl p-3 mt-1 font-normal" value={form.full_name} onChange={(e) => updateField("full_name", e.target.value)} /></label><label className="font-bold">Phone<input className="w-full border rounded-xl p-3 mt-1 font-normal" value={form.phone} onChange={(e) => updateField("phone", e.target.value)} /></label><label className="font-bold">City / Area<input className="w-full border rounded-xl p-3 mt-1 font-normal" value={form.city} onChange={(e) => updateField("city", e.target.value)} /></label><label className="font-bold">Profile Photo URL<input className="w-full border rounded-xl p-3 mt-1 font-normal" placeholder="Paste Cloudinary/Google Drive/public image URL" value={form.photo_url} onChange={(e) => updateField("photo_url", e.target.value)} /></label></div><label className="font-bold">Areas you want to help with<textarea className="w-full border rounded-xl p-3 mt-1 font-normal min-h-24" placeholder="Events, interviews, video editing, radio, photography, social media, sponsorships..." value={form.interests} onChange={(e) => updateField("interests", e.target.value)} /></label><label className="font-bold">Availability<textarea className="w-full border rounded-xl p-3 mt-1 font-normal min-h-20" placeholder="Weekdays, weekends, evenings, event coverage availability..." value={form.availability} onChange={(e) => updateField("availability", e.target.value)} /></label><label className="font-bold">Relevant experience<textarea className="w-full border rounded-xl p-3 mt-1 font-normal min-h-20" placeholder="Media, volunteering, camera, hosting, editing, community work..." value={form.experience} onChange={(e) => updateField("experience", e.target.value)} /></label><div className="grid md:grid-cols-2 gap-4"><label className="font-bold">Emergency Contact<input className="w-full border rounded-xl p-3 mt-1 font-normal" value={form.emergency_contact} onChange={(e) => updateField("emergency_contact", e.target.value)} /></label><label className="font-bold">Emergency Phone<input className="w-full border rounded-xl p-3 mt-1 font-normal" value={form.emergency_phone} onChange={(e) => updateField("emergency_phone", e.target.value)} /></label></div><section><h2 className="text-2xl font-black mb-3">SDTV Agreement</h2><div className="h-72 overflow-y-auto border rounded-2xl p-4 bg-slate-50 whitespace-pre-line text-sm leading-6">{agreementText}</div><label className="flex gap-3 items-start mt-4 font-bold"><input type="checkbox" className="mt-1" checked={form.agreement_acknowledged} onChange={(e) => updateField("agreement_acknowledged", e.target.checked)} /><span>I have read and acknowledge the SDTV volunteer/contributor agreement above.</span></label></section><button type="button" onClick={submitOnboarding} disabled={submitting} className="bg-pink-600 text-white px-6 py-4 rounded-xl font-black disabled:opacity-60">{submitting ? "Submitting..." : "Submit Onboarding"}</button></div>}</div></div></main>;
}
