...
function workflowLabel(a: any, videoWorkflow?: any) {
  ...
  if (videoWorkflow?.status === "changes_requested") return "Changes Requested from Editor";
...
function hasVideoProgress(videoWorkflow?: any) { return Boolean(videoWorkflow?.id && ["ready_for_editing", "in_editing", "awaiting_crew_review", "changes_requested", "awaiting_admin_approval", "approved_for_publishing", "published_complete"].includes(videoWorkflow.status)); }
...
export default function MyAssignmentsPage() {
  ...
  async function loadAssignments(currentUser: any) {
    ...
    if (ids.length) {
      ...
      const workflowResult = await supabase.from("event_video_workflows").select("id,event_id,status,assigned_editor_email,crew_reviewer_email,crew_notes,editor_notes,updated_at").in("event_id", ids);
      if (!workflowResult.error) { const workflowMap: Record<string, any> = {}; (workflowResult.data || []).forEach((workflow: any) => { workflowMap[workflow.event_id] = workflow; }); setVideoWorkflowsByEventId(workflowMap); }
