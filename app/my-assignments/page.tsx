...
function workflowLabel(a: any) { if (a.coverage_completed) return "Submitted to Editor"; if (a.crew_confirmed) return "Confirmed"; return "Assigned - Awaiting Confirmation"; }
...
export default function MyAssignmentsPage() {
  ...
  async function createOrUpdateVideoWorkflow(assignment: any, coverageNotes: string, mediaUrl: string) {
    ...
    const payload = {
      ...
      status: existing.data?.status && existing.data.status !== "published_complete" ? existing.data.status : "ready_for_editing",
      raw_media_url: existing.data?.raw_media_url || mediaUrl || null,
      external_media_url: existing.data?.external_media_url || null,
      crew_reviewer_email: user.email || assignment.user_email || null,
      crew_notes: nextNotes,
      created_by: existing.data?.id ? undefined : user.id,
      updated_by: user.id,
    ...
    if (existing.data?.id) {
      ...
      const { error } = await supabase.from("event_video_workflows").update(payload).eq("id", existing.data.id);
      if (error) { setActionMessage(`Could not update editor workflow: ${error.message}`); return false; }
      return true;
    }
