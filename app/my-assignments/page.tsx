...
export default function MyAssignmentsPage() {
  ...
  async function loadAssignments(currentUser: any) {
    ...
    const nextNotes: Record<string, string> = {}; rows.forEach((row: any) => { nextNotes[row.id] = row.coverage_notes || ""; }); setNotes(nextNotes);
  ...
  async function markComplete(id: string) { await updateAssignment(id, { coverage_completed: true, crew_confirmed: true, coverage_notes: notes[id] || null, completed_at: new Date().toISOString() }, "Coverage marked complete."); }
  async function saveNotes(id: string) { await updateAssignment(id, { coverage_notes: notes[id] || null }, "Coverage notes saved."); }
