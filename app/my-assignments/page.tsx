...
function workflowLabel(a: any) { if (a.coverage_completed) return "Submitted"; if (a.crew_confirmed) return "Confirmed"; return "Assigned - Awaiting Confirmation"; }
...
export default function MyAssignmentsPage() {
  ...
  async function updateAssignment(id: string, payload: any, success: string) { setActionMessage("Updating assignment..."); const { error } = await supabase.from("event_crew_assignments").update(payload).eq("id", id); if (error) { setActionMessage(`Update failed: ${error.message}`); return; } setActionMessage(success); await loadAssignments(user); }
  ...
  async function submitAssignment(assignment: any) {
    ...
    const payload = { coverage_completed: true, crew_confirmed: true, coverage_notes: `${form.has_content === false ? "No media content from this crew member." : `Media link: ${form.media_url}`}${form.notes ? `\n\nNotes: ${form.notes}` : ""}`, completed_at: new Date().toISOString() };
    await updateAssignment(assignment.id, payload, form.has_content === false ? "Marked as no media content." : "Submitted to editor.");
  }
