...
type TeamMember = { id: string; name: string; title: string; image: string; email?: string | null; user_id?: string | null; linked_user?: string | null; profile_photo_url?: string | null; id_badge_url?: string | null; admin_role?: string | null; coverage_score?: number };
...
function inferredRole(member: TeamMember) {
  const explicit = normalizeRole(member.admin_role);
  if (explicit) return explicit;
  const title = normalizeText(member.title);
  if (/founder|co-founder|cofounder|president|chair|board/.test(title)) return "super_admin";
  if (/program manager|project manager|pm admin|pm_admin|pm\b|operations manager/.test(title)) return "pm_admin";
  if (/admin|director|lead|manager/.test(title)) return "admin";
  return "team_member";
}
function roleRank(member: TeamMember) { const role = inferredRole(member); if (role === "super_admin" || role === "superadmin") return 1; if (["pm_admin", "program_manager_admin", "project_manager_admin"].includes(role)) return 2; if (role.includes("admin")) return 3; return 4; }
function groupTitle(key: string) { if (key === "super_admin") return "Super Admins"; if (key === "pm_admin") return "PM Admins"; if (key === "admin") return "Admins"; return "Team Members"; }
function groupSubtitle(key: string) { if (key === "super_admin") return "Leadership and platform oversight."; if (key === "pm_admin") return "Program and project management leadership."; if (key === "admin") return "Studio operations and administrative support."; return "Community builders, creators, hosts, volunteers, and media team members."; }
function groupKey(member: TeamMember) { const role = inferredRole(member); if (role === "super_admin" || role === "superadmin") return "super_admin"; if (["pm_admin", "program_manager_admin", "project_manager_admin"].includes(role)) return "pm_admin"; if (role.includes("admin")) return "admin"; return "team_member"; }
...
export default function PublicTeamPage() {
  ...
  useEffect(() => {
    async function loadTeam() {
      const [teamResult, assignmentsResult, volunteerProfilesResult, userProfilesResult, adminsResult, hidden] = await Promise.all([
      ...
      function enrich(member: any) {
        ...
        const linkedUser = member.linked_user || member.user_id || "";
        ...
        const admin = adminByUserId[linkedUser] || adminByUserId[member.user_id || ""] || adminByEmail[email] || {};
        const enriched: TeamMember = { ...member, linked_user: member.linked_user || null, profile_photo_url: baseProfile.profile_photo_url || volunteerProfile.photo_url || null, id_badge_url: baseProfile.id_badge_url || null, admin_role: admin.role || null };
      ...
      if (teamResult.error) {
        ...
        if (legacyResult.error) setError(legacyResult.error.message); else setMembers((legacyResult.data || []).filter((member: any) => !isPubliclyHidden(member, hidden)).map(enrich).sort((a: any, b: any) => roleRank(a) - roleRank(b) || Number(b.coverage_score || 0) - Number(a.coverage_score || 0) || String(a.name || "").localeCompare(String(b.name || ""))));
      } else {
        setMembers((teamResult.data || []).filter((member: any) => !isPubliclyHidden(member, hidden)).map(enrich).sort((a: any, b: any) => roleRank(a) - roleRank(b) || Number(b.coverage_score || 0) - Number(a.coverage_score || 0) || String(a.name || "").localeCompare(String(b.name || ""))));
      }

      const profilesByEmail: Record<string, any> = {};
      (volunteerProfilesResult.data || []).forEach((profile: any) => { if (profile.email) profilesByEmail[profile.email] = profile; });
      ...
      const { start, end } = previousWeekRange(); const thanksMap: Record<string, CoverageThanks> = {};
      completedRows.filter((row: any) => { if (!row.completed_at) return false; const completed = new Date(row.completed_at); return completed >= start && completed <= end; }).forEach((row: any) => { const key = row.user_email || row.user_id || row.id; const profile = profilesByEmail[row.user_email || ""] || {}; if (!thanksMap[key]) thanksMap[key] = { key, name: displayName(row, profilesByEmail), email: row.user_email || "", eventTitles: [], count: 0, photo: profile.photo_url }; thanksMap[key].count += 1; if (row.event_title && !thanksMap[key].eventTitles.includes(row.event_title)) thanksMap[key].eventTitles.push(row.event_title); });
      setWeeklyThanks(Object.values(thanksMap).sort((a, b) => b.count - a.count)); setLoading(false);
    }
    loadTeam();
  }, []);
  ...
  const groupedMembers = useMemo(() => {
    const groups: Record<string, TeamMember[]> = { super_admin: [], pm_admin: [], admin: [], team_member: [] };
    members.forEach((member) => groups[groupKey(member)].push(member));
    return ["super_admin", "pm_admin", "admin", "team_member"].map((key) => ({ key, members: groups[key].sort((a, b) => Number(b.coverage_score || 0) - Number(a.coverage_score || 0) || String(a.name || "").localeCompare(String(b.name || ""))) })).filter((group) => group.members.length > 0);
  }, [members]);

  return (
    <main className="min-h-screen bg-slate-950 text-white"><SiteHeader /><section className="px-6 py-12"><div className="max-w-7xl mx-auto"><div className="mb-10 text-center"><p className="text-pink-300 font-black uppercase tracking-[0.3em]">Seattle Desi TV</p><h1 className="text-4xl md:text-6xl font-black mt-3">Meet Our Team</h1><p className="text-slate-300 max-w-2xl mx-auto mt-4">The community builders, creators, hosts, volunteers, and media team powering Seattle Desi TV.</p></div>{loading && <div className="bg-white/10 border border-white/10 rounded-2xl p-6">Loading team...</div>}{error && <div className="bg-red-100 text-red-800 rounded-2xl p-6 mb-8">{error}</div>}{!loading && <div className="space-y-12"><section className="bg-white/10 border border-white/10 rounded-3xl p-6 md:p-8"><div className="flex flex-col md:flex-row md:items-end md:justify-between gap-3 mb-6"><div><p className="text-pink-300 font-black uppercase tracking-wide">Team Spotlight</p><h2 className="text-3xl md:text-4xl font-black mt-2">Top 3 Volunteers</h2><p className="text-slate-300 mt-2">Recognizing volunteers based on completed SDTV event coverage.</p></div></div><div className="grid md:grid-cols-3 gap-5">{topVolunteers.map((volunteer, index) => <article key={volunteer.key} className="bg-white text-slate-950 rounded-2xl p-5 shadow-xl"><div className="flex items-center gap-4"><SafeImage src={volunteer.photo} alt={volunteer.name} className="w-16 h-16 rounded-2xl object-cover" fallbackClassName="w-16 h-16 rounded-2xl bg-pink-50 grid place-items-center text-pink-600 font-black" fallbackLabel={`#${index + 1}`} /><div><p className="text-xs font-black uppercase text-pink-600">#{index + 1} Volunteer</p><h3 className="text-xl font-black">{volunteer.name}</h3><p className="text-gray-600 text-sm">{volunteer.count} completed coverage item(s)</p></div></div></article>)}{topVolunteers.length === 0 && <p className="text-slate-300">No completed coverage stats yet.</p>}</div></section><section className="bg-white text-slate-950 rounded-3xl p-6 md:p-8"><p className="text-pink-600 font-black uppercase tracking-wide">Thank You</p><h2 className="text-3xl md:text-4xl font-black mt-2">Event Coverage Last Week</h2><p className="text-gray-600 mt-2">Coverage completed from {start.toLocaleDateString()} to {end.toLocaleDateString()}.</p><div className="grid md:grid-cols-2 xl:grid-cols-3 gap-5 mt-6">{weeklyThanks.map((person) => <article key={person.key} className="border rounded-2xl p-5 bg-slate-50"><div className="flex gap-4 items-start"><SafeImage src={person.photo} alt={person.name} className="w-14 h-14 rounded-2xl object-cover" fallbackClassName="w-14 h-14 rounded-2xl bg-white grid place-items-center text-pink-600 font-black" fallbackLabel="SDTV" /><div><h3 className="text-xl font-black">{person.name}</h3><p className="text-gray-600 text-sm">Covered {person.count} event(s)</p></div></div>{person.eventTitles.length > 0 && <ul className="list-disc ml-5 mt-4 text-sm text-gray-700 space-y-1">{person.eventTitles.slice(0, 4).map((title) => <li key={title}>{title}</li>)}</ul>}</article>)}{weeklyThanks.length === 0 && <p className="text-gray-600">No completed coverage was marked for last week yet.</p>}</div></section>{!error && <section><h2 className="text-3xl font-black mb-5">Team Members</h2><div className="space-y-10">{groupedMembers.map((group) => <div key={group.key} className="rounded-3xl border border-white/10 bg-white/[0.06] p-5 md:p-7"><div className="mb-5"><p className="text-pink-300 font-black uppercase tracking-wide">{group.members.length} profile{group.members.length === 1 ? "" : "s"}</p><h3 className="text-2xl md:text-3xl font-black mt-1">{groupTitle(group.key)}</h3><p className="text-slate-300 mt-2">{groupSubtitle(group.key)}</p></div><div className="grid sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">{group.members.map((member) => { const displayImage = member.id_badge_url || member.image || member.profile_photo_url || ""; return <article key={member.id} className="bg-white text-slate-950 rounded-2xl overflow-hidden shadow-xl"><SafeImage src={displayImage} alt={member.name} className="w-full h-72 object-cover" fallbackClassName="w-full h-72 bg-pink-50 grid place-items-center text-pink-600 font-black" fallbackLabel="SDTV" /><div className="p-5"><h2 className="text-xl font-black">{member.name}</h2><p className="text-pink-600 font-bold mt-1">{member.title}</p>{Number(member.coverage_score || 0) > 0 && <p className="mt-2 text-xs font-black uppercase text-slate-500">{member.coverage_score} completed coverage item(s)</p>}{member.id_badge_url && <p className="mt-2 text-xs font-black uppercase text-slate-500">SDTV ID Badge</p>}</div></article>; })}</div></div>)}{members.length === 0 && <p className="text-slate-300">No team members found.</p>}</div></section>}</div>}</div></section><SiteFooter /></main>
  );
}
