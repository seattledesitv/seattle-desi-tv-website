...
const defaultSettings: any = { eyebrow: "Seattle Desi TV", title: "Meet Our Team", subtitle: "The community builders, creators, hosts, volunteers, and media team powering Seattle Desi TV.", spotlightTitle: "Team Spotlight", weeklyTitle: "Event Coverage Last Week", teamOrderMode: "score" };
...
function sortMembers(a: TeamMember, b: TeamMember, mode: string) { const orderSort = Number(a.managed_order ?? 999) - Number(b.managed_order ?? 999); const scoreSort = Number(b.coverage_score || 0) - Number(a.coverage_score || 0); const nameSort = String(a.name || "").localeCompare(String(b.name || "")); return mode === "override" ? orderSort || scoreSort || nameSort : scoreSort || orderSort || nameSort; }
...
export default function PublicTeamPage() {
  ...
  useEffect(() => {
    async function loadTeam() {
      ...
      function enrich(member: any) {
        const email = String(member.email || "").toLowerCase(); const userId = member.user_id || "";
        const baseProfile = userProfileByUserId[userId] || userProfileByEmail[email] || {}; const volunteerProfile = volunteerByEmail[email] || {}; const admin = adminByUserId[userId] || adminByEmail[email] || {}; const managed = managedByMember[member.id] || {};
  ...
  const groupedMembers = useMemo(() => {
    ...
    return Object.keys(buckets).map((key) => ({ section: sectionMap[key], members: buckets[key].sort((a, b) => sortMembers(a, b, mode)) })).sort((a, b) => Number(a.section.display_order || 999) - Number(b.section.display_order || 999));
