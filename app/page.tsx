...
function HeroCarousel({ items }: { items: HeroItem[] }) {
  ...
  useEffect(() => { if (heroItems.length <= 1) return; const timer = setInterval(() => setCurrent((v) => (v + 1) % heroItems.length), 6000); return () => clearInterval(timer); }, [heroItems.length]);
...
export default function HomePage() {
  ...
  async function loadDynamicHomepage() {
    ...
    (completedResult.data || []).forEach((row: any) => { const email = String(row.user_email || "").toLowerCase(); const key = email || row.user_id || row.id; const profile = profilesByEmail[email] || {}; if (!recognitionMap[key]) recognitionMap[key] = { id: key, name: profile.full_name || row.user_email || "SDTV Volunteer", photo: profile.photo_url, count: 0 }; recognitionMap[key].count = (recognitionMap[key].count || 0) + 1; });
    const realPeople = Object.values(recognitionMap).sort((a, b) => (b.count || 0) - (a.count || 0)).slice(0, 5);
    setPeople(realPeople.length ? realPeople : (teamResult.data || []).map((p: any) => ({ id: p.id, name: p.name, photo: firstImage(p), count: 100 })).slice(0, 5));
    ...
    setHeroItems([...festivalHeroes, ...featuredEventHeroes, ...marketingHeroes].filter((hero) => hero.title).sort((a, b) => Number(a.display_order || 0) - Number(b.display_order || 0)) || fallbackHero);
  ...
  function renderSection(key: string) {
    ...
    if (key === "team") return <RecognitionWallPremium key="team" people={people} />;
