...
const topVideos = [
  {
    ...
    description: "Watch the latest community interviews, events, and cultural highlights.",
  ...
  },
  {
    ...
    description: "Explore our YouTube channel for SDTV coverage across the Pacific Northwest.",
...
function StatCard({ label, value, note, href }: { label: string; value: string | number; note: string; href?: string }) {
  const card = (
    <div className="rounded-2xl bg-white border shadow-sm p-5 h-full">
      <p className="text-sm font-black uppercase tracking-wide text-gray-500">{label}</p>
      <p className="text-3xl font-black text-pink-600 mt-2">{value}</p>
    ...
    <main className="min-h-screen bg-gradient-to-b from-white to-slate-100 text-slate-950">
      ...
      <header className="sticky top-0 z-40 bg-white/95 backdrop-blur border-b px-6 md:px-10 py-4">
        <div className="max-w-7xl mx-auto flex items-center justify-between gap-4">
          ...
          <nav className="hidden lg:flex items-center gap-3 font-bold text-sm">
            ...
            <a href="#social" className="hover:text-pink-600">Social Stats</a>
      ...
      <section className="relative overflow-hidden bg-slate-950 text-white">
        ...
        <div className="relative max-w-7xl mx-auto px-6 md:px-10 py-20 md:py-28">
          ...
          <div className="flex flex-wrap gap-4 mt-8">
            ...
            <a href="#videos" className="bg-white text-slate-950 px-6 py-4 rounded-xl font-black">Watch Top Videos</a>
      ...
      <section className="max-w-7xl mx-auto px-6 md:px-10 py-10">
        <div className="grid md:grid-cols-2 xl:grid-cols-5 gap-5">
          <StatCard label="Events Published" value={loadingDynamic ? "..." : counts.events} note="Approved community events" href="/events" />
          <StatCard label="Businesses Listed" value={loadingDynamic ? "..." : counts.businesses} note="Approved local businesses" href="/businesses" />
          <StatCard label="Coverage Requests" value={loadingDynamic ? "..." : counts.coverage} note="Organizer SDTV coverage asks" href="/studio/coverage" />
          <StatCard label="Team Members" value={loadingDynamic ? "..." : counts.team} note="Public SDTV team profiles" href="/team" />
          <StatCard label="Radio Hosts" value={loadingDynamic ? "..." : counts.radio} note="Radio team profiles" href="/radio-team" />
        </div>
      </section>
        ...
        <div className="bg-slate-950 text-white rounded-3xl p-8 md:p-10">
          <div className="flex items-end justify-between gap-4 mb-6">
            <div>
              ...
              <p className="text-slate-300 mt-1">Featured YouTube destinations for SDTV interviews, event coverage, and shorts.</p>
            </div>
            <a href="https://www.youtube.com/@SeattleDesiTV" target="_blank" rel="noreferrer" className="hidden md:inline-block text-pink-300 font-black">Open YouTube →</a>
          </div>
          <div className="grid md:grid-cols-3 gap-5">
            {topVideos.map((video) => <a key={video.url} href={video.url} target="_blank" rel="noreferrer" className="bg-white/10 border border-white/10 rounded-2xl overflow-hidden hover:bg-white/15 transition"><div className="h-44 bg-white/5 grid place-items-center overflow-hidden"><img src={video.thumbnail} alt={video.title} className="w-full h-full object-cover" /></div><div className="p-5"><h3 className="font-black text-xl">{video.title}</h3><p className="text-slate-300 text-sm mt-2">{video.description}</p></div></a>)}
          </div>
        </div>
      </section>

      <section id="social" className="max-w-7xl mx-auto px-6 md:px-10 py-10">
        <div className="flex items-end justify-between gap-4 mb-6">
          <div>
            <p className="text-pink-600 font-black uppercase tracking-wide">Social Reach</p>
            <h2 className="text-3xl md:text-4xl font-black">Social Media Stats</h2>
            <p className="text-gray-600 mt-1">A placeholder dashboard for followers, subscribers, views, reels, and reach across SDTV channels.</p>
          </div>
        </div>
        <div className="grid md:grid-cols-2 xl:grid-cols-4 gap-5">
          {socialStats.map((stat) => <StatCard key={stat.label} label={stat.label} value={stat.value} note={stat.note} href={stat.href} />)}
        </div>
        <p className="text-sm text-gray-500 mt-4">Next phase: connect these cards to live YouTube/Instagram/Facebook metrics or an admin-managed social stats table.</p>
      </section>

      <section className="max-w-7xl mx-auto px-6 md:px-10 py-10">
        <div className="flex items-end justify-between gap-4 mb-6">
          <div>
            <p className="text-pink-600 font-black uppercase tracking-wide">People behind SDTV</p>
            <h2 className="text-3xl md:text-4xl font-black">Team Spotlight</h2>
            <p className="text-gray-600 mt-1">Meet some of the community members supporting SDTV.</p>
          </div>
          <a href="/team" className="hidden md:inline-block text-pink-600 font-black">Meet the team →</a>
        </div>
        {featuredTeam.length === 0 ? <div className="bg-white border rounded-2xl p-8 text-gray-500">Team profiles will appear here after they are added in Studio.</div> : <div className="grid md:grid-cols-2 xl:grid-cols-4 gap-5">{featuredTeam.map((member) => { const image = firstImage(member); return <a key={member.id} href="/team" className="bg-white border rounded-2xl p-5 text-center shadow-sm hover:shadow-xl transition">{image ? <img src={image} alt={member.name} className="w-24 h-24 rounded-full object-cover mx-auto" /> : <div className="w-24 h-24 rounded-full bg-pink-50 text-pink-600 grid place-items-center mx-auto font-black">SDTV</div>}<h3 className="text-xl font-black mt-4">{member.name}</h3>{member.title && <p className="text-gray-600 text-sm mt-1">{member.title}</p>}</a>; })}</div>}
      </section>
