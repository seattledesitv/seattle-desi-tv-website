...
type RecognitionRow = { key: string; name: string; email: string; count: number; photo?: string };
...
function initials(name: string) { return name.split(" ").filter(Boolean).slice(0, 2).map((part) => part[0]?.toUpperCase()).join("") || "SD"; }
...
function HeroCarousel({ items }: { items: HeroItem[] }) {
  ...
  return <section key="home" className="relative overflow-hidden bg-slate-950 text-white h-[430px] md:h-[500px]"><div className={`absolute inset-0 bg-cover bg-center transition-all duration-700 ${isEventHero ? "opacity-35 blur-sm scale-105" : ""}`} style={{ backgroundImage: `url('${image}')` }} /><div className={`absolute inset-0 ${isEventHero ? "bg-gradient-to-r from-slate-950 via-slate-950/90 to-slate-950/70" : "bg-gradient-to-r from-slate-950 via-slate-950/80 to-slate-950/20"}`} /><div className={`relative max-w-7xl mx-auto px-6 md:px-10 h-full grid gap-6 items-center overflow-hidden ${isEventHero ? "lg:grid-cols-[1.05fr_0.95fr]" : "lg:grid-cols-1"}`}><div className="max-w-4xl"><p className="text-pink-300 font-black uppercase tracking-wide text-sm md:text-base">{item.badge || "Seattle Desi TV"}</p><h1 className={`${isEventHero ? "text-3xl md:text-5xl" : "text-4xl md:text-6xl"} font-black leading-tight mt-2 line-clamp-3`}>{item.title}</h1>{item.subtitle && <p className="text-base md:text-lg text-slate-200 max-w-3xl mt-4 line-clamp-2">{item.subtitle}</p>}<div className="flex flex-wrap gap-3 mt-6">{item.button_text && item.button_url && <a href={item.button_url} className="bg-pink-600 text-white px-5 py-3 rounded-xl font-black">{item.button_text}</a>}<a href="/radio" className="bg-white text-slate-950 px-5 py-3 rounded-xl font-black">Listen to Radio</a><a href="/businesses" className="border border-white/70 px-5 py-3 rounded-xl font-black">Local Businesses</a></div></div>{isEventHero && <div className="hidden md:flex justify-center lg:justify-end max-h-[390px]"><div className="w-full max-w-[280px] aspect-[4/5] rounded-[1.6rem] border border-white/20 bg-white/10 p-3 shadow-2xl backdrop-blur overflow-hidden"><div className="w-full h-full rounded-[1.2rem] bg-white grid place-items-center overflow-hidden"><img src={image} alt={item.title} className="w-full h-full object-contain" /></div></div></div>}</div>{heroItems.length > 1 && <><button aria-label="Previous hero" onClick={() => setCurrent((value) => (value - 1 + heroItems.length) % heroItems.length)} className="absolute left-4 md:left-8 top-1/2 -translate-y-1/2 bg-white/15 hover:bg-white/25 text-white rounded-full w-10 h-10 font-black">‹</button><button aria-label="Next hero" onClick={() => setCurrent((value) => (value + 1) % heroItems.length)} className="absolute right-4 md:right-8 top-1/2 -translate-y-1/2 bg-white/15 hover:bg-white/25 text-white rounded-full w-10 h-10 font-black">›</button><div className="absolute bottom-4 left-0 right-0 flex justify-center gap-2">{heroItems.map((hero, index) => <button key={hero.id} aria-label={`Show hero ${index + 1}`} onClick={() => setCurrent(index)} className={`h-3 rounded-full transition-all ${index === current ? "w-8 bg-pink-400" : "w-3 bg-white/50"}`} />)}</div></>}</section>;
}

function RecognitionMedal({ person, index }: { person: RecognitionRow; index: number }) {
  const frame = index === 0 ? "w-28 h-28 md:w-32 md:h-32 from-[#f4c24f] via-[#b98016] to-[#4b2404]" : index === 1 ? "w-24 h-24 md:w-28 md:h-28 from-slate-100 via-slate-300 to-slate-700" : index === 2 ? "w-24 h-24 md:w-28 md:h-28 from-[#f0a54b] via-[#9a5511] to-[#4b2404]" : "w-20 h-20 md:w-24 md:h-24 from-[#c02f72] via-[#5b0928] to-[#23020e]";
  const position = index === 0 ? "md:-mt-8" : index === 1 ? "md:mt-6" : index === 2 ? "md:mt-2" : "md:mt-16";
  return <a href="/team" className={`relative z-10 flex flex-col items-center ${position}`}><div className={`relative rounded-full bg-gradient-to-br ${frame} p-2 shadow-[0_14px_34px_rgba(0,0,0,.55)]`}><div className="absolute -left-1.5 -top-2 z-20 grid h-7 w-7 place-items-center rounded-full border-2 border-white bg-[#d6a12a] text-xs font-black text-[#190704] shadow-xl">{index + 1}</div><div className="h-full w-full overflow-hidden rounded-full bg-white p-1.5">{person.photo ? <img src={person.photo} alt={person.name} className="h-full w-full rounded-full object-cover" /> : <div className="grid h-full w-full place-items-center rounded-full bg-white text-lg font-black text-pink-600">{index < 3 ? initials(person.name) : "SDTV"}</div>}</div></div><div className="mt-2 text-center"><h3 className="max-w-[8rem] truncate text-sm font-black text-white drop-shadow-[0_2px_6px_rgba(0,0,0,.9)]">{person.name}</h3><p className="text-[11px] font-bold text-[#f4c24f] drop-shadow-[0_2px_5px_rgba(0,0,0,.9)]">{person.count} Point{person.count === 1 ? "" : "s"}</p></div></a>;
}

function RecognitionWall({ recognition, featuredTeam }: { recognition: RecognitionRow[]; featuredTeam: TeamRow[] }) {
  const fallbackPeople: RecognitionRow[] = featuredTeam.slice(0, 5).map((member, index) => ({ key: member.id, email: "", name: member.name, photo: firstImage(member), count: Math.max(5 - index, 1) }));
  const people = (recognition.length > 0 ? recognition : fallbackPeople).slice(0, 5);
  return <section key="team" className="relative overflow-hidden bg-[#090105] text-white"><div className="absolute inset-0 bg-[radial-gradient(circle_at_8%_18%,rgba(255,185,48,.20),transparent_16rem),radial-gradient(circle_at_92%_16%,rgba(255,185,48,.16),transparent_15rem),linear-gradient(100deg,#070105_0%,#3a071c_19%,#12030b_36%,#070105_52%,#18030c_68%,#3a071c_83%,#070105_100%)]" /><div className="absolute inset-0 opacity-80 bg-[radial-gradient(circle,rgba(212,160,23,.42)_1px,transparent_2px)] [background-size:34px_34px]" /><div className="absolute -left-20 bottom-2 h-28 w-[48%] rotate-[-8deg] rounded-full bg-[linear-gradient(90deg,transparent,rgba(196,138,20,.18),rgba(255,223,122,.62),rgba(196,138,20,.18),transparent)] blur-[1px]" /><div className="absolute -right-20 bottom-2 h-28 w-[48%] rotate-[8deg] rounded-full bg-[linear-gradient(90deg,transparent,rgba(196,138,20,.18),rgba(255,223,122,.62),rgba(196,138,20,.18),transparent)] blur-[1px]" /><div className="absolute inset-x-[-10%] -bottom-28 h-40 rounded-[50%] bg-white" /><div className="relative mx-auto grid min-h-[360px] max-w-7xl items-center gap-8 px-6 py-12 pb-20 md:grid-cols-[.82fr_1.18fr] md:px-10"><div><p className="text-xs font-black uppercase tracking-[.22em] text-[#d4a017]">🏆 SDTV Recognition Wall</p><h2 className="mt-3 text-4xl font-black leading-[.9] tracking-[-.06em] md:text-5xl">Thank You<br />Volunteers!</h2><p className="mt-4 max-w-md text-sm leading-6 text-white/85 md:text-base">Your time, talent and energy power everything we do at SDTV. You are the heart of our community.</p><a href="/team" className="mt-5 inline-flex rounded-full bg-[#d4a017] px-5 py-3 text-sm font-black text-[#180603] shadow-xl">View Leaderboard →</a></div><div className="relative"><div className="flex items-start justify-center gap-3 md:gap-5">{people.map((person, index) => <RecognitionMedal key={person.key} person={person} index={index} />)}</div></div></div></section>;
}
...
export default function HomePage() {
  ...
  async function loadDynamicHomepage() {
    ...
    const [eventsResult, businessesResult, teamResult, profilesResult, completedResult, settingsResult, socialResult, sponsorsResult, featuredEventsResult, heroBannerResult, festivalResult, eventsCount, businessesCount, coverageCount, teamCount, radioCount] = await Promise.all([
      ...
      supabase.from("event_crew_assignments").select("id,user_email,user_id,event_title,coverage_completed,completed_at").eq("coverage_completed", true).order("completed_at", { ascending: false }).limit(250),
    ...
    (profilesResult.data || []).forEach((profile: any) => { if (profile.email) profilesByEmail[String(profile.email).toLowerCase()] = profile; });
    const recognitionMap: Record<string, RecognitionRow> = {};
    (completedResult.data || []).forEach((row: any) => {
      ...
      if (!recognitionMap[key]) recognitionMap[key] = { key, email, name: profile.full_name || row.user_email || "SDTV Volunteer", photo: profile.photo_url, count: 0 };
      recognitionMap[key].count += 1;
    });
    setRecognition(Object.values(recognitionMap).sort((a, b) => b.count - a.count).slice(0, 5));
  ...
  const featuredTeam = useMemo(() => team.slice(0, 5), [team]);
  ...
  function renderSection(key: string) {
    ...
    if (key === "team") return <RecognitionWall key="team" recognition={recognition} featuredTeam={featuredTeam} />;
