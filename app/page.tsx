...
export default function HomePage() {
  ...
  async function loadDynamicHomepage() {
    ...
    (completedResult.data || []).forEach((row: any) => {
      ...
      if (!recognitionMap[key]) recognitionMap[key] = { key, email: row.user_email || "", name: profile.full_name || row.user_email || "SDTV Volunteer", photo: profile.photo_url, count: 0 };
      recognitionMap[key].count += 1;
    });
    setRecognition(Object.values(recognitionMap).sort((a, b) => b.count - a.count).slice(0, 3));
  ...
  function renderSection(key: string) {
    ...
    if (key === "team") return <section key="team" className="max-w-7xl mx-auto px-6 md:px-10 py-10"><div className="flex items-end justify-between gap-4 mb-6"><div><p className="text-pink-600 font-black uppercase tracking-wide">People behind SDTV</p><h2 className="text-3xl md:text-4xl font-black">Team Spotlight</h2><p className="text-gray-600 mt-1">Recognition for community members supporting SDTV coverage.</p></div><a href="/team" className="hidden md:inline-block text-pink-600 font-black">Meet the team →</a></div>{recognition.length > 0 ? <div className="grid md:grid-cols-3 gap-5">{recognition.map((person, index) => <a key={person.key} href="/team" className="bg-white border rounded-2xl p-5 shadow-sm hover:shadow-xl transition"><div className="flex gap-4 items-center">{person.photo ? <img src={person.photo} alt={person.name} className="w-16 h-16 rounded-2xl object-cover" /> : <div className="w-16 h-16 rounded-2xl bg-pink-50 text-pink-600 grid place-items-center font-black">#{index + 1}</div>}<div><p className="text-xs font-black uppercase text-pink-600">Top Volunteer #{index + 1}</p><h3 className="text-xl font-black">{person.name}</h3><p className="text-gray-600 text-sm">{person.count} completed coverage item(s)</p></div></div></a>)}</div> : featuredTeam.length === 0 ? <div className="bg-white border rounded-2xl p-8 text-gray-500">Recognition will appear here after coverage is marked complete. Team profiles can also be added in Studio.</div> : <div className="grid md:grid-cols-2 xl:grid-cols-4 gap-5">{featuredTeam.map((member) => { const image = firstImage(member); return <a key={member.id} href="/team" className="bg-white border rounded-2xl p-5 text-center shadow-sm hover:shadow-xl transition">{image ? <img src={image} alt={member.name} className="w-24 h-24 rounded-full object-cover mx-auto" /> : <div className="w-24 h-24 rounded-full bg-pink-50 text-pink-600 grid place-items-center mx-auto font-black">SDTV</div>}<h3 className="text-xl font-black mt-4">{member.name}</h3>{member.title && <p className="text-gray-600 text-sm mt-1">{member.title}</p>}</a>; })}</div>}</section>;
