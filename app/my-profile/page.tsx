...
function visibilityModeFromProfile(profile: any): VisibilityMode {
  if (profile?.keep_profile_private === false && (profile?.show_name_publicly || profile?.allow_social_credit)) return "public";
  if (profile?.show_name_publicly || profile?.allow_social_credit) return "credit";
  return "private";
}
...
function applyVisibilityMode(mode: VisibilityMode) {
  ...
  if (mode === "credit") return { keep_profile_private: true, show_name_publicly: true, allow_social_credit: true };
  return { keep_profile_private: true, show_name_publicly: false, allow_social_credit: false };
}

function visibilityLabel(mode: VisibilityMode) {
  if (mode === "public") return "Public Profile";
  if (mode === "credit") return "Credit OK";
  return "Private Profile";
}

function visibilityDescription(mode: VisibilityMode) {
  if (mode === "public") return "Your SDTV profile can be shown publicly where appropriate, such as team, directory, recognition, or published-content credits.";
  if (mode === "credit") return "Your profile remains private by default, but SDTV may credit your name for submitted or published content when appropriate.";
  return "Your profile is used internally by SDTV. Your name/profile should not be shown publicly unless you approve it separately.";
}
...
export default function MyProfilePage() {
  ...
  const [activeTab, setActiveTab] = useState<"profile" | "social" | "interests" | "privacy">("profile");
  ...
  async function saveProfile() {
    ...
    setSaving(true);
    setMessage("");
    const visibility = applyVisibilityMode(visibilityMode);
  ...
  const completeness = useMemo(() => {
    const fields = [form.full_name, form.phone, form.city, form.short_bio, form.profile_photo_url, form.interests?.length ? "interests" : ""];
    return Math.round((fields.filter(Boolean).length / fields.length) * 100);
  }, [form]);

  return <main className="min-h-screen bg-slate-950 text-white"><MyHubHeader /><section className="mx-auto max-w-6xl px-6 py-10"><div className="mb-8"><p className="font-black uppercase tracking-wide text-pink-300">My SDTV</p><h1 className="mt-2 text-4xl font-black md:text-5xl">My Profile</h1><p className="mt-2 text-slate-300">Centrally manage your SDTV identity, contact details, interests, social links, and public visibility.</p></div>{loading && <div className="rounded-3xl bg-white/10 p-6">{message}</div>}{!loading && !user?.id && <div className="rounded-3xl bg-white p-8 text-slate-950"><h2 className="text-2xl font-black">Login required</h2><p className="mt-2 text-slate-600">Please login to create or edit your SDTV profile.</p><a href="/login?next=/my-profile" className="mt-5 inline-flex rounded-xl bg-pink-600 px-5 py-3 font-black text-white">Login</a></div>}{!loading && user?.id && <div className="grid gap-5 lg:grid-cols-[340px_1fr]"><aside className="rounded-[2rem] bg-white p-6 text-slate-950 shadow-xl"><div className="grid place-items-center text-center"><div className="grid h-32 w-32 place-items-center overflow-hidden rounded-full bg-slate-100 text-4xl font-black text-pink-600">{form.profile_photo_url ? <img src={form.profile_photo_url} alt="Profile" className="h-full w-full object-cover" /> : (form.full_name || user.email || "S").slice(0,1).toUpperCase()}</div><h2 className="mt-4 text-2xl font-black">{form.full_name || "Your Profile"}</h2><p className="text-sm font-bold text-slate-500">{form.email}</p><span className="mt-3 rounded-full bg-pink-50 px-3 py-1 text-xs font-black text-pink-700">{visibilityLabel(visibilityMode)}</span></div><div className="mt-6 grid gap-3"><div className="rounded-2xl bg-slate-50 p-4"><p className="text-2xl font-black">{completeness}%</p><p className="text-xs font-bold text-slate-500">Profile completeness</p></div><div className="rounded-2xl bg-slate-50 p-4"><p className="text-sm font-black">Current role</p><p className="mt-1 text-sm text-slate-600">{String(role).replaceAll("_", " ")}</p></div><label className="block text-sm font-bold">Upload Photo<input type="file" accept="image/*" onChange={(e) => uploadPhoto(e.target.files?.[0])} className="mt-2 w-full rounded-xl border p-2 text-xs" /></label></div></aside><section className="rounded-[2rem] bg-white p-5 text-slate-950 shadow-xl md:p-8">{message && <div className="mb-5 rounded-2xl bg-yellow-50 p-4 font-bold text-yellow-900">{message}</div>}<div className="flex flex-wrap gap-2">{(["profile", "social", "interests", "privacy"] as const).map((tab) => <button key={tab} onClick={() => setActiveTab(tab)} className={`rounded-xl px-4 py-2 text-sm font-black ${activeTab === tab ? "bg-pink-600 text-white" : "bg-slate-100 text-slate-700"}`}>{tab === "profile" ? "Profile" : tab === "social" ? "Social" : tab === "interests" ? "Interests" : "Privacy"}</button>)}</div>{activeTab === "profile" && <div className="mt-6 grid gap-4 md:grid-cols-2"><label className="font-bold">Full Name<input value={form.full_name || ""} onChange={(e) => updateField("full_name", e.target.value)} className="mt-1 w-full rounded-xl border p-3 font-normal" /></label><label className="font-bold">Preferred Display Name<input value={form.preferred_name || ""} onChange={(e) => updateField("preferred_name", e.target.value)} className="mt-1 w-full rounded-xl border p-3 font-normal" /></label><label className="font-bold">Email<input disabled value={form.email || ""} className="mt-1 w-full rounded-xl border bg-slate-100 p-3 font-normal text-slate-600" /></label><label className="font-bold">Phone<input value={form.phone || ""} onChange={(e) => updateField("phone", e.target.value)} className="mt-1 w-full rounded-xl border p-3 font-normal" /></label><label className="font-bold">City<input value={form.city || ""} onChange={(e) => updateField("city", e.target.value)} className="mt-1 w-full rounded-xl border p-3 font-normal" /></label><label className="font-bold">State<input value={form.state || ""} onChange={(e) => updateField("state", e.target.value)} className="mt-1 w-full rounded-xl border p-3 font-normal" /></label><label className="font-bold md:col-span-2">Photo URL<input value={form.profile_photo_url || ""} onChange={(e) => updateField("profile_photo_url", e.target.value)} className="mt-1 w-full rounded-xl border p-3 font-normal" /></label><label className="font-bold md:col-span-2">Short Bio<textarea value={form.short_bio || ""} onChange={(e) => updateField("short_bio", e.target.value)} className="mt-1 min-h-28 w-full rounded-xl border p-3 font-normal" /></label></div>}{activeTab === "social" && <div className="mt-6 grid gap-4 md:grid-cols-2"><input value={form.instagram_url || ""} onChange={(e) => updateField("instagram_url", e.target.value)} placeholder="Instagram URL" className="rounded-xl border p-3" /><input value={form.linkedin_url || ""} onChange={(e) => updateField("linkedin_url", e.target.value)} placeholder="LinkedIn URL" className="rounded-xl border p-3" /><input value={form.facebook_url || ""} onChange={(e) => updateField("facebook_url", e.target.value)} placeholder="Facebook URL" className="rounded-xl border p-3" /><input value={form.website_url || ""} onChange={(e) => updateField("website_url", e.target.value)} placeholder="Website URL" className="rounded-xl border p-3" /><input value={form.youtube_url || ""} onChange={(e) => updateField("youtube_url", e.target.value)} placeholder="YouTube URL" className="rounded-xl border p-3 md:col-span-2" /></div>}{activeTab === "interests" && <div className="mt-6"><p className="font-black">What are you interested in?</p><p className="mt-1 text-sm text-slate-500">This helps SDTV recommend the right opportunities and contact you about relevant submissions.</p><div className="mt-4 flex flex-wrap gap-2">{interestOptions.map((item) => <button key={item} type="button" onClick={() => toggleInterest(item)} className={`rounded-xl px-3 py-2 text-sm font-black ${form.interests?.includes(item) ? "bg-pink-600 text-white" : "bg-slate-100 text-slate-700"}`}>{item}</button>)}</div></div>}{activeTab === "privacy" && <div className="mt-6 space-y-4"><div><p className="font-black">Public Visibility</p><p className="mt-1 text-sm text-slate-500">Choose how SDTV may use your profile publicly. Admin emergency visibility controls may still override this if needed.</p></div><div className="grid gap-3"><button type="button" onClick={() => setMode("private")} className={`rounded-2xl border p-4 text-left ${visibilityMode === "private" ? "border-pink-500 bg-pink-50" : "border-slate-200 bg-white"}`}><p className="font-black">Private Profile</p><p className="text-sm text-slate-600">Used internally only. Do not show my name/profile publicly by default.</p></button><button type="button" onClick={() => setMode("credit")} className={`rounded-2xl border p-4 text-left ${visibilityMode === "credit" ? "border-pink-500 bg-pink-50" : "border-slate-200 bg-white"}`}><p className="font-black">Credit OK</p><p className="text-sm text-slate-600">Keep my profile private, but SDTV may credit my name when publishing my content.</p></button><button type="button" onClick={() => setMode("public")} className={`rounded-2xl border p-4 text-left ${visibilityMode === "public" ? "border-pink-500 bg-pink-50" : "border-slate-200 bg-white"}`}><p className="font-black">Public Profile</p><p className="text-sm text-slate-600">SDTV may show my profile publicly where appropriate.</p></button></div><div className="rounded-2xl bg-slate-50 p-4 text-sm text-slate-600"><b>Current choice:</b> {visibilityLabel(visibilityMode)}. {visibilityDescription(visibilityMode)}</div><label className="block rounded-2xl bg-slate-50 p-4"><input type="checkbox" checked={Boolean(form.allow_sdtv_contact)} onChange={(e) => updateField("allow_sdtv_contact", e.target.checked)} /> <b>Allow SDTV to contact me</b><p className="ml-5 text-sm text-slate-600">Used for submissions, opportunities, follow-up questions, and community programs.</p></label></div>}<button onClick={saveProfile} disabled={saving || uploading} className="mt-8 rounded-xl bg-pink-600 px-6 py-4 font-black text-white disabled:opacity-60">{uploading ? "Uploading..." : saving ? "Saving..." : "Save Profile"}</button></section></div>}</section><SiteFooter /></main>;
}
