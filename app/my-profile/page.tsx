...
function visibilityDescription(mode: VisibilityMode) { if (mode === "public") return "Your SDTV profile can be shown publicly where appropriate, such as team, directory, recognition, or published-content credits."; if (mode === "credit") return "Your profile remains private by default, but SDTV may credit your name for submitted or published content when appropriate."; return "Your profile is used internally by SDTV. Your name/profile should not be shown publicly unless you approve it separately."; }
...
export default function MyProfilePage() {
  ...
  async function safeSingle(labelText: string, query: any, editUrl: string, statusField = "status") {
    const result = await query;
    if (result.error || !result.data) return null;
    const row = Array.isArray(result.data) ? result.data[0] : result.data;
    if (!row) return null;
    return { label: labelText, row, editUrl, status: row[statusField] || row.role || row.title || "active" };
  }
  ...
  async function loadConnectedProfiles(currentUser: any) {
    ...
    const items = await Promise.all([
      safeSingle("Base SDTV Profile", supabase.from("user_profiles").select("*").or(`user_id.eq.${id},email.eq.${email}`).limit(1).maybeSingle(), "/my-profile", "role"),
      safeSingle("Team Profile", supabase.from("team_members").select("*").or(`user_id.eq.${id},email.eq.${email}`).limit(1).maybeSingle(), "/my-hub", "title"),
      safeSingle("Radio Profile", supabase.from("radio_team_members").select("*").or(`user_id.eq.${id},email.eq.${email}`).limit(1).maybeSingle(), "/radio-team", "segment_name"),
      safeSingle("Volunteer Onboarding", supabase.from("volunteer_onboarding_submissions").select("*").or(`user_id.eq.${id},email.eq.${email}`).order("created_at", { ascending: false }).limit(1).maybeSingle(), "/my-role-requests", "status"),
      safeSingle("Influencer Profile", supabase.from("influencer_profiles").select("*").or(`user_id.eq.${id},email.eq.${email}`).limit(1).maybeSingle(), "/my-influencer-profile", "status"),
      safeSingle("Public Content Submissions", supabase.from("public_content_requests").select("*").or(`submitter_user_id.eq.${id},submitter_email.eq.${email}`).order("created_at", { ascending: false }).limit(1).maybeSingle(), "/my-video-assignments", "status"),
    ]);
