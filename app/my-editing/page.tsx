...
function statusLabel(value?: string | null) {
  return EDITOR_STATUSES.find((status) => status.key === value)?.label || String(value || "ready_for_editing").replaceAll("_", " ");
}
...
export default function MyEditingPage() {
  ...
  async function loadQueue(currentUser?: any, currentRole?: string) {
    ...
    if (error) {
      setActionMessage(`Could not load editing queue: ${error.message}`);
      setWorkflows([]);
      return;
    }
    ...
    const visibleRows = isAdminRole(activeRole)
      ? rows
      : rows.filter((row: any) => emailMatches(row.assigned_editor_email, activeEmail));
  ...
  async function init() {
    ...
    if (!currentUser) {
      ...
      setMessage("Please login to access your editing queue.");
      setLoading(false);
      return;
    }
    ...
    if (!isVideoEditorRole(nextRole) && !isAdminRole(nextRole)) {
      setMessage("Your account does not have video editor access yet.");
      setLoading(false);
      return;
    }
  ...
  function WorkflowCard({ workflow }: { workflow: any }) {
    ...
    return (
      <article className="bg-white text-slate-950 rounded-3xl p-5 shadow-xl border">
        <div className="flex gap-4">
          {flyer && <img src={flyer} alt={event.title || "Event flyer"} className="w-20 h-24 object-cover rounded-2xl border shrink-0" />}
          <div className="min-w-0">
            ...
            <p className="text-xs text-gray-400 mt-2">Updated {dateText(workflow.updated_at)}</p>
          </div>
        </div>

        <div className="text-sm text-gray-700 space-y-2 mt-4">
          {workflow.crew_notes && <p className="whitespace-pre-line"><b>Crew notes:</b><br />{workflow.crew_notes}</p>}
          {workflow.editor_notes && <p className="whitespace-pre-line"><b>Latest feedback / notes:</b><br />{workflow.editor_notes}</p>}
          {sourceUrl && <p><a href={sourceUrl} target="_blank" rel="noreferrer" className="text-pink-600 font-bold">Open source media</a></p>}
        </div>

        <div className="flex flex-wrap gap-2 mt-5">
          <a href={`/studio/video-production/${workflow.id}`} className="bg-slate-950 text-white px-4 py-2 rounded-xl font-black text-sm">Open Workflow</a>
          {workflow.status === "ready_for_editing" && <button onClick={() => updateWorkflow(workflow, { status: "in_editing", assigned_editor_email: workflow.assigned_editor_email || user?.email || null }, "Editing started.")} className="bg-pink-600 text-white px-4 py-2 rounded-xl font-black text-sm">Start Editing</button>}
          {workflow.status === "in_editing" && <a href={`/studio/video-production/${workflow.id}`} className="bg-pink-600 text-white px-4 py-2 rounded-xl font-black text-sm">Upload Draft</a>}
          {workflow.status === "changes_requested" && <a href={`/studio/video-production/${workflow.id}`} className="bg-yellow-500 text-white px-4 py-2 rounded-xl font-black text-sm">Upload Updated Draft</a>}
          {workflow.status === "approved_for_publishing" && <a href={`/studio/video-production/${workflow.id}`} className="bg-purple-700 text-white px-4 py-2 rounded-xl font-black text-sm">Publish / Mark Complete</a>}
        </div>
      </article>
    );
  }
  ...
  return (
    <main className="min-h-screen bg-slate-950 text-white">
      ...
      <section className="max-w-7xl mx-auto px-6 py-10">
        <a href="/my-hub" className="text-pink-300 font-bold">← Back to My Hub</a>
        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 mt-5 mb-8">
          <div>
            <p className="text-pink-300 font-black uppercase tracking-wide">Editor Workspace</p>
            <h1 className="text-4xl md:text-5xl font-black mt-2">My Editing Queue</h1>
            <p className="text-slate-300 mt-2">Ready for editing → draft upload → crew review → changes → publishing complete.</p>
            {user?.email && <p className="text-slate-400 text-sm mt-1">{user.email} · {role}</p>}
          </div>
          <button onClick={init} className="bg-white text-slate-950 px-5 py-3 rounded-xl font-bold w-fit">Refresh</button>
        </div>

        {loading && <div className="bg-white/10 border border-white/10 rounded-2xl p-6">{message}</div>}
        {!loading && !canAccess && <div className="bg-white text-slate-950 rounded-3xl p-8 max-w-xl"><h2 className="text-2xl font-black">Video Editor Access Required</h2><p className="text-gray-600 mt-3">{message}</p><a href="/my-role-requests" className="inline-block bg-pink-600 text-white px-5 py-3 rounded-xl font-bold mt-5">Request Access</a></div>}
        ...
        {!loading && canAccess && <div className="space-y-8">
          ...
          <div className="flex flex-wrap gap-3">
            <button onClick={() => setFilter("open")} className={`px-4 py-2 rounded-xl font-bold ${filter === "open" ? "bg-pink-600" : "bg-white/10"}`}>Open ({workflows.filter((workflow) => openStatuses.includes(workflow.status)).length})</button>
            ...
            {EDITOR_STATUSES.map((status) => <button key={status.key} onClick={() => setFilter(status.key)} className={`px-4 py-2 rounded-xl font-bold ${filter === status.key ? "bg-pink-600" : "bg-white/10"}`}>{status.label} ({workflows.filter((workflow) => workflow.status === status.key).length})</button>)}
          </div>

          <div className="grid lg:grid-cols-2 gap-5">
            {filteredWorkflows.map((workflow) => <WorkflowCard key={workflow.id} workflow={workflow} />)}
            {filteredWorkflows.length === 0 && <div className="bg-white/10 border border-dashed border-white/20 rounded-3xl p-8 text-slate-300">No editor workflows in this view.</div>}
          </div>
        </div>}
      </section>
      <SiteFooter />
