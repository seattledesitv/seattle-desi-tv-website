...
function Field({ label, children }: { label: string; children: React.ReactNode }) { return <label className="grid gap-1 text-sm font-black text-slate-800"><span>{label}</span>{children}</label>; }

type BusinessRow = {
  id: string; name: string; address?: string; website?: string; category?: string; discount?: string; offer?: string;
  poc_name?: string; poc_email?: string; poc_phone?: string; image?: string; image_urls?: string[];
  image_position_x?: number; image_position_y?: number; image_zoom?: number; image_display_mode?: DirectoryImageMode;
  status?: string; created_by?: string; owner_verified_at?: string; access_type?: "submitted" | "claimed"; manager_role?: string;
};
...
async function uploadBusinessImage(file: File) {
  ...
  const form = new FormData(); form.append("file", file); form.append("upload_preset", CLOUDINARY_UPLOAD_PRESET); form.append("folder", "seattle-desi-tv/businesses");
  const response = await fetch(`https://api.cloudinary.com/v1_1/${CLOUDINARY_CLOUD_NAME}/image/upload`, { method: "POST", body: form });
  const result = await response.json(); if (!response.ok) throw new Error(result?.error?.message || "Image upload failed."); return String(result.secure_url || "");
}

function BusinessImagePreview({ row }: { row: BusinessRow }) {
  const src = row.image || row.image_urls?.[0] || ""; if (!src) return null;
  ...
  return <div className="relative mt-6 aspect-[16/9] overflow-hidden rounded-2xl border bg-white">
    {mode === "blur" && <img src={src} alt="" aria-hidden="true" className="absolute inset-0 h-full w-full scale-110 object-cover blur-xl opacity-55"/>}
    <img src={src} alt={row.name} className={mode === "cover" ? "h-full w-full object-cover" : "relative z-10 h-full w-full object-contain p-3"} style={mode === "cover" ? { objectPosition: `${row.image_position_x ?? 50}% ${row.image_position_y ?? 50}%`, transform: `scale(${row.image_zoom ?? 1})` } : undefined}/>
  </div>;
}
...
export default function MyBusinessesPage() {
  ...
  const [imageCrop, setImageCrop] = useState<DirectoryImageCrop>({ x: 50, y: 50, zoom: 1, mode: "cover" });
  const [searchText, setSearchText] = useState(""); const [statusFilter, setStatusFilter] = useState("all"); const [user, setUser] = useState<any>(null);
  ...
  function startEdit(row: BusinessRow) {
    setSelectedId(row.id); setEditingId(row.id); setEditTab("general"); setImageFile(null);
    setImageCrop({ x: Number(row.image_position_x ?? 50), y: Number(row.image_position_y ?? 50), zoom: Number(row.image_zoom ?? 1), mode: row.image_display_mode || "cover" });
  ...
  async function saveBusiness(row: BusinessRow) {
    if (!user?.id) return; setSaving(true); setMessage("");
    try {
    ...
    } catch (error: any) { setMessage(error?.message || "Could not save the business listing."); } finally { setSaving(false); }
  }
  ...
  return <main className="min-h-screen bg-slate-950 text-white"><MyHubHeader/><section className="mx-auto max-w-7xl px-6 py-10">
    <div className="mb-8 flex flex-col gap-4 md:flex-row md:items-end md:justify-between"><div><p className="font-black uppercase tracking-wide text-pink-300">My Hub</p><h1 className="mt-2 text-4xl font-black md:text-5xl">My Businesses</h1><p className="mt-2 text-slate-300">{message}</p></div><div className="flex flex-wrap gap-2"><a href="/my-businesses/offers" className="rounded-xl bg-amber-400 px-5 py-3 text-center font-black text-slate-950">Manage Offers</a><a href="/businesses" className="rounded-xl bg-pink-600 px-5 py-3 text-center font-black text-white">Business Directory</a></div></div>
    <section className="mb-6 rounded-3xl bg-white/10 p-4"><div className="grid gap-3 md:grid-cols-[1fr_180px]"><input value={searchText} onChange={(e) => setSearchText(e.target.value)} placeholder="Search your businesses..." className="rounded-xl bg-white px-4 py-3 font-bold text-slate-950"/><select value={statusFilter} onChange={(e) => setStatusFilter(e.target.value)} className="rounded-xl bg-white px-4 py-3 font-bold text-slate-950"><option value="all">All statuses</option><option value="pending">Pending</option><option value="approved">Approved</option><option value="rejected">Rejected</option></select></div></section>
    {rows.length === 0 ? <div className="rounded-3xl bg-white p-8 text-slate-950">No submitted or claimed businesses found.</div> : <div className="grid gap-6 xl:grid-cols-[420px_1fr]">
      ...
      <section className="min-h-[560px] rounded-[2rem] bg-white p-6 text-slate-950 shadow-2xl">
        {!selectedRow ? <p>Select a business.</p> : editingId === selectedRow.id ? <div className="grid gap-5">
          <div className="flex flex-wrap justify-between gap-3"><div><p className="text-xs font-black uppercase text-pink-600">Manage business</p><h2 className="text-3xl font-black">{selectedRow.name}</h2></div><button onClick={() => setEditingId("")} className="rounded-xl bg-slate-100 px-4 py-3 font-black">Cancel</button></div>
          ...
          {editTab === "general" ? <><div className="grid gap-4 md:grid-cols-2"><Field label="Business name"><input className="rounded-xl border p-3 font-normal" value={editForm.name} onChange={(e) => setEditForm({...editForm,name:e.target.value})}/></Field><Field label="Category"><input className="rounded-xl border p-3 font-normal" value={editForm.category} onChange={(e) => setEditForm({...editForm,category:e.target.value})}/></Field><Field label="Address"><input className="rounded-xl border p-3 font-normal" value={editForm.address} onChange={(e) => setEditForm({...editForm,address:e.target.value})}/></Field><Field label="Website"><input className="rounded-xl border p-3 font-normal" value={editForm.website} onChange={(e) => setEditForm({...editForm,website:e.target.value})}/></Field><Field label="Discount / special offer"><input className="rounded-xl border p-3 font-normal" value={editForm.discount} onChange={(e) => setEditForm({...editForm,discount:e.target.value})}/></Field><Field label="Contact name"><input className="rounded-xl border p-3 font-normal" value={editForm.poc_name} onChange={(e) => setEditForm({...editForm,poc_name:e.target.value})}/></Field><Field label="Contact email"><input className="rounded-xl border p-3 font-normal" value={editForm.poc_email} onChange={(e) => setEditForm({...editForm,poc_email:e.target.value})}/></Field><Field label="Contact phone"><input className="rounded-xl border p-3 font-normal" value={editForm.poc_phone} onChange={(e) => setEditForm({...editForm,poc_phone:e.target.value})}/></Field><Field label="Description / offer"><textarea className="min-h-28 rounded-xl border p-3 font-normal" value={editForm.offer} onChange={(e) => setEditForm({...editForm,offer:e.target.value})}/></Field></div><button onClick={() => saveBusiness(selectedRow)} disabled={saving} className="rounded-xl bg-pink-600 px-5 py-4 font-black text-white disabled:opacity-60">{saving ? "Saving..." : "Save Business Listing"}</button></> : <><DirectoryImageCropper src={selectedRow.image || selectedRow.image_urls?.[0] || ""} value={imageCrop} onChange={setImageCrop} onFileChange={setImageFile} label="Upload or replace business cover image"/><button onClick={() => saveMedia(selectedRow)} disabled={saving} className="rounded-xl bg-pink-600 px-5 py-4 font-black text-white disabled:opacity-60">{saving ? "Saving..." : "Save Image Presentation"}</button></>}
        </div> : <div><div className="flex flex-wrap items-start justify-between gap-4"><div><div className="flex flex-wrap items-center gap-2"><h2 className="text-3xl font-black">{selectedRow.name}</h2>{selectedRow.owner_verified_at && <span className="rounded-full bg-emerald-50 px-3 py-1 text-xs font-black text-emerald-700">✓ Owner verified</span>}</div><p className="mt-2 text-slate-600">{selectedRow.category || "Business"} · {selectedRow.address || "No address"}</p></div><button onClick={() => startEdit(selectedRow)} className="rounded-xl bg-pink-600 px-5 py-3 font-black text-white">Manage business</button></div><BusinessImagePreview row={selectedRow}/><div className="mt-6 grid gap-4 rounded-2xl bg-slate-50 p-5 text-sm md:grid-cols-2"><p><b>Access:</b><br/>{selectedRow.access_type === "claimed" ? `Claimed ${selectedRow.manager_role || "owner"}` : "Submitted by you"}</p><p><b>Status:</b><br/>{statusText(selectedRow.status)}</p><p><b>Website:</b><br/>{selectedRow.website || "—"}</p><p><b>Contact:</b><br/>{[selectedRow.poc_name,selectedRow.poc_email,selectedRow.poc_phone].filter(Boolean).join(" · ") || "—"}</p><p className="md:col-span-2"><b>Description / offer:</b><br/>{selectedRow.offer || "—"}</p></div></div>}
      </section>
    </div>}
