...
export default function BusinessesPage() {
  ...
  const visibleBusinesses = useMemo(() => {
    ...
    const filtered = businesses.filter((business) => {
      const matchesSearch = !query || [business.name, business.address, business.category, business.offer, business.discount].some((value) => String(value || "").toLowerCase().includes(query));
    ...
    return [...filtered].sort((a, b) => sortOrder === "name" ? a.name.localeCompare(b.name) : sortOrder === "city" ? a.address.localeCompare(b.address) : 0);
  }, [businesses, directorySearch, categoryFilter, offerFilter, sortOrder]);
  const filtersActive = Boolean(directorySearch.trim() || categoryFilter !== "all" || offerFilter !== "all" || sortOrder !== "newest");
  function clearFilters() { setDirectorySearch(""); setCategoryFilter("all"); setOfferFilter("all"); setSortOrder("newest"); }
  ...
  return (
    <main className="min-h-screen bg-slate-50 text-[#081024]"><SiteHeader /><section className="px-6 md:px-14 py-10"><div className="max-w-7xl mx-auto"><div className="flex flex-col md:flex-row md:items-end md:justify-between gap-4 mb-8"><div><p className="text-pink-600 font-black uppercase tracking-wide">Seattle Desi Marketplace</p><h1 className="text-4xl md:text-5xl font-black mt-2">Local Business Directory</h1><p className="text-gray-500 mt-2">Approved Seattle Desi TV local business listings.</p><p className="text-sm text-gray-500 mt-2">{message}</p></div><div className="flex flex-wrap gap-3"><button type="button" onClick={() => setShowSubmitForm((value) => !value)} className="bg-pink-600 text-white px-5 py-3 rounded-xl font-bold">{showSubmitForm ? "Hide Add Business" : "Add Business"}</button><button type="button" onClick={loadBusinesses} className="border border-pink-600 text-pink-600 px-5 py-3 rounded-xl font-bold bg-white">Refresh Businesses</button></div></div>
      ...
      {businesses.length > 0 && <section className="mb-7 rounded-2xl border bg-white p-4 shadow-sm"><div className="grid gap-3 lg:grid-cols-[minmax(0,2fr)_1fr_1fr_1fr_auto]"><label className="relative"><span className="sr-only">Search businesses</span><input value={directorySearch} onChange={(e) => setDirectorySearch(e.target.value)} placeholder="Search business, city, service or offer..." className="w-full rounded-xl border px-4 py-3 pl-10"/><span className="pointer-events-none absolute left-3 top-3.5 text-slate-400">⌕</span></label><select value={categoryFilter} onChange={(e) => setCategoryFilter(e.target.value)} className="rounded-xl border px-3 py-3"><option value="all">All categories</option>{categories.map((category) => <option key={category} value={category}>{category}</option>)}</select><select value={offerFilter} onChange={(e) => setOfferFilter(e.target.value)} className="rounded-xl border px-3 py-3"><option value="all">All businesses</option><option value="offers">Offers available</option><option value="no_offers">No current offer</option></select><select value={sortOrder} onChange={(e) => setSortOrder(e.target.value)} className="rounded-xl border px-3 py-3"><option value="newest">Newest first</option><option value="name">Name A–Z</option><option value="city">City/address A–Z</option></select><button type="button" onClick={clearFilters} disabled={!filtersActive} className="rounded-xl border border-pink-200 px-4 py-3 font-bold text-pink-600 disabled:cursor-not-allowed disabled:opacity-40">Clear</button></div><div className="mt-3 flex flex-wrap items-center justify-between gap-2 text-sm text-slate-500"><p>Showing <strong className="text-slate-900">{visibleBusinesses.length}</strong> of {businesses.length} businesses</p>{filtersActive && <p>Filters update instantly as you type.</p>}</div></section>}

      <section>{businesses.length === 0 ? <div className="border rounded-2xl p-8 text-gray-500 bg-white">{message}</div> : visibleBusinesses.length === 0 ? <div className="border rounded-2xl p-8 text-center bg-white"><h2 className="text-xl font-black">No businesses match these filters</h2><p className="mt-2 text-gray-500">Try a different name, city, category, or clear the filters.</p><button type="button" onClick={clearFilters} className="mt-4 rounded-xl bg-pink-600 px-5 py-3 font-bold text-white">Clear filters</button></div> : <div className="grid md:grid-cols-2 xl:grid-cols-3 gap-6">{visibleBusinesses.map((business) => { const imgs = getImages(business); const expanded = Boolean(expandedOffers[business.id]); const longOffer = String(business.offer || "").length > 180; return <article key={business.id} className="border rounded-2xl overflow-hidden shadow-sm bg-white">{imgs.length > 0 ? <SafeImage src={imgs[0]} alt={business.name} className="w-full h-56 object-cover bg-gray-100" fallbackClassName="w-full h-56 bg-pink-50 grid place-items-center text-pink-600 font-black" fallbackLabel="Seattle Desi TV" widthHint={900} /> : <div className="w-full h-56 bg-pink-50 grid place-items-center text-pink-600 font-black">Seattle Desi TV</div>}<div className="p-5"><div className="flex items-start justify-between gap-3"><div><h2 className="text-xl font-black">{business.name}</h2><p className="text-gray-500 mt-1">{business.address}</p></div>{business.category && <span className="text-xs font-black bg-pink-50 text-pink-700 px-3 py-1 rounded-full">{business.category}</span>}</div>{business.discount && <p className="mt-3 text-sm font-black text-green-700">{business.discount}</p>}{business.offer && <div className="mt-3"><p className={`text-sm text-gray-600 whitespace-pre-line ${expanded ? "" : "line-clamp-4"}`}>{business.offer}</p>{longOffer && <button type="button" onClick={() => setExpandedOffers((current) => ({ ...current, [business.id]: !expanded }))} className="mt-2 text-sm font-black text-pink-600">{expanded ? "Show less" : "Show more"}</button>}</div>}<div className="flex flex-wrap gap-3 mt-5">{business.website && <CheckedExternalLink href={business.website} notFoundMessage="Page not found. This business link is not available." className="bg-pink-600 text-white px-4 py-2 rounded-lg font-bold text-sm disabled:opacity-60">Website</CheckedExternalLink>}<a href={`https://www.google.com/maps?q=${encodeURIComponent(business.address)}`} target="_blank" rel="noreferrer" className="border px-4 py-2 rounded-lg font-bold text-sm">Map</a></div></div></article>; })}</div>}</section>
