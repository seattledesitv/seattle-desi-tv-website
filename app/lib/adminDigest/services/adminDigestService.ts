...
import type { DailyAdminDigest, DigestRoleRequest, DigestUser } from "../types";
...
export const AdminDigestService = {
  ...
  subject(digest: DailyAdminDigest) {
    const total = digest.users.length + digest.volunteerRequests.length + digest.teamMemberRequests.length;
    return `SDTV daily user activity: ${total} new update${total === 1 ? "" : "s"}`;
  },

  html(digest: DailyAdminDigest, studioUrl: string) {
    return `<div style="font-family:Arial,sans-serif;max-width:760px;margin:auto;line-height:1.5;color:#334155"><div style="background:#080d1d;color:#fff;padding:24px;border-radius:18px 18px 0 0"><p style="margin:0;color:#f9a8d4;font-weight:bold;text-transform:uppercase">Seattle Desi TV</p><h1 style="margin:8px 0 0">Daily user activity</h1><p style="margin:10px 0 0;color:#cbd5e1">${escapeHtml(pacificTime(digest.from))} through ${escapeHtml(pacificTime(digest.to))}</p></div><div style="padding:24px;border:1px solid #e2e8f0;border-top:0;border-radius:0 0 18px 18px">${table("New registrations", ["Name", "Email", "Registered"], userRows(digest.users), digest.users.length)}${table("Volunteer requests", ["Email", "Status", "Requested"], requestRows(digest.volunteerRequests, "No new volunteer requests during this reporting period."), digest.volunteerRequests.length)}${table("Team-member requests", ["Email", "Status", "Requested"], requestRows(digest.teamMemberRequests, "No new team-member requests during this reporting period."), digest.teamMemberRequests.length)}<p style="margin-top:26px"><a href="${escapeHtml(studioUrl)}" style="display:inline-block;background:#db2777;color:#fff;text-decoration:none;padding:12px 18px;border-radius:10px;font-weight:bold">Review in SDTV Studio</a></p><p style="margin-top:18px;color:#64748b;font-size:13px">This automated report covers the previous 24 hours and contains administrative account information. Please do not forward it outside the SDTV team.</p></div></div>`;
  },
};
