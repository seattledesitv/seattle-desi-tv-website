...
const paths: Record<SeoEntityKind, string> = {
  event: "/events/", classified: "/classifieds/", organization: "/community-organizations/",
...
export const getEntity = cache(async (kind: SeoEntityKind, id: string): Promise<SeoEntity | null> => {
  const row = await repository.getPublicEntity(kind, id);
  if (!row) return null;
  ...
  return {
    id, kind, title: title || "Seattle Desi TV", description: description(row.description || row.summary, fallback),
    path: `${paths[kind]}${id}`, image: image(row), modifiedAt: text(row.updated_at) || null,
    ...
    location: text(row.location) || null, organizationName: text(row.organization_name) || null,
...
export async function listSitemapEntries(): Promise<SeoSitemapEntry[]> {
  ...
  const map = (items: Array<Record<string, unknown>>, prefix: string) => items.map((row) => ({ path: `${prefix}${text(row.id)}`, modifiedAt: text(row.updated_at || row.created_at) || null }));
  return [...map(rows.events, "/events/"), ...map(rows.classifieds, "/classifieds/"), ...map(rows.organizations, "/community-organizations/"), ...map(rows.releases, "/press-releases/"), ...map(rows.publications, "/publications/")];
}
