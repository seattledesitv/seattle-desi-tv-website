...
const entityColumns: Record<SeoEntityKind, string> = {
  ...
  classified:
    "id,title,description,location,image_urls,price_cents,updated_at,created_at,starts_at",
  organization: "id,name,description,location,image,updated_at,created_at",
  press_release:
    "id,title,summary,organization_name,location,release_date,image_urls,published_at,updated_at",
  publication:
    "id,name,description,edition_label,cover_image_url,created_at,updated_at",
};
...
export async function getPublicEntity(
  ...
  let query = db
    .from(entityTables[kind])
    .select(entityColumns[kind])
    .eq("id", id);
  if (
    (kind === "event" ||
      kind === "business" ||
