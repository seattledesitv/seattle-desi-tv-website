...
function validate(input: BusinessOfferInput) { if (!input.business_id) throw new Error("Choose a business."); if (input.title.trim().length < 3) throw new Error("Offer title must be at least 3 characters."); if (!input.starts_at) throw new Error("Offer start date is required."); if (input.ends_at && input.ends_at < input.starts_at) throw new Error("Offer end date cannot be before its start date."); if (input.destination_url && !/^https?:\/\//i.test(input.destination_url)) throw new Error("Offer link must begin with http:// or https://."); }
