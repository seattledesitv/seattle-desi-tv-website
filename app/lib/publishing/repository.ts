...
export async function listPublications(supabase: SupabaseClient): Promise<PublicationRecord[]> {
...
export async function createPublication(
  ...
  const { data, error } = await supabase
    ...
    .insert({
      ...
      edition_label: clean(input.edition_label),
  ...
  const sectionRows = DEFAULT_PUBLICATION_SECTIONS.map((section) => ({
    ...section,
    publication_id: data.id,
  }));
  const sectionResult = await supabase.from("publication_sections").insert(sectionRows);
  if (sectionResult.error) {
    await supabase.from("publications").delete().eq("id", data.id);
...
export async function updatePublication(
  ...
  changes: Partial<PublicationDraftInput> & { status?: PublicationStatus },
  userId: string,
): Promise<PublicationRecord> {
...
export async function duplicatePublication(
  ...
  const duplicate = await createPublication(supabase, {
  ...
  const { data: sourceSections, error } = await supabase
    .from("publication_sections")
    .select("section_key,title,introduction,included,sort_order,section_type,source_config,generated_content,manual_content,is_manually_edited")
    .eq("publication_id", source.id)
    .order("sort_order");
  ...
  const copyResult = await supabase.from("publication_sections").insert(
    sourceSections.map((section) => ({ ...section, publication_id: duplicate.id })),
  );
