...
function imageFrom(row: any) {
  return absoluteUrl(
    firstArrayValue(row?.image_urls) || row?.image || row?.image_url || row?.photo_url || row?.profile_photo_url ||
    row?.logo_url || row?.logo || row?.thumbnail || row?.thumbnailUrl || row?.mediaUrl || row?.media_url || "",
  );
}
...
async function newTeamMembers(limit: number) {
  ...
  return {
    items: rows.map((row: any) => itemFrom({ ...row, meta: row.created_at ? `Joined ${dateLabel(row.created_at)}` : "New team member", description: row.title || "Seattle Desi TV Team Member" }, `${SITE_URL}/team`, { cta: "Meet the team", useFallbackImage: true, badge: "New" })),
    reason: rows.length ? "" : "No public team members found.",
  };
}
...
async function teamLeaderboard(limit: number) {
  ...
  profiles.forEach((profile: any) => {
    if (profile.user_id) byUser[`u:${profile.user_id}`] = profile;
    if (profile.email) byUser[`e:${String(profile.email).toLowerCase()}`] = profile;
  });
  const scores: Record<string, any> = {};
  (assignmentsResult.data || []).forEach((row: any) => {
    const key = row.user_id ? `u:${row.user_id}` : `e:${String(row.user_email || "").toLowerCase()}`;
    if (!key || key === "e:") return;
    const profile = byUser[key] || byUser[`e:${String(row.user_email || "").toLowerCase()}`] || {};
    if (!scores[key]) scores[key] = { key, count: 0, name: profile.full_name || row.user_email || "SDTV Team Member", image: profile.profile_photo_url || profile.id_badge_url || "" };
    scores[key].count += 1;
  });
  const ranked = Object.values(scores).sort((a: any, b: any) => b.count - a.count).slice(0, limit);
  return {
    items: ranked.map((row: any, index: number) => itemFrom({ ...row, title: `${index + 1}. ${row.name}`, description: `${row.count} completed event coverage assignment${row.count === 1 ? "" : "s"}`, meta: index === 0 ? "Community Champion" : `Rank #${index + 1}` }, `${SITE_URL}/team`, { cta: "View team", useFallbackImage: true, badge: index < 3 ? ["Gold", "Silver", "Bronze"][index] : "" })),
    reason: ranked.length ? "" : "No completed coverage activity found.",
  };
}

async function communityStats(limit: number) {
  const sources = [
    ["Events", "events"], ["Businesses", "local_businesses"], ["Organizations", "community_organizations"],
    ["Community Groups", "community_groups"], ["Team Members", "team_members"], ["Newsletter Subscribers", "newsletter_subscribers"],
  ];
  const results = await Promise.all(sources.map(async ([label, table]) => {
    let query: any = supabase.from(table).select("*", { count: "exact", head: true });
    if (table === "newsletter_subscribers") query = query.eq("status", "active");
    const { count, error } = await query;
    return { label, count: error ? 0 : Number(count || 0) };
  }));
...
async function milestones(limit: number) {
  ...
  const items = stats.items.map((item: any) => {
    const value = Number(String(item.title).replace(/,/g, ""));
    const hit = thresholds.filter((threshold) => value >= threshold).pop();
    if (!hit) return null;
    return { ...item, title: `We reached ${hit.toLocaleString("en-US")} ${item.text}!`, text: `Thank you to everyone who helped Seattle Desi TV reach this community milestone.`, meta: "Milestone Celebration", badge: "Celebrate", image: FALLBACK_IMAGE, imageFit: "contain" };
...
async function tableItems(sectionKey: string, limit: number): Promise<TableResult> {
  ...
  const items = rows.slice(0, limit).map((row: any) => {
    const meta = row.date ? `${dateLabel(row.date)}${row.location ? ` · ${row.location}` : ""}` : [row.category, row.city, row.platform].filter(Boolean).join(" · ");
...
export async function generateNewsletterDraft(settings: any[]) {
  ...
  return {
    ...
    preheader: "Upcoming events, latest videos, new team members, community achievements, businesses, organizations, and highlights from Seattle Desi TV.",
