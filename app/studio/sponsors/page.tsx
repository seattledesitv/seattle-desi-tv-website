...
export default function SponsorsPage() {
  ...
  const [form, setForm] = useState({ name: "", website: "", logo_url: "", display_order: 0, active: true });
  const canAccess = Boolean(user && isAdminRole(role));

  async function loadSponsors() {
    const { data, error } = await supabase.from("homepage_sponsors").select("id,name,website,logo_url,display_order,active").order("display_order", { ascending: true });
  ...
  async function saveSponsor() {
    ...
    const payload: any = { name: form.name, website: form.website || null, logo_url: form.logo_url || null, display_order: Number(form.display_order || 0), active: Boolean(form.active), updated_at: new Date().toISOString() };
    const { error } = await supabase.from("homepage_sponsors").insert(payload);
    if (error) { setMessage(`Save failed: ${error.message}`); return; }
    setForm({ name: "", website: "", logo_url: "", display_order: 0, active: true });
  ...
  return (
    <main className="min-h-screen bg-slate-950 text-white">
      ...
      <div className="max-w-7xl mx-auto px-6 py-10">
        ...
        <p className="text-slate-300 mt-2">Manage sponsors shown on the public homepage.</p>
        ...
        {!loading && canAccess && <div className="grid lg:grid-cols-[380px_1fr] gap-6 mt-8">
          ...
          <section className="bg-white text-slate-950 rounded-2xl p-6">
            ...
            <div className="grid gap-4">
              {sponsors.map((sponsor) => <article key={sponsor.id} className="border rounded-xl p-4 flex items-center justify-between gap-4"><div><h3 className="text-xl font-black">{sponsor.name}</h3><p className="text-sm text-gray-600">Order {sponsor.display_order || 0} · {sponsor.active ? "Active" : "Inactive"}</p></div><button onClick={() => toggleSponsor(sponsor)} className="border px-3 py-2 rounded-lg font-bold text-sm">{sponsor.active ? "Deactivate" : "Activate"}</button></article>)}
