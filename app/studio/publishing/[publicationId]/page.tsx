...
export default function UnifiedPublicationEditorPage() {
  ...
  const activeStage = stageForMode(mode);
  const needsSectionRail = mode === "section" || mode === "items" || mode === "ai";
  ...
  return <main className="min-h-screen bg-slate-100 text-slate-950"><StudioHeader /><div className="mx-auto max-w-[1600px] px-4 py-6 md:px-6">
    ...
    {pageError ? <div className="mt-6 rounded-3xl bg-white p-8 font-bold text-red-700">{pageError}</div> : !publication ? <div className="mt-6 rounded-3xl bg-white p-8 font-bold">Opening editor…</div> : <>
      <header className="mt-4"><div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between"><div><p className="text-xs font-black uppercase tracking-[0.2em] text-pink-600">Publication workspace</p><h1 className="mt-1 text-3xl font-black md:text-4xl">{publication.name}</h1><p className="mt-1 text-sm text-slate-500">{publication.edition_label || "Edition not set"}</p></div><span className={`w-fit rounded-full px-4 py-2 text-xs font-black uppercase ${publication.status === "approved" || publication.status === "published" ? "bg-emerald-100 text-emerald-800" : publication.status === "review" ? "bg-blue-100 text-blue-800" : "bg-amber-100 text-amber-900"}`}>{publication.status}</span></div>
        <nav aria-label="Publication workflow" className="mt-5 overflow-x-auto rounded-2xl border border-slate-200 bg-white p-2 shadow-sm"><div className="flex min-w-max gap-2">{stages.map((stage) => { const active = activeStage.key === stage.key; return <button key={stage.key} type="button" onClick={() => setMode(stage.modes[0].key)} className={`rounded-xl px-5 py-3 text-sm font-black transition ${active ? "bg-slate-950 text-white" : "text-slate-500 hover:bg-slate-100 hover:text-slate-950"}`}>{stage.label}</button>; })}</div></nav>
        <div className="mt-3 flex flex-col gap-3 rounded-2xl bg-slate-200/70 p-3 sm:flex-row sm:items-center sm:justify-between"><div><p className="text-xs font-black uppercase tracking-wide text-slate-500">{activeStage.label}</p><p className="mt-1 text-sm text-slate-700">{activeStage.description}</p></div><div className="flex flex-wrap gap-2">{activeStage.modes.map((item) => <button key={item.key} type="button" onClick={() => setMode(item.key)} className={`rounded-lg px-4 py-2 text-sm font-black ${mode === item.key ? "bg-pink-600 text-white" : "bg-white text-slate-700"}`}>{item.label}</button>)}</div></div>
      </header>
      ...
      <div className={`mt-6 grid gap-6 ${needsSectionRail ? "lg:grid-cols-[300px_minmax(0,1fr)]" : "grid-cols-1"}`}>
        {needsSectionRail && <aside className="rounded-3xl border border-slate-200 bg-white p-3 shadow-sm lg:sticky lg:top-4 lg:max-h-[calc(100vh-2rem)] lg:self-start lg:overflow-y-auto">
        ...
        <section className="min-w-0">
          ...
          {mode === "preview" && <PublicationPreviewWorkspace supabase={supabase} publicationId={publication.id} />}
        </section>
      </div>
    </>}
  </div></main>;
