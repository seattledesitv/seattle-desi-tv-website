...
type EditorMode = "section" | "items" | "content" | "preview";
    ...
    {pageError ? <div className="mt-6 rounded-3xl bg-white p-8 font-bold text-red-700">{pageError}</div> : !publication ? <div className="mt-6 rounded-3xl bg-white p-8 font-bold">Opening editor…</div> : <>
      ...
      <div className="mt-6 grid gap-6 lg:grid-cols-[300px_minmax(0,1fr)]">
        ...
        <section className="min-w-0">
          ...
          {selected && mode === "items" && <ItemsWorkspace key={selected.id} supabase={supabase} publicationSectionId={selected.id} />}
          {selected && mode === "preview" && <div><div className="mb-4"><p className="text-xs font-black uppercase tracking-wide text-slate-400">Live publication preview</p><p className="text-sm text-slate-600">Channel-specific and PDF previews arrive in Sprint 6.</p></div><SectionPreview section={selected} /></div>}
        </section>
      </div>
    </>}
  </div></main>;
