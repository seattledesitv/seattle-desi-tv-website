...
export default function CoverageRequestsPage() {
  ...
  async function init() {
    ...
    if (!currentUser) {
      setMessage("Please login to access coverage requests.");
      setLoading(false);
      return;
    }
    ...
    if (!isAdminRole(nextRole)) {
      setMessage("This account does not have coverage request access.");
      setLoading(false);
      return;
    }
      ...
      <div className="max-w-6xl mx-auto">
        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 mb-8">
          <div>
            ...
            <h1 className="text-4xl md:text-5xl font-black mt-3">Coverage Requests</h1>
            <p className="text-slate-300 mt-2">Organizer requests for SDTV event coverage.</p>
          </div>
        ...
        {!loading && canAccess && (
          <div className="space-y-5">
            ...
            <div className="bg-white/10 rounded-2xl p-5"><p className="text-slate-300">Coverage Requests</p><p className="text-4xl font-black">{requests.length}</p></div>
            <div className="grid gap-4">
              {requests.map((request) => {
                const ev = eventFor(request);
                const title = request.event_title || ev?.title || "Unknown event";
                const assignedCrewCount = Array.isArray(ev?.crew_member_ids) ? ev.crew_member_ids.length : 0;
                return (
                  <article key={request.id} className="bg-white text-slate-950 rounded-2xl p-4 grid md:grid-cols-[112px_1fr_auto] gap-4 items-center">
                    {getImage(ev) ? <img src={getImage(ev)} alt={title} className="w-28 h-28 rounded-xl object-cover" /> : <div className="w-28 h-28 bg-pink-50 rounded-xl grid place-items-center text-pink-600 font-black text-xs">No image</div>}
                    <div>
                      <h2 className="text-xl font-black">{title}</h2>
                      {ev && <p className="text-sm text-gray-600">{dateText(ev.date)} · {ev.location}</p>}
                      <p className="text-sm text-gray-700 mt-2">Organizer: {request.user_email || request.user_id}</p>
                      <p className="text-xs text-gray-500">Status: {request.status || "pending"} · Assigned crew: {assignedCrewCount}</p>
                    </div>
                    <div className="flex flex-wrap gap-2 md:justify-end">
                      <a href={`/studio/events/${request.event_id}`} className="bg-slate-900 text-white px-3 py-2 rounded-lg font-bold text-sm">Open Event</a>
                      <a href="/studio/crew/pending" className="border border-slate-900 text-slate-900 px-3 py-2 rounded-lg font-bold text-sm">Crew Inbox</a>
                      <button onClick={() => updateRequestStatus(request, "approved")} className="bg-green-600 text-white px-3 py-2 rounded-lg font-bold text-sm">Approve</button>
                      <button onClick={() => updateRequestStatus(request, "on_hold")} className="bg-yellow-500 text-white px-3 py-2 rounded-lg font-bold text-sm">On Hold</button>
                      <button onClick={() => updateRequestStatus(request, "rejected")} className="bg-red-600 text-white px-3 py-2 rounded-lg font-bold text-sm">Decline</button>
                    </div>
                  </article>
                );
              })}
              {requests.length === 0 && <div className="bg-white text-slate-950 rounded-2xl p-8">No coverage requests found.</div>}
            </div>
          </div>
        )}
      </div>
