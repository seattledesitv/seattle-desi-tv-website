...
export default function StudioAnalyticsPage() {
  ...
  async function loadAnalytics(currentUser: any) {
    ...
    const [
      ...
      forSite(
        supabase.from("events").select("id,title,date,location,status"),
        site.id,
      )
        .eq("status", "approved")
        .gte("date", today)
    ...
    if (upcomingResult.error)
      setMessage(
        `Analytics loaded, but upcoming events failed: ${upcomingResult.error.message}`,
      );
    else setMessage("");
  }
  ...
  async function init() {
    ...
    setUser(currentUser);
    if (!currentUser) {
      setMessage("Please login to view analytics.");
      setLoading(false);
      return;
    }
    ...
    if (!isAdminRole(nextRole)) {
      setMessage(
        `Studio analytics requires admin access. Current role: ${nextRole}`,
      );
      setLoading(false);
      return;
    }
        ...
        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 mb-8">
          <div>
            ...
            <p className="text-slate-300 mt-2">
              Operational dashboard for Seattle Desi TV.
            </p>
            {user?.email && (
          ...
          <div className="space-y-8">
            ...
            <section>
              ...
              <div className="grid md:grid-cols-3 gap-5">
                ...
                <MetricCard
                  ...
                  value={counts.approvedBusinesses}
                />
                <MetricCard
            ...
            <section>
              ...
              <div className="grid md:grid-cols-2 xl:grid-cols-3 gap-5">
                <MetricCard
                  ...
                  value={counts.coverageRequests}
                />
                <MetricCard
                  title="Pending Coverage"
                  value={counts.pendingCoverage}
                />
                <MetricCard
                  title="Approved Coverage"
                  value={counts.approvedCoverage}
                />
                ...
                <MetricCard
                  ...
                  value={counts.completedCoverage}
                />
              </div>
            </section>
            ...
            <section>
              ...
              <div className="grid md:grid-cols-2 xl:grid-cols-4 gap-5">
                ...
                <MetricCard
                  ...
                  value={counts.notificationsUnread}
                />
                <MetricCard title="Role Requests" value={counts.roleRequests} />
                <MetricCard
                  title="Pending Roles"
                  value={counts.pendingRoleRequests}
                />
                <MetricCard
                  title="Approved Roles"
                  value={counts.approvedRoleRequests}
                />
              </div>
            </section>
            ...
            <section className="bg-white text-slate-950 rounded-2xl p-6 shadow-xl">
              <div className="flex items-center justify-between gap-4 mb-4">
                ...
                <a href="/studio/events" className="text-pink-600 font-bold">
                  Open Events
                </a>
              </div>
              <div className="grid gap-3">
                {upcomingEvents.map((event) => (
                  <a
                    ...
                    className="border rounded-xl p-4 hover:bg-pink-50"
                  >
                ...
                {upcomingEvents.length === 0 && (
                  <p className="text-gray-500">
                    No approved upcoming events in the next 30 days.
                  </p>
                )}
              </div>
            </section>
            ...
            <section className="bg-white/10 border border-white/10 rounded-2xl p-6">
              ...
              <div className="grid md:grid-cols-5 gap-4 mt-5">
                <a
                  href="/studio/events/pending"
                  className="bg-slate-900 rounded-xl p-4 font-bold"
                >
                  Events: {counts.pendingEvents}
                </a>
                <a
                  href="/studio/businesses"
                  className="bg-slate-900 rounded-xl p-4 font-bold"
                >
                  Businesses: {counts.pendingBusinesses}
                </a>
                <a
                  href="/studio/coverage"
                  className="bg-slate-900 rounded-xl p-4 font-bold"
                >
                  Coverage: {counts.pendingCoverage}
                </a>
                <a
                  href="/studio/crew/pending"
                  className="bg-slate-900 rounded-xl p-4 font-bold"
                >
                  Crew: {counts.crewRequests}
                </a>
                <a
                  href="/studio/roles"
                  className="bg-slate-900 rounded-xl p-4 font-bold"
                >
                  Roles: {counts.pendingRoleRequests}
                </a>
              </div>
            </section>
          </div>
