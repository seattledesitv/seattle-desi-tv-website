...
export default function StudioHomepagePage() {
  ...
  async function seedDefaults() {
    const sectionPayload: any[] = defaultSections;
    await supabase.from("homepage_settings").upsert(sectionPayload, { onConflict: "section_key" });
    const socialPayload: any[] = defaultSocial;
    await supabase.from("social_media_stats").upsert(socialPayload, { onConflict: "platform" });
  }
  ...
  async function saveSections() {
    ...
    const payload: any[] = sections.map((section, index) => ({
  ...
  async function saveSocialStats() {
    ...
    const payload: any[] = socialRows.map((row) => ({
  ...
  function moveSection(index: number, direction: -1 | 1) {
    ...
    const next = [...sections];
    const currentOrder = next[index].display_order;
    next[index].display_order = next[target].display_order;
    next[target].display_order = currentOrder;
    [next[index], next[target]] = [next[target], next[index]];
    setSections(next.map((item, itemIndex) => ({ ...item, display_order: itemIndex + 1 })));
  }

  function updateSocial(index: number, field: string, value: any) {
    setSocialRows((current) => current.map((item, i) => i === index ? { ...item, [field]: value } : item));
  }
    ...
    <main className="min-h-screen bg-slate-950 text-white">
      ...
      <div className="max-w-7xl mx-auto px-6 py-10">
        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 mb-8">
          <div>
            <h1 className="text-4xl md:text-5xl font-black">Homepage CMS</h1>
            <p className="text-slate-300 mt-2">Reorder sections, enable or disable homepage content, and update social stats.</p>
          ...
          <div className="flex flex-wrap gap-3">
            ...
            <a href="/" target="_blank" rel="noreferrer" className="bg-pink-600 text-white px-5 py-3 rounded-xl font-bold">Preview Homepage</a>
          </div>
        </div>
        ...
        {!loading && canAccess && <div className="space-y-8">
          ...
          <section className="bg-white text-slate-950 rounded-2xl p-6 shadow-xl">
            <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 mb-5">
              <div>
                ...
                <p className="text-gray-600 text-sm mt-1">Move sections up/down, hide sections, and customize headings.</p>
              </div>
              <div className="flex gap-3">
                <button onClick={initializeDefaults} className="border px-4 py-2 rounded-xl font-bold">Seed Defaults</button>
                <button onClick={saveSections} className="bg-pink-600 text-white px-4 py-2 rounded-xl font-bold">Save Sections</button>
              </div>
            </div>
            <div className="grid gap-4">
              {sections.map((section, index) => (
                <article key={section.section_key} className="border rounded-2xl p-4 grid lg:grid-cols-[160px_120px_1fr_1fr_auto] gap-3 items-center">
                  <div>
                    <p className="font-black">{section.section_key}</p>
                    <p className="text-xs text-gray-500">Order {section.display_order}</p>
                  </div>
                  <label className="flex items-center gap-2 font-bold text-sm">
                    <input type="checkbox" checked={section.enabled !== false} onChange={(event) => updateSection(index, "enabled", event.target.checked)} /> Enabled
                  </label>
                  <input className="border rounded-lg p-3" placeholder="Title" value={section.title || ""} onChange={(event) => updateSection(index, "title", event.target.value)} />
                  <input className="border rounded-lg p-3" placeholder="Subtitle" value={section.subtitle || ""} onChange={(event) => updateSection(index, "subtitle", event.target.value)} />
                  <div className="flex gap-2 justify-end">
                    <button onClick={() => moveSection(index, -1)} className="border px-3 py-2 rounded-lg font-bold">Up</button>
                    <button onClick={() => moveSection(index, 1)} className="border px-3 py-2 rounded-lg font-bold">Down</button>
                  </div>
                </article>
              ))}
            </div>
          ...
          <section className="bg-white text-slate-950 rounded-2xl p-6 shadow-xl">
            <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 mb-5">
              <div>
                <h2 className="text-2xl font-black">Social Media Stats</h2>
                <p className="text-gray-600 text-sm mt-1">Update followers, views, videos, and links shown on the homepage.</p>
              </div>
              <button onClick={saveSocialStats} className="bg-pink-600 text-white px-4 py-2 rounded-xl font-bold">Save Social Stats</button>
            </div>
            <div className="grid gap-4">
              {socialRows.map((row, index) => (
                <article key={row.platform} className="border rounded-2xl p-4 grid lg:grid-cols-[160px_1fr_1fr_1fr_2fr] gap-3 items-center">
                  <input className="border rounded-lg p-3 font-bold" value={row.platform || ""} onChange={(event) => updateSocial(index, "platform", event.target.value)} />
                  <input className="border rounded-lg p-3" type="number" placeholder="Followers" value={row.followers || 0} onChange={(event) => updateSocial(index, "followers", event.target.value)} />
                  <input className="border rounded-lg p-3" type="number" placeholder="Views" value={row.views || 0} onChange={(event) => updateSocial(index, "views", event.target.value)} />
                  <input className="border rounded-lg p-3" type="number" placeholder="Videos" value={row.videos || 0} onChange={(event) => updateSocial(index, "videos", event.target.value)} />
                  <input className="border rounded-lg p-3" placeholder="URL" value={row.href || ""} onChange={(event) => updateSocial(index, "href", event.target.value)} />
                </article>
              ))}
            </div>
          </section>
