...
function wrapText(ctx: CanvasRenderingContext2D, text: string, x: number, y: number, maxWidth: number, lineHeight: number, maxLines: number, align: CanvasTextAlign = "left") {
  ctx.textAlign = align;
...
export default function StudioRecognitionPage() {
  ...
  const [dateBoxTitle, setDateBoxTitle] = useState("LAST WEEK");
  const [dateBoxSubtitle, setDateBoxSubtitle] = useState("COVERAGE");
  const [thankMessage, setThankMessage] = useState("Huge thank you to our amazing SDTV team members for covering community stories last week. Your time, energy, and passion help us celebrate the people and events that bring our community together.");
  const [impactText, setImpactText] = useState("YOU MADE A DIFFERENCE!");
  const [emptyStateText, setEmptyStateText] = useState("No completed coverage found for last week yet.");
  ...
  const caption = useMemo(() => {
    ...
    return ["✨ SDTV Team Recognition", "", thankMessage, extraNote, "", names ? `Recognizing: ${names}` : "Recognizing our amazing SDTV team members.", events.length ? `Covered last week: ${events.join(" • ")}` : "Thank you for supporting SDTV coverage last week.", handleList.length ? `\n${handleList.join(" ")}` : "", "", "#SeattleDesiTV #SDTVTeam #SeattleEvents #DesiCommunity #CommunityStories #PNWDesi"].filter(Boolean).join("\n");
  }, [selected, thankMessage, extraNote, handleList]);

  function markDirty() { setImageDataUrl(""); }
  async function loadData() {
    ...
    if (assignments.error) { setMessage(`Could not load last week coverage: ${assignments.error.message}`); setMembers([]); setLoading(false); return; }
  ...
  function toggleMember(key: string) { setMembers((items) => items.map((v) => v.key === key ? { ...v, selected: !v.selected } : v)); setImageDataUrl(""); }
  ...
  async function generateImage() {
    ...
    try {
      ...
      const logo = await loadImage("/sdtv-logo.png"); drawImageContain(ctx, logo, 62, 54, 120, 90);
      ...
      const gold = ctx.createLinearGradient(260, 280, 830, 380); gold.addColorStop(0, "#f59e0b"); gold.addColorStop(0.5, "#fde68a"); gold.addColorStop(1, "#f59e0b"); ctx.fillStyle = gold; ctx.font = "900 96px Arial"; ctx.fillText(headlineText, 540, 350);
      ctx.fillStyle = "#fbbf24"; roundRect(ctx, 820, 0, 220, 205, 0); ctx.fill();
      ctx.fillStyle = "#111827"; ctx.textAlign = "center"; ctx.font = "900 25px Arial"; ctx.fillText(dateBoxTitle, 930, 52); ctx.fillText(dateBoxSubtitle, 930, 84); ctx.font = "800 18px Arial"; ctx.fillText(`${shortFmt(start)} - ${shortFmt(end)}`, 930, 120); ctx.fillText(String(end.getFullYear()), 930, 148);
      ...
      if (!people.length) { ctx.fillStyle = "#ffffff"; ctx.font = "800 34px Arial"; wrapText(ctx, emptyStateText, 540, 735, 820, 42, 2, "center"); }
