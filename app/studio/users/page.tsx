...
function imageUse(kind: string) { const m: Record<string, string> = { base: "Default SDTV account/profile photo. This is the only value shown as Base Profile Photo and is what users see in My Profile.", id: "Official SDTV-created ID badge. Public Team page uses this first when available.", team: "Team image. This is a connected-profile image, not the base profile photo.", radio: "Radio host/team image. This is a connected-profile image, not the base profile photo.", volunteer: "Volunteer onboarding/recognition image. This is a connected-profile image, not the base profile photo.", influencer: "Influencer directory image. This is a connected-profile image, not the base profile photo." }; return m[kind] || "Connected SDTV profile image."; }
...
export default function StudioUsersPage() {
  ...
  async function loadRows(keepSelected = true) {
    ...
    if (keepSelected && selectedKeyRef.current) {
      ...
      if (updated) { setEditForm(emptyEditForm(updated)); await loadConnectedFor(updated); }
      else clearSelected();
    }
  }

  async function init() { setLoading(true); const { data } = await supabase.auth.getUser(); const currentUser = data?.user || null; setUser(currentUser); const nextRole = currentUser ? await resolveUserRole(supabase, currentUser) : ""; setRole(nextRole); if (!currentUser || !isAdminRole(nextRole)) { setMessage("Admin access required."); setLoading(false); return; } await loadRows(false); setMessage(""); setLoading(false); }
  ...
  async function loadConnectedFor(row: PersonRow) {
    ...
    const defaultOr = buildOr(row); const contentOr = buildOr(row, "submitter_user_id", "submitter_email");
  ...
  async function uploadAsset(file: File | undefined, field: "profile_photo_url" | "id_badge_url" | string, target: "base" | "connected" = "base") {
    setActionMessage(""); if (!file) return; if (!file.type.startsWith("image/")) { setActionMessage("Please upload an image file."); return; } if (!cloudName || !uploadPreset) { setActionMessage("Cloudinary is not configured. Paste the image URL instead."); return; }
    setUploading(true);
