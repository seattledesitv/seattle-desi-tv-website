...
function imageUse(kind: string) {
  const map: Record<string, string> = {
    base: "Default SDTV account/profile photo from user_profiles.profile_photo_url. Users see this in My Profile.",
    id: "Official SDTV-created ID badge. Public Team page uses this first when available.",
    team: "Team image. By default this should match the base photo unless a different role-specific image is needed.",
    radio: "Radio host/team image. By default this should match the base photo unless a different radio image is needed.",
    volunteer: "Volunteer onboarding/recognition image. By default this should match the base photo unless a different volunteer image is needed.",
    influencer: "Influencer directory image. By default this should match the base photo unless a different influencer image is needed.",
  };
  return map[kind] || "Connected SDTV profile image.";
}
function imageValue(kind: string, row: any) { return row?.[imageField(kind)] || ""; }
...
function ImageBox({ title, src, note }: { title: string; src?: string | null; note: string }) {
  return <div className="rounded-3xl border bg-slate-50 p-4"><p className="text-lg font-black">{title}</p><p className="mt-1 text-sm text-slate-600">{note}</p>{src ? <img src={src} alt={title} className="mt-4 h-56 w-full rounded-2xl border bg-white object-contain" /> : <div className="mt-4 grid h-56 place-items-center rounded-2xl bg-white text-slate-400">No image yet</div>}</div>;
}
function StatCard({ value, label, tone = "white" }: { value: number; label: string; tone?: "white" | "green" | "red" | "pink" }) {
  const toneClass = tone === "green" ? "bg-green-50 text-green-700" : tone === "red" ? "bg-red-50 text-red-700" : tone === "pink" ? "bg-pink-50 text-pink-700" : "bg-white text-slate-950";
  return <div className={`rounded-3xl p-5 ${toneClass}`}><p className="text-3xl font-black">{value}</p><p className="text-xs font-bold opacity-80">{label}</p></div>;
}
...
export default function StudioUsersPage() {
  ...
  function mergePerson(map: Map<string, PersonRow>, incoming: any, source: string, sourceRole?: string) {
    ...
    if (source === "Profile") existing.profile_photo_url = incoming.profile_photo_url || null;
    existing.id_badge_url = existing.id_badge_url || incoming.id_badge_url || null;
    existing.image = existing.image || incoming.image || incoming.photo_url || incoming.photo || incoming.picture || null;
  ...
  async function loadRows(keepSelected = true) {
    setActionMessage("");
    ...
    visibility.forEach((vis: any) => { const key = cleanEmail(vis.email) || String(vis.user_id || ""); const existing: PersonRow = map.get(key) || { key, email: cleanEmail(vis.email), user_id: vis.user_id, roles: ["general_public"], sources: ["Visibility"] }; existing.public_visibility_disabled = Boolean(vis.public_visibility_disabled); existing.visibility_disabled_reason = vis.reason || existing.visibility_disabled_reason; addUnique(existing.sources, "Visibility"); map.set(key, existing); });
    const nextRows = Array.from(map.values()).sort((a, b) => String(a.full_name || a.email || "").localeCompare(String(b.full_name || b.email || "")));
    setRows(nextRows);
    if (keepSelected && selectedKeyRef.current) {
      const updated = nextRows.find((row) => row.key === selectedKeyRef.current);
      if (updated) { setEditForm(emptyEditForm(updated)); await loadConnectedFor(updated); } else clearSelected();
    }
  }
  ...
  function buildOr(row: PersonRow, userField = "user_id", emailField = "email") { const parts = []; if (row.user_id) parts.push(`${userField}.eq.${row.user_id}`); if (row.email) parts.push(`${emailField}.eq.${row.email}`); return parts.join(","); }
  async function one(kind: string, title: string, table: string, query: any, statusField = "status") { const result = await query; if (result.error || !result.data) return null; const row = Array.isArray(result.data) ? result.data[0] : result.data; return row ? { kind, title, table, row, status: row[statusField] || row.role || row.title || "active" } : null; }
  async function loadConnectedFor(row: PersonRow) {
    const token = ++loadTokenRef.current;
    setConnectedProfiles([]); setSelectedConnected(null); setConnectedEdit({});
    ...
    if (token !== loadTokenRef.current || selectedKeyRef.current !== row.key) return;
    setConnectedProfiles(items.filter(Boolean) as ConnectedProfile[]);
  }

  function clearSelected() { loadTokenRef.current += 1; selectedKeyRef.current = ""; setSelectedKey(""); setEditForm(emptyEditForm()); setConnectedProfiles([]); setSelectedConnected(null); setConnectedEdit({}); setActiveTab("profile"); }
  async function choose(row: PersonRow) { if (selectedKeyRef.current === row.key) { clearSelected(); return; } loadTokenRef.current += 1; selectedKeyRef.current = row.key; setSelectedKey(row.key); setEditForm(emptyEditForm(row)); setConnectedProfiles([]); setSelectedConnected(null); setConnectedEdit({}); setActiveTab("profile"); await loadConnectedFor(row); }
  ...
  async function uploadAsset(file: File | undefined, field: "profile_photo_url" | "id_badge_url" | string, target: "base" | "connected" = "base") {
    ...
    setUploading(true);
    const body = new FormData(); body.append("file", file); body.append("upload_preset", uploadPreset); body.append("folder", field === "id_badge_url" ? "sdtv/id-badges" : target === "connected" ? "sdtv/connected-profiles" : "sdtv/user-profiles");
    try { const response = await fetch(`https://api.cloudinary.com/v1_1/${cloudName}/image/upload`, { method: "POST", body }); const result = await response.json(); if (!response.ok || !result.secure_url) throw new Error(result.error?.message || "Upload failed."); target === "connected" ? updateConnected(field, result.secure_url) : updateEdit(field, result.secure_url); setActionMessage("Image uploaded. Click Save to keep it."); }
  ...
  async function saveProfile() {
    ...
    if (!editForm.user_id) { setActionMessage("This person is not linked to an auth user yet, so base profile/privacy fields cannot be saved to My Profile. Link the user_id first or update the connected role profile instead."); return; }
    setActionMessage("Saving user profile...");
    const payload = { user_id: editForm.user_id, email: editForm.email || null, role: editForm.role, full_name: editForm.full_name || null, preferred_name: editForm.preferred_name || null, phone: editForm.phone || null, city: editForm.city || null, state: editForm.state || null, country: editForm.country || null, short_bio: editForm.short_bio || null, profile_photo_url: editForm.profile_photo_url || null, id_badge_url: editForm.id_badge_url || null, interests: editForm.interests || [], show_name_publicly: Boolean(editForm.show_name_publicly), allow_social_credit: Boolean(editForm.allow_social_credit), allow_sdtv_contact: Boolean(editForm.allow_sdtv_contact), keep_profile_private: Boolean(editForm.keep_profile_private), updated_at: new Date().toISOString() };
    const { data, error } = await supabase.from("user_profiles").upsert(payload, { onConflict: "user_id" }).select("*").single();
    if (error) { setActionMessage(`Profile update failed: ${error.message}. Run supabase/public-user-profile.sql and try again.`); return; }
    const updatedKey = cleanEmail(data.email) || String(data.user_id || selectedKeyRef.current);
    selectedKeyRef.current = updatedKey; setSelectedKey(updatedKey);
    setEditForm(emptyEditForm({ ...data, key: updatedKey, roles: [data.role || "general_public"], sources: ["Profile"] }));
    setActionMessage("User profile saved. This should now match what the user sees in My Profile.");
    await loadRows(true);
  }
  ...
  async function saveConnectedProfile() {
    ...
    const keys = allowed[selectedConnected.kind]; if (!keys) { setActionMessage("This connected record is read-only here."); return; }
    const payload: any = { updated_at: new Date().toISOString() }; keys.forEach((key) => { if (connectedEdit[key] !== undefined) payload[key] = connectedEdit[key] || null; });
    ...
    const selected = rows.find((row) => row.key === selectedKeyRef.current); if (selected) await loadConnectedFor(selected);
  }

  async function useBaseForAll() {
    if (!editForm.profile_photo_url) { setActionMessage("Add a base profile photo first, then use this action."); return; }
    const targets = connectedProfiles.filter((item) => connectedKinds.includes(item.kind));
    if (targets.length === 0) { setActionMessage("No connected profiles found to update."); return; }
    const ok = window.confirm("Apply Base Profile Photo to all connected profiles?\n\nThis will immediately overwrite the image currently used by Team, Radio, Volunteer, and Influencer profiles that exist for this user.\n\nThe Base Profile Photo itself will not change. You can choose different images later by editing each connected profile individually.\n\nThis action takes effect immediately and is not tied to the Save User Profile button.");
    if (!ok) return;
    setActionMessage("Applying base profile photo to connected profiles...");
    ...
    setActionMessage(failed ? `Some connected profiles could not be updated: ${failed.error.message}` : `Base profile photo applied immediately to ${targets.length} connected profile(s).`);
    const selected = rows.find((row) => row.key === selectedKeyRef.current); if (selected) await loadConnectedFor(selected);
  }

  function useBaseHere() {
    if (!selectedConnected || !editForm.profile_photo_url) { setActionMessage("Add a base profile photo first, then use this action."); return; }
    const ok = window.confirm(`Use Base Photo for ${selectedConnected.title}?\n\nThis will replace the image field in this connected profile form with the Base Profile Photo.\n\nClick Save Connected Profile after this to keep the change. You can choose a different image later.`);
    if (!ok) return;
    updateConnected(imageField(selectedConnected.kind), editForm.profile_photo_url);
    setActionMessage("Base profile photo copied into this connected profile form. Click Save Connected Profile to keep it.");
  }

  async function setVisibilityDisabled(row: PersonRow, disabled: boolean) {
    const email = cleanEmail(row.email); if (!email && !row.user_id) { setActionMessage("This row does not have an email or user id, so global visibility cannot be changed yet."); return; }
    const reason = disabled ? window.prompt("Reason for disabling public visibility?", row.visibility_disabled_reason || "Admin disabled public visibility") : ""; if (disabled && reason === null) return;
    ...
    setActionMessage(error ? `Visibility update failed: ${error.message}` : disabled ? "Public visibility disabled for this user." : "Public visibility restored for this user."); await loadRows(true);
  }
