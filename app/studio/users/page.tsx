...
function visibilityText(row: PersonRow) { if (row.public_visibility_disabled) return "Hidden everywhere"; if (row.keep_profile_private) return "Private"; if (row.show_name_publicly || row.allow_social_credit) return "Allowed for credit"; return "Limited"; }
...
function emptyEditForm(row?: PersonRow | null) { return { user_id: row?.user_id || "", email: row?.email || "", full_name: row?.full_name || "", preferred_name: row?.preferred_name || "", phone: row?.phone || "", city: row?.city || "", state: row?.state || "", country: row?.country || "", short_bio: row?.short_bio || "", profile_photo_url: row?.profile_photo_url || row?.image || "", role: row?.role || "general_public", interests: [] as string[], show_name_publicly: Boolean(row?.show_name_publicly), allow_social_credit: row?.allow_social_credit !== false, allow_sdtv_contact: row?.allow_sdtv_contact !== false, keep_profile_private: row?.keep_profile_private !== false }; }
...
export default function StudioUsersPage() {
  ...
  const stats = useMemo(() => ({ total: rows.length, hidden: rows.filter((row) => row.public_visibility_disabled).length, visible: rows.filter((row) => !row.public_visibility_disabled).length, team: rows.filter((row) => row.roles.includes("team_member")).length, influencers: rows.filter((row) => row.roles.includes("influencer")).length, admins: rows.filter((row) => row.roles.some((item) => item.includes("admin"))).length, radio: rows.filter((row) => row.roles.includes("radio")).length, volunteers: rows.filter((row) => row.roles.includes("volunteer")).length }), [rows]);
  ...
  async function saveProfile() { if (!selected) return; if (!selected.user_id) { setActionMessage("Profile updates require a linked user_id. Visibility can still be controlled by email."); return; } const { error } = await supabase.from("user_profiles").upsert({ user_id: selected.user_id, email: editForm.email || selected.email, role: editForm.role, full_name: editForm.full_name || null, preferred_name: editForm.preferred_name || null, phone: editForm.phone || null, city: editForm.city || null, state: editForm.state || null, country: editForm.country || null, short_bio: editForm.short_bio || null, profile_photo_url: editForm.profile_photo_url || null, interests: editForm.interests || [], show_name_publicly: Boolean(editForm.show_name_publicly), allow_social_credit: Boolean(editForm.allow_social_credit), allow_sdtv_contact: Boolean(editForm.allow_sdtv_contact), keep_profile_private: Boolean(editForm.keep_profile_private), updated_at: new Date().toISOString() }, { onConflict: "user_id" }); setActionMessage(error ? `Profile update failed: ${error.message}` : "User profile updated."); await loadRows(); }
