...
function publicStatusText(row: PersonRow) { if (row.public_visibility_disabled) return "Hidden"; if (row.keep_profile_private) return "Private"; if (row.show_name_publicly || row.allow_social_credit) return "Credit OK"; return "Public limited"; }
function publicStatusExplain(row: PersonRow) { if (row.public_visibility_disabled) return "This person is hidden from public SDTV pages."; if (row.keep_profile_private) return "Profile is private by default, but internal SDTV operations still work."; if (row.show_name_publicly || row.allow_social_credit) return "SDTV may publicly credit this person when appropriate."; return "Not hidden, but no public credit/name permission has been granted yet."; }
...
export default function StudioUsersPage() {
  ...
  const [activeTab, setActiveTab] = useState<"profile" | "roles" | "privacy" | "sources" | "assets">("profile");
  ...
  async function loadRows() {
    ...
    if (selectedKey) {
      ...
      else { setSelectedKey(""); setEditForm(emptyEditForm()); }
    }
  }

  async function init() { setLoading(true); const { data } = await supabase.auth.getUser(); const currentUser = data?.user || null; setUser(currentUser); const nextRole = currentUser ? await resolveUserRole(supabase, currentUser) : ""; setRole(nextRole); if (!currentUser || !isAdminRole(nextRole)) { setMessage("Admin access required."); setLoading(false); return; } await loadRows(); setMessage(""); setLoading(false); }
  ...
  const stats = useMemo(() => ({ total: rows.length, hidden: rows.filter((row) => row.public_visibility_disabled).length, visible: rows.filter((row) => !row.public_visibility_disabled).length, generalPublic: rows.filter((row) => (row.roles.length ? row.roles : [row.role || "general_public"]).includes("general_public")).length, team: rows.filter((row) => row.roles.includes("team_member")).length, influencers: rows.filter((row) => row.roles.includes("influencer")).length, admins: rows.filter((row) => row.roles.some((item) => item.includes("admin"))).length, radio: rows.filter((row) => row.roles.includes("radio")).length, volunteers: rows.filter((row) => row.roles.includes("volunteer")).length }), [rows]);

  function choose(row: PersonRow) { if (selectedKey === row.key) { setSelectedKey(""); setEditForm(emptyEditForm()); setActiveTab("profile"); return; } setSelectedKey(row.key); setEditForm(emptyEditForm(row)); setActiveTab("profile"); }
  ...
  async function uploadAsset(file: File | undefined, field: "profile_photo_url" | "id_badge_url") {
    setActionMessage(""); if (!selected?.user_id) { setActionMessage("Select a linked user before uploading an asset."); return; } if (!file) return;
    ...
    setUploading(true);
    const body = new FormData(); body.append("file", file); body.append("upload_preset", uploadPreset); body.append("folder", field === "id_badge_url" ? "sdtv/id-badges" : "sdtv/user-profiles");
    try { const response = await fetch(`https://api.cloudinary.com/v1_1/${cloudName}/image/upload`, { method: "POST", body }); const result = await response.json(); if (!response.ok || !result.secure_url) throw new Error(result.error?.message || "Upload failed."); updateEdit(field, result.secure_url); setActionMessage(field === "id_badge_url" ? "ID badge uploaded. Click Save User Profile to keep it." : "Profile image uploaded. Click Save User Profile to keep it."); }
  ...
  async function saveProfile() {
    ...
    setActionMessage(error ? `Profile update failed: ${error.message}` : "User profile updated."); await loadRows();
  }

  async function setVisibilityDisabled(row: PersonRow, disabled: boolean) { const email = cleanEmail(row.email); if (!email && !row.user_id) { setActionMessage("This row does not have an email or user id, so global visibility cannot be changed yet."); return; } const reason = disabled ? window.prompt("Reason for disabling public visibility?", row.visibility_disabled_reason || "Admin disabled public visibility") : ""; if (disabled && reason === null) return; const payload = { email: email || `${row.user_id}@unknown.local`, user_id: row.user_id || null, public_visibility_disabled: disabled, reason: disabled ? reason : null, disabled_at: disabled ? new Date().toISOString() : null, disabled_by: disabled ? user?.email || null : null, updated_at: new Date().toISOString() }; const { error } = await supabase.from("public_visibility_controls").upsert(payload, { onConflict: "email" }); if (!error && row.user_id) await supabase.from("user_profiles").update({ public_visibility_disabled: disabled, visibility_disabled_reason: disabled ? reason : null, visibility_disabled_at: disabled ? new Date().toISOString() : null, visibility_disabled_by: disabled ? user?.email || null : null, keep_profile_private: disabled ? true : row.keep_profile_private, show_name_publicly: disabled ? false : row.show_name_publicly, allow_social_credit: disabled ? false : row.allow_social_credit, updated_at: new Date().toISOString() }).eq("user_id", row.user_id); setActionMessage(error ? `Visibility update failed: ${error.message}` : disabled ? "Public visibility disabled for this user." : "Public visibility restored for this user."); await loadRows(); }

  useEffect(() => { init(); }, []);

  return <main className="min-h-screen bg-slate-950 text-white"><StudioHeader /><section className="mx-auto max-w-7xl px-6 py-10"><div className="mb-8 flex flex-col gap-4 md:flex-row md:items-end md:justify-between"><div><p className="font-black uppercase tracking-wide text-pink-300">People</p><h1 className="mt-2 text-4xl font-black md:text-5xl">User Control</h1><p className="mt-2 max-w-3xl text-slate-300">Unified people directory with admin profile editing, ID badge assets, roles, source history, and global public visibility controls.</p></div><button onClick={init} className="rounded-xl bg-white px-5 py-3 font-black text-slate-950">Refresh</button></div>{loading && <div className="rounded-2xl bg-white/10 p-6">{message}</div>}{!loading && !canAccess && <div className="rounded-2xl bg-white p-8 text-slate-950">{message}</div>}{!loading && canAccess && <div className="space-y-5">{actionMessage && <div className="rounded-2xl bg-yellow-100 p-4 font-bold text-yellow-900">{actionMessage}</div>}<div className="grid gap-5 xl:grid-cols-[420px_1fr]"><section className="rounded-[2rem] bg-white p-4 text-slate-950 shadow-xl"><div className="p-2"><h2 className="text-2xl font-black">People</h2><p className="text-sm text-slate-500">Click a user to manage. Click again to return to analytics.</p></div><div className="mt-3 grid gap-2"><input value={searchText} onChange={(event) => setSearchText(event.target.value)} placeholder="Search name, email, phone, city, role..." className="rounded-xl border p-3 font-bold" /><div className="grid grid-cols-2 gap-2"><select value={roleFilter} onChange={(event) => setRoleFilter(event.target.value)} className="rounded-xl border p-3 font-bold"><option value="all">All roles</option>{roles.map((item) => <option key={item} value={item}>{label(item)}</option>)}</select><select value={visibilityFilter} onChange={(event) => setVisibilityFilter(event.target.value)} className="rounded-xl border p-3 font-bold"><option value="all">All visibility</option><option value="hidden">Hidden</option><option value="public_ok">Not hidden</option></select></div></div><div className="mt-4 max-h-[720px] space-y-3 overflow-y-auto pr-1">{filteredRows.map((row) => <button key={row.key} onClick={() => choose(row)} className={`w-full rounded-3xl border p-4 text-left transition hover:-translate-y-0.5 ${selected?.key === row.key ? "border-pink-500 bg-pink-50 ring-2 ring-pink-100" : row.public_visibility_disabled ? "border-red-200 bg-red-50" : "border-slate-100 bg-white"}`}><div className="flex gap-3"><div className="grid h-14 w-14 shrink-0 place-items-center overflow-hidden rounded-full bg-slate-100 text-xl font-black text-pink-600">{row.profile_photo_url || row.image ? <img src={row.profile_photo_url || row.image || ""} alt="Profile" className="h-full w-full object-cover" /> : avatarText(row)}</div><div className="min-w-0 flex-1"><div className="flex items-start justify-between gap-2"><h3 className="truncate text-lg font-black">{row.full_name || row.preferred_name || "Unnamed User"}</h3><span title={publicStatusExplain(row)} className={`shrink-0 rounded-full px-2 py-1 text-[10px] font-black ${statusClass(row)}`}>{publicStatusText(row)}</span></div><p className="truncate text-xs font-bold text-slate-500">{row.email || "No email linked"}</p><div className="mt-2 flex flex-wrap gap-1">{(row.roles.length ? row.roles : [row.role || "general_public"]).slice(0, 3).map((item) => <span key={item} className="rounded-full bg-slate-100 px-2 py-1 text-[10px] font-black text-slate-600">{label(item)}</span>)}</div></div></div></button>)}{filteredRows.length === 0 && <div className="rounded-3xl bg-slate-50 p-6 text-slate-500">No users found.</div>}</div></section><section className="space-y-5">{!selected && <><div className="rounded-[2rem] bg-white p-6 text-slate-950 shadow-xl"><p className="text-xs font-black uppercase tracking-wide text-pink-600">All-up analytics</p><h2 className="mt-1 text-3xl font-black">People Overview</h2><p className="mt-2 text-slate-600">Select a person on the left to edit profile, upload SDTV ID badge, manage roles, privacy, and sources.</p></div><div className="grid gap-4 md:grid-cols-4"><StatCard value={stats.total} label="Total people" /><StatCard value={stats.generalPublic} label="General public" tone="pink" /><StatCard value={stats.visible} label="Public not hidden" tone="green" /><StatCard value={stats.hidden} label="Hidden" tone="red" /></div><div className="grid gap-4 md:grid-cols-4"><StatCard value={stats.team} label="Team" /><StatCard value={stats.volunteers} label="Volunteers" /><StatCard value={stats.influencers} label="Influencers" /><StatCard value={stats.radio} label="Radio" /></div></>}{selected && <><div className="rounded-[2rem] bg-white p-6 text-slate-950 shadow-xl"><div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between"><div className="flex gap-4"><div className="grid h-20 w-20 shrink-0 place-items-center overflow-hidden rounded-full bg-slate-100 text-3xl font-black text-pink-600">{selected.profile_photo_url || selected.image ? <img src={selected.profile_photo_url || selected.image || ""} alt="Profile" className="h-full w-full object-cover" /> : avatarText(selected)}</div><div><p className="text-xs font-black uppercase tracking-wide text-pink-600">Selected User</p><h2 className="text-3xl font-black">{selected.full_name || selected.preferred_name || "Unnamed User"}</h2><p className="text-sm font-bold text-slate-500">{selected.email || "No email linked"}</p><div className="mt-2 flex flex-wrap gap-2">{(selected.roles.length ? selected.roles : [selected.role || "general_public"]).map((item) => <span key={item} className="rounded-full bg-slate-100 px-3 py-1 text-xs font-black text-slate-700">{label(item)}</span>)}<span title={publicStatusExplain(selected)} className={`rounded-full px-3 py-1 text-xs font-black ${statusClass(selected)}`}>{publicStatusText(selected)}</span>{selected.id_badge_url && <span className="rounded-full bg-green-100 px-3 py-1 text-xs font-black text-green-700">ID badge uploaded</span>}</div></div></div><div className="flex flex-wrap gap-2"><button onClick={() => choose(selected)} className="rounded-xl bg-slate-100 px-4 py-3 text-sm font-black text-slate-700">Back to Analytics</button>{selected.public_visibility_disabled ? <button onClick={() => setVisibilityDisabled(selected, false)} className="rounded-xl bg-green-700 px-4 py-3 text-sm font-black text-white">Restore Public Visibility</button> : <button onClick={() => setVisibilityDisabled(selected, true)} className="rounded-xl bg-red-600 px-4 py-3 text-sm font-black text-white">Disable Public Visibility</button>}</div></div></div><div className="rounded-[2rem] bg-white p-5 text-slate-950 shadow-xl"><div className="flex flex-wrap gap-2">{(["profile", "assets", "roles", "privacy", "sources"] as const).map((tab) => <button key={tab} onClick={() => setActiveTab(tab)} className={`rounded-xl px-4 py-2 text-sm font-black ${activeTab === tab ? "bg-pink-600 text-white" : "bg-slate-100 text-slate-700"}`}>{tab === "profile" ? "Profile" : tab === "assets" ? "Images / ID Badge" : tab === "roles" ? "Roles" : tab === "privacy" ? "Privacy" : "Sources"}</button>)}</div>{activeTab === "profile" && <div className="mt-5 grid gap-4 md:grid-cols-2"><label className="font-bold">Full name<input value={editForm.full_name} onChange={(e) => updateEdit("full_name", e.target.value)} className="mt-1 w-full rounded-xl border p-3 font-normal" /></label><label className="font-bold">Preferred name<input value={editForm.preferred_name} onChange={(e) => updateEdit("preferred_name", e.target.value)} className="mt-1 w-full rounded-xl border p-3 font-normal" /></label><label className="font-bold">Email<input value={editForm.email} disabled className="mt-1 w-full rounded-xl border bg-slate-100 p-3 font-normal" /></label><label className="font-bold">Phone<input value={editForm.phone} onChange={(e) => updateEdit("phone", e.target.value)} className="mt-1 w-full rounded-xl border p-3 font-normal" /></label><label className="font-bold">City<input value={editForm.city} onChange={(e) => updateEdit("city", e.target.value)} className="mt-1 w-full rounded-xl border p-3 font-normal" /></label><label className="font-bold">State<input value={editForm.state} onChange={(e) => updateEdit("state", e.target.value)} className="mt-1 w-full rounded-xl border p-3 font-normal" /></label><label className="font-bold md:col-span-2">Bio<textarea value={editForm.short_bio} onChange={(e) => updateEdit("short_bio", e.target.value)} className="mt-1 min-h-28 w-full rounded-xl border p-3 font-normal" /></label></div>}{activeTab === "assets" && <div className="mt-5 grid gap-5 md:grid-cols-2"><div className="rounded-3xl border bg-slate-50 p-4"><p className="text-lg font-black">Base Profile Photo</p><p className="mt-1 text-sm text-slate-600">Used as the default SDTV account/profile photo and fallback for internal experiences.</p>{editForm.profile_photo_url ? <img src={editForm.profile_photo_url} alt="Profile" className="mt-4 h-56 w-full rounded-2xl object-cover" /> : <div className="mt-4 grid h-56 place-items-center rounded-2xl bg-white text-slate-400">No profile image</div>}<label className="mt-4 block text-sm font-bold">Upload profile photo<input type="file" accept="image/*" onChange={(e) => uploadAsset(e.target.files?.[0], "profile_photo_url")} className="mt-2 w-full rounded-xl border bg-white p-2 text-xs" /></label><label className="mt-3 block text-sm font-bold">Profile Photo URL<input value={editForm.profile_photo_url} onChange={(e) => updateEdit("profile_photo_url", e.target.value)} className="mt-1 w-full rounded-xl border bg-white p-3 font-normal" /></label></div><div className="rounded-3xl border bg-slate-50 p-4"><p className="text-lg font-black">SDTV ID Badge</p><p className="mt-1 text-sm text-slate-600">Official SDTV-created ID badge. This appears to the user in My Profile and is managed by admins only.</p>{editForm.id_badge_url ? <img src={editForm.id_badge_url} alt="SDTV ID Badge" className="mt-4 max-h-72 w-full rounded-2xl border bg-white object-contain" /> : <div className="mt-4 grid h-56 place-items-center rounded-2xl bg-white text-slate-400">No ID badge uploaded</div>}<label className="mt-4 block text-sm font-bold">Upload ID badge image<input type="file" accept="image/*" onChange={(e) => uploadAsset(e.target.files?.[0], "id_badge_url")} className="mt-2 w-full rounded-xl border bg-white p-2 text-xs" /></label><label className="mt-3 block text-sm font-bold">ID Badge URL<input value={editForm.id_badge_url} onChange={(e) => updateEdit("id_badge_url", e.target.value)} className="mt-1 w-full rounded-xl border bg-white p-3 font-normal" /></label></div></div>}{activeTab === "roles" && <div className="mt-5 grid gap-4"><label className="font-bold">Base role<select value={editForm.role} onChange={(e) => updateEdit("role", e.target.value)} className="mt-1 w-full rounded-xl border p-3 font-normal">{roleOptions.map((item) => <option key={item} value={item}>{label(item)}</option>)}</select></label><div><p className="font-black">Detected roles</p><div className="mt-2 flex flex-wrap gap-2">{(selected.roles.length ? selected.roles : [selected.role || "general_public"]).map((item) => <span key={item} className="rounded-full bg-slate-100 px-3 py-2 text-sm font-black text-slate-700">{label(item)}</span>)}</div></div><div><p className="font-black">Interests</p><div className="mt-2 flex flex-wrap gap-2">{interestOptions.map((item) => <button key={item} type="button" onClick={() => toggleInterest(item)} className={`rounded-xl px-3 py-2 text-sm font-black ${editForm.interests.includes(item) ? "bg-pink-600 text-white" : "bg-slate-100 text-slate-700"}`}>{item}</button>)}</div></div></div>}{activeTab === "privacy" && <div className="mt-5 grid gap-3 rounded-2xl bg-slate-50 p-4"><label><input type="checkbox" checked={Boolean(editForm.show_name_publicly)} onChange={(e) => updateEdit("show_name_publicly", e.target.checked)} /> Show name publicly when content is published</label><label><input type="checkbox" checked={Boolean(editForm.allow_social_credit)} onChange={(e) => updateEdit("allow_social_credit", e.target.checked)} /> Allow SDTV social credit</label><label><input type="checkbox" checked={Boolean(editForm.allow_sdtv_contact)} onChange={(e) => updateEdit("allow_sdtv_contact", e.target.checked)} /> Allow SDTV contact</label><label><input type="checkbox" checked={Boolean(editForm.keep_profile_private)} onChange={(e) => updateEdit("keep_profile_private", e.target.checked)} /> Keep profile private by default</label>{selected.visibility_disabled_reason && <div className="rounded-2xl bg-red-100 p-3 font-bold text-red-800">Visibility disabled reason: {selected.visibility_disabled_reason}</div>}</div>}{activeTab === "sources" && <div className="mt-5 grid gap-4"><div><p className="font-black">Detected sources</p><div className="mt-2 flex flex-wrap gap-2">{selected.sources.map((source) => <span key={source} className="rounded-full bg-slate-100 px-3 py-2 text-sm font-black text-slate-700">{source}</span>)}</div></div><div className="grid gap-3 text-sm text-slate-700 md:grid-cols-3"><p><b>User ID:</b> {selected.user_id || "Not linked"}</p><p><b>Phone:</b> {selected.phone || "—"}</p><p><b>Updated:</b> {selected.updated_at ? new Date(selected.updated_at).toLocaleDateString() : "—"}</p></div>{selected.short_bio && <p className="rounded-2xl bg-slate-50 p-3 text-sm text-slate-700">{selected.short_bio}</p>}</div>}<button onClick={saveProfile} disabled={uploading} className="mt-5 rounded-xl bg-slate-950 px-5 py-3 font-black text-white disabled:opacity-60">{uploading ? "Uploading..." : "Save User Profile"}</button></div></>}</section></div></div>}</section></main>;
}
