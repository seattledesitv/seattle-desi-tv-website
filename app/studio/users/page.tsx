...
export default function StudioUsersPage() {
  ...
  const stats = useMemo(() => ({
    ...
    visible: rows.filter((row) => !row.public_visibility_disabled).length,
    generalPublic: rows.filter((row) => (row.roles.length ? row.roles : [row.role || "general_public"]).includes("general_public")).length,
    ...
    radio: rows.filter((row) => row.roles.includes("radio")).length,
    volunteers: rows.filter((row) => row.roles.includes("volunteer")).length,
  }), [rows]);

  function choose(row: PersonRow) {
    if (selectedKey === row.key) { setSelectedKey(""); setEditForm(emptyEditForm()); setActiveTab("profile"); return; }
    setSelectedKey(row.key); setEditForm(emptyEditForm(row)); setActiveTab("profile");
  }
