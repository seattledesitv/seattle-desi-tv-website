...
export default function StudioUsersPage() {
  ...
  async function choose(row: PersonRow) {
    if (selectedKeyRef.current === row.key) { clearSelected(); return; }
    loadTokenRef.current += 1;
  ...
  async function saveProfile() {
    ...
    selectedKeyRef.current = updatedKey;
  ...
  async function saveConnectedProfile() {
    ...
    const selected = rows.find((row) => row.key === selectedKeyRef.current);
