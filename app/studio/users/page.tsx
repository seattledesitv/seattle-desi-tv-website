...
export default function StudioUsersPage() {
  ...
  async function useBaseForAll() {
    ...
    setActionMessage(failed ? `Some connected profiles could not be updated: ${failed.error.message}` : "Base profile photo applied to all connected profile images.");
    const selected = rows.find((row) => row.key === selectedKeyRef.current); if (selected) await loadConnectedFor(selected);
  }
