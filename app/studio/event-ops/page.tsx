...
export default function EventOpsPage() {
  ...
  async function ensureVideoWorkflow() {
    ...
    const { error } = await supabase.from("event_video_workflows").insert({ event_id: selectedEventId, status: "ready_for_editing", assigned_editor_email: selectedEditorEmail || null, crew_reviewer_email: user?.email || null, crew_notes: `Priority: ${videoPriority}`, created_by: user?.id || null, updated_by: user?.id || null });
  ...
  async function updateWorkflowStatus(status: string) {
    ...
    const payload: any = { status, assigned_editor_email: selectedEditorEmail || eventWorkflow.assigned_editor_email || null, crew_notes: `Priority: ${videoPriority}${eventWorkflow.crew_notes ? `\n\n${eventWorkflow.crew_notes}` : ""}`, updated_by: user?.id || null, updated_at: new Date().toISOString() };
