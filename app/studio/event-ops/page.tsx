...
export default function EventOpsPage() {
  ...
  function statsForEvent(eventId: string) {
    const eventAssignments = assignments.filter((item) => item.event_id === eventId);
    const assigned = eventAssignments.filter((item) => String(item.status || "").toLowerCase() === "approved").length;
    const completed = eventAssignments.filter((item) => item.coverage_completed).length;
    const requested = eventAssignments.filter((item) => String(item.status || "").toLowerCase() === "pending").length;
    const workflow = workflows.find((item) => item.event_id === eventId);
    return { assigned, completed, requested, videoStatus: workflow?.status || "not_started" };
  }
  ...
  function EventCard({ event, compact = false }: { event: any; compact?: boolean }) {
    ...
    return <button type="button" onClick={() => setSelectedEventId(event.id)} className={`w-full text-left rounded-2xl p-4 transition border shadow-sm hover:shadow-md ${active ? "border-pink-500 bg-pink-50 ring-2 ring-pink-100" : "border-slate-200 bg-white hover:bg-slate-50"}`}><div className="flex items-start justify-between gap-3"><div className="min-w-0"><h3 className="font-black text-slate-950 truncate">{event.title}</h3><p className="text-sm text-gray-600 mt-1">{dateText(event.date)} · {event.location || "No location"}</p>{!compact && <p className="text-xs text-gray-500 mt-1 truncate">POC: {event.poc_email || "—"}</p>}</div><span className={`text-[11px] font-black px-2 py-1 rounded-full whitespace-nowrap ${statusClass(event.status)}`}>{label(event.status)}</span></div>{!compact && <div className="grid grid-cols-3 gap-2 mt-4"><div className="rounded-xl bg-slate-50 p-2 text-center"><p className="text-lg font-black">{stats.assigned}</p><p className="text-[10px] text-gray-500 font-bold">Crew</p></div><div className="rounded-xl bg-slate-50 p-2 text-center"><p className="text-lg font-black">{stats.completed}</p><p className="text-[10px] text-gray-500 font-bold">Done</p></div><div className="rounded-xl bg-slate-50 p-2 text-center"><p className="text-xs font-black truncate capitalize">{label(stats.videoStatus)}</p><p className="text-[10px] text-gray-500 font-bold">Video</p></div></div>}{stats.requested > 0 && <p className="text-xs font-bold text-pink-600 mt-2">{stats.requested} pending crew/coverage request(s)</p>}</button>;
  }
