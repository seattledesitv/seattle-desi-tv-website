...
export default function EventOpsPage() {
  ...
  async function loadTeamUsers() {
    ...
    const merged = adminRows.map((item: any) => {
      ...
      return { user_id: item.user_id || profile.user_id || email, email, role: item.role || "team_member", name: profile.full_name || item.name || email, photo_url: profile.photo_url || "" };
    }).filter((item: any) => {
  ...
  async function assignCrew() {
    ...
    const assignmentType = normalizedRoleKey(selectedCrewRole);
    const rows = selectedCrewUsers.map((member) => ({ event_id: selectedEventId, user_id: member.user_id?.includes("@") ? null : member.user_id, user_email: member.email, assignment_type: assignmentType, status: "approved", event_title: selectedEvent?.title || null, approved_by: user?.email || user?.id || null, approved_at: new Date().toISOString() }));
  ...
  async function createVideoWorkflow() {
    ...
    const priorityNote = `Priority: ${videoPriority}`;
    const notes = [priorityNote, crewNotes.trim()].filter(Boolean).join("\n\n");
    ...
    setSelectedEditorEmail(""); setMediaUrl(""); setCrewNotes(""); setVideoPriority("Normal");
  ...
  function EventButton({ event }: { event: any }) {
    const active = selectedEventId === event.id;
    return <button type="button" onClick={() => setSelectedEventId(event.id)} className={`text-left border rounded-2xl p-4 transition ${active ? "border-pink-600 bg-pink-50" : "bg-white hover:bg-slate-50"}`}><div className="flex items-start justify-between gap-3"><div><h3 className="font-black text-slate-950">{event.title}</h3><p className="text-sm text-gray-600">{dateText(event.date)} · {event.location || "No location"}</p><p className="text-xs text-gray-500 mt-1">POC: {event.poc_email || "—"}</p></div><span className="text-xs font-black bg-slate-100 px-2 py-1 rounded-full">{label(event.status)}</span></div></button>;
  }
  ...
  return (
    <main className="min-h-screen bg-slate-950 text-white">
      ...
      <section className="max-w-7xl mx-auto px-6 py-10">
        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 mb-8"><div><h1 className="text-4xl md:text-5xl font-black">Event Ops</h1><p className="text-slate-300 mt-2">One event control center: approval, coverage, crew, completion, and video production.</p>{user?.email && <p className="text-slate-400 text-sm mt-1">Logged in as {user.email} · Role: {role}</p>}</div><button onClick={init} className="bg-white text-slate-950 px-5 py-3 rounded-xl font-bold">Refresh</button></div>
