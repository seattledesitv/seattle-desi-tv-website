...
export default function HeroLayoutDesigner({ banners, featuredEvents, festivals }: { banners: any[]; featuredEvents: any[]; festivals: any[] }) {
  ...
  useEffect(() => { (async () => { const { data, error } = await supabase.from("homepage_hero_settings").select("layout_style").eq("id", "default").maybeSingle(); if (!error && data?.layout_style) { setLayout(data.layout_style as HeroLayoutStyle); setSavedLayout(data.layout_style as HeroLayoutStyle); } if (error) setMessage("Run supabase/homepage-hero-settings.sql to enable saving the global layout."); })(); }, []);
  async function save() { setSaving(true); const { error } = await supabase.from("homepage_hero_settings").upsert({ id: "default", layout_style: layout, updated_at: new Date().toISOString() }); if (error) setMessage(`Layout save failed: ${error.message}`); else { setSavedLayout(layout); setMessage("Global hero layout saved."); } setSaving(false); }

  return <>
    <section className="mt-8 overflow-hidden rounded-3xl bg-white text-slate-950 shadow-2xl">
      <div className="border-b border-slate-200 bg-slate-50 p-5 sm:p-7"><p className="text-xs font-black uppercase tracking-[0.2em] text-pink-600">Whole Hero Section</p><div className="mt-2 flex flex-col gap-3 lg:flex-row lg:items-end lg:justify-between"><div><h2 className="text-3xl font-black">Global Layout & Live Preview</h2><p className="mt-2 max-w-3xl text-slate-600">Choose the default structure for the homepage hero. Individual items can override it below.</p></div><div className="rounded-full bg-slate-950 px-4 py-2 text-xs font-black uppercase text-white">Saved: {savedLayout.replaceAll("_", " ")}</div></div></div>
      <div className="grid gap-7 p-5 sm:p-7 xl:grid-cols-[420px_1fr]"><div><div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-1">{LAYOUTS.map((option) => <button key={option.key} onClick={() => setLayout(option.key)} className={`rounded-2xl border-2 p-4 text-left transition ${layout === option.key ? "border-pink-500 bg-pink-50" : "border-slate-200 hover:border-slate-400"}`}><div className="flex items-center justify-between gap-3"><h3 className="font-black">{option.name}</h3><span className={`h-4 w-4 rounded-full border-4 ${layout === option.key ? "border-pink-600 bg-pink-600" : "border-slate-300"}`} /></div><p className="mt-1 text-sm text-slate-600">{option.note}</p></button>)}</div><button onClick={save} disabled={saving || layout === savedLayout} className="mt-5 w-full rounded-xl bg-pink-600 px-5 py-4 font-black text-white disabled:opacity-40">{saving ? "Saving..." : layout === savedLayout ? "Layout Saved" : "Save Global Layout"}</button>{message && <p className="mt-3 text-sm font-bold text-slate-600">{message}</p>}</div><div className="min-w-0"><div className="mb-4 flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between"><div><p className="text-xs font-black uppercase tracking-wide text-pink-600">Instant Preview</p><h3 className="text-2xl font-black">How this layout will look</h3></div>{previewItems.length > 0 && <select value={previewSource} onChange={(e) => setPreviewSource(e.target.value)} className="rounded-xl border border-slate-200 bg-white px-4 py-3 font-bold"><option value="auto">Use first active item</option>{previewItems.map((entry) => <option key={entry.key} value={entry.key}>{entry.label}</option>)}</select>}</div><HeroPreview layout={layout} item={previewItem} /><p className="mt-3 text-sm text-slate-500">Saving stores the default used by items set to “Use global layout.”</p></div></div>
    </section>
