...
export default function HeroCmsPage() {
  ...
  async function deleteItem(table: string, id: string, label: string) {
    ...
    if (error) setMessage(`Delete failed: ${error.message}`); else { setMessage(`${label} deleted.`); await loadContent(); }
  }

  async function duplicateBanner(banner: any) {
    const payload: any = { title: `${banner.title || "Untitled"} Copy`, subtitle: banner.subtitle || "", image_url: banner.image_url || "", button_text: banner.button_text || "", button_url: banner.button_url || "", banner_type: banner.banner_type || "marketing", start_date: banner.start_date || null, end_date: banner.end_date || null, display_order: Number(banner.display_order || 0) + 1, active: false, updated_at: new Date().toISOString() };
    const { error } = await supabase.from("homepage_hero_banners").insert(payload);
    if (error) setMessage(`Duplicate failed: ${error.message}`); else { setMessage("Banner duplicated as inactive copy."); await loadContent(); }
  }

  async function duplicateFestival(festival: any) {
    const payload: any = { festival_name: `${festival.festival_name || "Festival"} Copy`, festival_key: `${festival.festival_key || "festival"}_copy_${Date.now()}`, title: festival.title || "", subtitle: festival.subtitle || "", image_url: festival.image_url || "", start_date: festival.start_date || null, end_date: festival.end_date || null, active: false, updated_at: new Date().toISOString() };
    const { error } = await supabase.from("festival_hero_assets").insert(payload);
    if (error) setMessage(`Duplicate failed: ${error.message}`); else { setMessage("Festival duplicated as inactive copy."); await loadContent(); }
  }
