...
export default function HeroCmsPage() {
  ...
  async function uploadFestivalImage() {
    ...
    try {
      ...
      setMessage("Festival image uploaded.");
    } catch (error: any) {
          ...
          <section className="grid xl:grid-cols-[460px_1fr] gap-6">
            <div className="bg-white text-slate-950 rounded-2xl p-6">
              ...
              <div className="border rounded-xl p-4 mb-3 bg-slate-50">
                ...
                <input type="file" accept="image/*" onChange={(e) => setBannerFile(e.target.files?.[0] || null)} className="w-full text-sm mb-3" />
                <button type="button" onClick={uploadBannerImage} disabled={!bannerFile || uploadingBanner} className="w-full bg-slate-950 text-white px-4 py-3 rounded-xl font-bold disabled:opacity-50">{uploadingBanner ? "Uploading..." : "Upload Banner Image"}</button>
              </div>
              ...
              <button onClick={saveBanner} className="w-full bg-pink-600 text-white px-5 py-3 rounded-xl font-black">Save Banner</button>
            </div>
          ...
          <section className="grid xl:grid-cols-[460px_1fr] gap-6">
            <div className="bg-white text-slate-950 rounded-2xl p-6">
              ...
              <div className="border rounded-xl p-4 mb-3 bg-slate-50">
                ...
                <input type="file" accept="image/*" onChange={(e) => setFestivalFile(e.target.files?.[0] || null)} className="w-full text-sm mb-3" />
                <button type="button" onClick={uploadFestivalImage} disabled={!festivalFile || uploadingFestival} className="w-full bg-slate-950 text-white px-4 py-3 rounded-xl font-bold disabled:opacity-50">{uploadingFestival ? "Uploading..." : "Upload Festival Image"}</button>
              </div>
              ...
              <button onClick={saveFestival} className="w-full bg-pink-600 text-white px-5 py-3 rounded-xl font-black">Save Festival</button>
            </div>
