...
function roleCanBeTeamMember(role?: string | null) {
  ...
  return (
    ...
    normalized.includes("team_member") ||
...
function ImageThumb({ src, label }: { src?: string; label: string }) {
  if (!src)
    return (
      <div className="w-24 h-24 rounded-xl bg-pink-50 grid place-items-center text-pink-600 font-black text-xs text-center px-2">
        No image
      </div>
    );
  return (
    <img
      src={src}
      alt={label}
      className="w-24 h-24 rounded-xl object-cover bg-gray-100 border"
    />
  );
}
...
export default function StudioRadioTeamPage() {
  ...
  const selectedApprovedUser =
    approvedUsers.find(
      (item) => item.email.toLowerCase() === linkEmail.toLowerCase(),
    ) || null;
  const unlistedApprovedUsers = useMemo(
    () => approvedUsers.filter((item) => !item.already_radio),
    [approvedUsers],
  );
  ...
  async function enrichMembers(rows: RadioTeamMember[]) {
    ...
    if (emails.length > 0 || userIds.length > 0) {
      ...
      if (emails.length > 0 && userIds.length > 0)
        adminQuery = adminQuery.or(
          `email.in.(${emails.join(",")}),user_id.in.(${userIds.join(",")})`,
        );
      ...
      const { data: profileRows } = await profileQuery.order("created_at", {
        ascending: false,
      });
      (profileRows || []).forEach((item: any) => {
        if (item.email && !profileByEmail[String(item.email).toLowerCase()])
          profileByEmail[String(item.email).toLowerCase()] = item;
        if (item.user_id && !profileByUserId[item.user_id])
          profileByUserId[item.user_id] = item;
      });
    }
    ...
    return rows.map((member) => {
      ...
      const profile =
        profileByUserId[member.user_id || ""] || profileByEmail[emailKey] || {};
      return {
        ...member,
        user_role:
          roleByUserId[member.user_id || ""] || roleByEmail[emailKey] || "",
  ...
  async function loadData() {
    if (!site.id) {
      setActionMessage("The active site is not configured.");
      return;
    }
    const modernResult = await supabase
      .from("radio_team_members")
      .select(
        "id,name,title,segment_name,image,user_id,email,show_on_public_radio,created_by,created_at,updated_at",
      )
      .eq("site_id", site.id)
    ...
    if (modernResult.error) {
      ...
      if (legacyResult.error) {
        setActionMessage(
          `Could not load radio team members: ${legacyResult.error.message}`,
        );
        return;
      }
      setMembers(legacyResult.data || []);
      setActionMessage(
        "Radio team loaded in legacy mode. Run the latest supabase/team-profile-enhancements.sql for linking and visibility.",
      );
      return;
    }
    ...
    if (adminResult.error) {
      setActionMessage(
        `Could not load approved team users: ${adminResult.error.message}`,
      );
      return;
    }
    ...
    const emails = Array.from(
      new Set(adminRows.map((item: any) => item.email).filter(Boolean)),
    );
    const userIds = Array.from(
      new Set(adminRows.map((item: any) => item.user_id).filter(Boolean)),
    ) as string[];
    const profileByEmail: Record<string, any> = {};
    const profileByUserId: Record<string, any> = {};
    ...
    const linkedUserIds = new Set(
      enrichedMembers.map((member) => member.user_id).filter(Boolean),
    );
    ...
    setApprovedUsers(
      adminRows
        ...
        .map((item: any) => {
          const profile =
            profileByUserId[item.user_id || ""] ||
            profileByEmail[item.email] ||
            {};
          return {
            ...
            profile_created_at: profile.created_at || null,
            already_radio:
              linkedEmails.has(item.email) || linkedUserIds.has(item.user_id),
          };
        }),
    );
  }
  ...
  function startFromApprovedUser(item: ApprovedUser) {
    ...
    setActionMessage(
      `Publishing ${item.full_name || item.email} to Radio Team`,
    );
    window.scrollTo({ top: 0, behavior: "smooth" });
  }
  ...
  async function uploadImage(event: ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      setActionMessage(
        `Image upload failed: ${error?.message || String(error)}`,
      );
    } finally {
  ...
  async function saveMember() {
    ...
    if (selectedUser) {
      if (!selectedUser.user_id) {
        setActionMessage(
          "Selected user is missing user_id. Re-approve Team Access from Volunteer Requests and try again.",
        );
        return;
      }
    ...
    setActionMessage(
      editingId
        ? "Updating radio team member..."
        : "Adding radio team member...",
    );
    ...
    if (
      result.error &&
      String(result.error.message || "").includes("updated_at")
    ) {
    ...
    setActionMessage(
      editingId ? "Radio team member updated." : "Radio team member added.",
    );
    resetForm();
    await loadData();
    setSaving(false);
  ...
  async function toggleVisibility(member: RadioTeamMember) {
    ...
    const { error } = await supabase
      .from("radio_team_members")
      .update({
        show_on_public_radio: nextVisible,
        updated_at: new Date().toISOString(),
      })
      .eq("id", member.id)
      .eq("site_id", site.id!);
    ...
    setActionMessage(
      nextVisible
        ? "Radio member is now visible publicly."
        : "Radio member is now hidden publicly.",
    );
    await loadData();
  }
  ...
  async function deleteMember(member: RadioTeamMember) {
    const ok = window.confirm(
      `Delete radio team member: ${member.name}? This cannot be undone.`,
    );
    if (!ok) return;
          ...
          <div className="space-y-8">
            ...
            <section className="bg-white text-slate-950 rounded-2xl p-6">
              <h2 className="text-2xl font-black mb-4">
                {editingId
                  ? "Edit Radio Team Member"
                  : "Add / Publish Radio Team Member"}
              </h2>
              {!editingMember?.user_id && (
                <div className="bg-yellow-50 border border-yellow-200 rounded-2xl p-4 mb-5">
                  <p className="font-black text-yellow-900">
                    Connect to approved team user
                  </p>
                  <p className="text-xs text-yellow-800 mt-1">
                    This option only appears for new/unconnected radio profiles.
                  </p>
                  <select
                    className="border rounded-xl p-3 w-full bg-white mt-3"
                    value={linkEmail}
                    onChange={(event) => setLinkEmail(event.target.value)}
                  >
                    <option value="">
                      Manual radio profile / no user link
                    </option>
                    {approvedUsers.map((item) => (
                      <option
                        key={`${item.email}-${item.user_id || "no-id"}`}
                        value={item.email}
                      >
                        {item.full_name ? `${item.full_name} · ` : ""}
                        {item.email} · {roleLabel(item.role)}
                        {item.already_radio ? " · already in radio" : ""}
                      </option>
                    ))}
                  </select>
                </div>
              ...
              <div className="grid md:grid-cols-[1fr_1fr_1fr_1.4fr] gap-4">
                ...
                <label className="grid gap-2 text-sm font-bold">
                  ...
                  <input
                    ...
                    onChange={(event) =>
                      setForm({ ...form, segment_name: event.target.value })
                    }
                    placeholder="Segment"
                  />
                </label>
                <div className="grid gap-2 text-sm font-bold">
                  ...
                  <p className="text-xs text-gray-500">
                    Image files must be image type and 5MB or smaller. Pasted
                    image URL must be valid.
                  </p>
                  {!cloudinaryReady && (
                    <p className="text-xs text-orange-600">
                      Cloudinary upload is not configured; image URL still
                      works.
                    </p>
                  )}
                </div>
              ...
              <label className="mt-4 flex items-center gap-3 text-sm font-bold">
                <input
                  type="checkbox"
                  checked={form.show_on_public_radio}
                  onChange={(event) =>
                    setForm({
                      ...form,
                      show_on_public_radio: event.target.checked,
                    })
                  }
                />{" "}
                Show on public Radio page
              </label>
              <div className="flex flex-wrap gap-3 mt-5">
                <button
                  onClick={saveMember}
                  disabled={saving || uploadingImage}
                  className="bg-pink-600 text-white px-5 py-3 rounded-xl font-bold disabled:opacity-60"
                >
                  {saving
                    ? "Saving..."
                    : editingId
                ...
                {editingId && (
                  <button
                    onClick={resetForm}
                    className="border border-gray-400 text-gray-700 px-5 py-3 rounded-xl font-bold"
                  >
                    Cancel Edit
                  </button>
                )}
            ...
            {unlistedApprovedUsers.length > 0 && (
              <section className="bg-white text-slate-950 rounded-2xl p-6">
                <h2 className="text-2xl font-black">
                  Approved Team Members Not on Radio Page
                </h2>
                <div className="grid md:grid-cols-2 xl:grid-cols-3 gap-4 mt-4">
                  {unlistedApprovedUsers.map((item) => (
                    <article
                      key={item.email}
                    ...
                    >
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <button
                          onClick={() => startFromApprovedUser(item)}
                          className="bg-slate-900 text-white px-3 py-2 rounded-lg font-bold text-sm mt-3"
                        >
                          Publish to Radio Team
                        </button>
                      </div>
                    </article>
                  ))}
            ...
            <section className="bg-white text-slate-950 rounded-2xl p-6">
              ...
              <div className="grid md:grid-cols-2 xl:grid-cols-3 gap-4">
                {members.map((member) => (
                  ...
                  >
                    <ImageThumb
                      src={member.profile_photo_url || member.image}
                      label={member.name}
                    />
                    <div className="flex-1 min-w-0">
                      <div className="flex gap-2 justify-between">
                        <div>
                          ...
                          <p className="text-sm font-bold text-pink-600 mt-1">
                            {member.segment_name || "No segment"}
                          </p>
                        </div>
                        <span
                          className={`text-xs font-black rounded-full px-2 py-1 h-fit ${member.user_role ? "bg-green-50 text-green-700" : "bg-yellow-50 text-yellow-700"}`}
                        >
                          {roleLabel(member.user_role)}
                        </span>
                      </div>
                      <div className="mt-3 grid gap-1 text-xs text-gray-600 break-words">
                        <p>
                          <b>Email:</b> {member.email || "Not linked"}
                        </p>
                        <p>
                          <b>User ID:</b> {shortId(member.user_id)}
                        </p>
                        <p>
                          <b>Public Radio:</b>{" "}
                          {member.show_on_public_radio === false
                            ? "Hidden"
                            : "Visible"}
                        </p>
                      </div>
                      <div className="flex flex-wrap gap-2 mt-4">
                        ...
                        <button
                          ...
                          className="border border-slate-400 text-slate-700 px-3 py-2 rounded-lg font-bold text-sm"
                        >
                          {member.show_on_public_radio === false
                            ? "Show Publicly"
                            : "Hide Publicly"}
                ...
                {members.length === 0 && (
                  <p className="text-gray-500">No radio team members found.</p>
                )}
              </div>
            </section>
          </div>
