      ...
      <div className="max-w-7xl mx-auto px-6 py-10">
        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 mb-6">
          <div>
            ...
            <p className="text-slate-300 mt-2">Generate, save, preview, and reuse SDTV newsletters.</p>
        ...
        {!loading && canAccess && <div className="space-y-8">
          ...
          <div className="grid gap-3 md:grid-cols-3">
            <button onClick={() => setActiveTab("generation")} className={tabClass("generation")}>Generate Newsletter</button>
            <button onClick={() => setActiveTab("ordering")} className={tabClass("ordering")}>Newsletter Ordering</button>
            <button onClick={() => setActiveTab("subscribers")} className={tabClass("subscribers")}>Subscribers</button>
          </div>

          {activeTab === "ordering" && <section className="bg-white text-slate-950 rounded-2xl p-6 shadow-xl">
            <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 mb-5"><div><h2 className="text-2xl font-black">Newsletter Ordering</h2><p className="text-gray-600 text-sm mt-1">Independent from homepage order. Enable, reorder, rename, and set item limits.</p></div><div className="flex gap-3"><button onClick={seedDefaults} className="border px-4 py-2 rounded-xl font-bold">Seed Defaults</button><button onClick={saveSettings} className="bg-pink-600 text-white px-4 py-2 rounded-xl font-bold">Save Control</button></div></div>
          ...
          {activeTab === "subscribers" && <div className="space-y-8">
            <section className="grid gap-4 md:grid-cols-4">{[["Total", stats.total, "all"], ["Subscribed", stats.active, "active"], ["Pending", stats.pending, "pending"], ["Unsubscribed", stats.unsubscribed, "unsubscribed"]].map(([label, value, filter]) => <button key={label as string} onClick={() => setSubscriberFilter(filter as string)} className={`${subscriberFilter === filter ? "ring-4 ring-pink-300" : ""} text-left rounded-2xl bg-white p-5 text-slate-950 shadow-xl`}><p className="text-sm font-black text-slate-500">{label}</p><p className="mt-2 text-4xl font-black text-pink-600">{value as number}</p></button>)}</section>
            <section className="bg-white text-slate-950 rounded-2xl p-6 shadow-xl"><div className="flex flex-col md:flex-row md:items-center md:justify-between gap-3 mb-4"><div><h2 className="text-2xl font-black">Subscribers</h2><p className="text-sm text-gray-600">Filter: {subscriberFilter} · showing {filteredSubscribers.length}</p></div><div className="flex gap-2 flex-wrap">{["all","active","pending","unsubscribed"].map((filter) => <button key={filter} onClick={() => setSubscriberFilter(filter)} className={`${subscriberFilter === filter ? "bg-pink-600 text-white" : "border"} px-3 py-2 rounded-xl font-bold capitalize`}>{filter}</button>)}</div></div><div className="overflow-x-auto"><table className="w-full text-sm"><thead><tr className="text-left border-b"><th className="py-3">Email</th><th>Name</th><th>Status</th><th>Source</th><th>Subscribed</th><th>Unsubscribed</th><th>Actions</th></tr></thead><tbody>{filteredSubscribers.slice(0, 200).map((subscriber) => <tr key={subscriber.id || subscriber.email} className="border-b"><td className="py-3 font-bold">{subscriber.email}</td><td>{subscriber.name || "—"}</td><td><span className="rounded-full bg-slate-100 px-2 py-1 text-xs font-black uppercase">{subscriber.status || "active"}</span></td><td>{subscriber.source_page || "—"}</td><td>{subscriber.subscribed_at ? new Date(subscriber.subscribed_at).toLocaleDateString() : "—"}</td><td>{subscriber.unsubscribed_at ? new Date(subscriber.unsubscribed_at).toLocaleDateString() : "—"}</td><td className="py-2"><div className="flex gap-2">{subscriber.status === "unsubscribed" ? <button onClick={() => setSubscriberStatus(subscriber, "active")} className="border px-3 py-2 rounded-lg font-bold">Resubscribe</button> : <button onClick={() => setSubscriberStatus(subscriber, "unsubscribed")} className="border px-3 py-2 rounded-lg font-bold text-red-600">Unsubscribe</button>}</div></td></tr>)}</tbody></table></div></section>
          </div>}
        </div>}
      </div>
    </main>
