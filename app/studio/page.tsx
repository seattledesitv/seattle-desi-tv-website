...
type DashboardCounts = { events: number; pendingEvents: number; approvedEvents: number; onHoldEvents: number; rejectedEvents: number; businesses: number; pendingBusinesses: number; crew: number; pendingCrew: number; team: number; radioTeam: number; roleRequests: number; coverageRequests: number; videoWorkflows: number; videoAdminApprovals: number; videoCrewReviews: number; videoPublishingReady: number; };
const emptyCounts: DashboardCounts = { events: 0, pendingEvents: 0, approvedEvents: 0, onHoldEvents: 0, rejectedEvents: 0, businesses: 0, pendingBusinesses: 0, crew: 0, pendingCrew: 0, team: 0, radioTeam: 0, roleRequests: 0, coverageRequests: 0, videoWorkflows: 0, videoAdminApprovals: 0, videoCrewReviews: 0, videoPublishingReady: 0 };
...
export default function StudioDashboardPage() {
  ...
  async function loadCounts() {
    const [eventsResult, pendingEventsResult, approvedEventsResult, onHoldEventsResult, rejectedEventsResult, businessesResult, pendingBusinessesResult, crewResult, pendingCrewResult, teamResult, radioTeamResult, roleRequestsResult, coverageRequestsResult, videoWorkflowsResult, videoAdminApprovalsResult, videoCrewReviewsResult, videoPublishingReadyResult] = await Promise.all([
