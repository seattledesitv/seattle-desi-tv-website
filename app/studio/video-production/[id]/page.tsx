...
function getWorkflowIdFromPath() { if (typeof window === "undefined") return ""; const parts = window.location.pathname.split("/").filter(Boolean); const idx = parts.indexOf("video-production"); return idx >= 0 ? parts[idx + 1] || "" : ""; }
...
export default function VideoWorkflowDetailPage() {
  ...
  async function submitDraft(e: React.FormEvent) {
    e.preventDefault(); if (!workflow?.id || !user?.id) return;
  ...
  async function markPublished(e: React.FormEvent) { e.preventDefault(); await updateWorkflow({ status: "published_complete", youtube_url: publish.youtube_url || null, instagram_url: publish.instagram_url || null, facebook_url: publish.facebook_url || null, publish_notes: publish.publish_notes || null, published_at: new Date().toISOString() }, "Publishing cycle marked complete."); }
