...
export default function StudioDirectoryImageEditorPage() {
  const [record, setRecord] = useState<any>(null); const [message, setMessage] = useState("Checking access..."); const [kind, setKind] = useState<"business" | "organization">("business");
  async function load() {
    const params = new URLSearchParams(window.location.search); const nextKind = params.get("type") === "organization" ? "organization" : "business"; const id = params.get("id") || ""; setKind(nextKind);
    if (!id) return setMessage("Choose a business or organization from Studio.");
    const { data: auth } = await supabase.auth.getUser(); const user = auth?.user || null; if (!user) return setMessage("Please login to access Studio.");
    const role = await resolveUserRole(supabase, user); if (!isAdminRole(role)) return setMessage("Admin access required.");
    const table = nextKind === "business" ? "local_businesses" : "community_organizations";
    const full = await supabase.from(table).select("id,name,image,image_urls,image_position_x,image_position_y,image_zoom,image_display_mode").eq("id", id).maybeSingle();
    if (!full.error && full.data) { setRecord(full.data); setMessage(""); return; }
    if (!/image_urls/i.test(full.error?.message || "")) return setMessage(full.error?.message || "Listing not found.");
    const fallback = await supabase.from(table).select("id,name,image,image_position_x,image_position_y,image_zoom,image_display_mode").eq("id", id).maybeSingle();
    if (fallback.error || !fallback.data) return setMessage(fallback.error?.message || "Listing not found.");
    setRecord(fallback.data); setMessage("");
  }
  useEffect(() => { void load(); }, []);
