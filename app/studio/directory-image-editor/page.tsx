...
export default function StudioDirectoryImageEditorPage() {
  ...
  async function load() {
    ...
    const { data, error } = await supabase.from(table).select("id,name,image,image_urls,image_position_x,image_position_y,image_zoom").eq("id", id).maybeSingle();
  ...
  return <main className="min-h-screen bg-slate-950 text-white"><StudioHeader/><section className="mx-auto max-w-5xl px-6 py-10"><a href={backHref} className="font-black text-pink-300">← Back to {kind === "business" ? "Businesses" : "Organizations"}</a><h1 className="mt-3 text-4xl font-black">Studio Directory Image Editor</h1><p className="mt-2 text-slate-300">Upload an image and choose exactly what appears on the public directory card.</p>{message && <div className="mt-6 rounded-2xl bg-yellow-100 p-4 font-bold text-yellow-900">{message}</div>}{record && <div className="mt-6 rounded-3xl bg-white p-6 text-slate-950"><StudioDirectoryImageManager table={kind === "business" ? "local_businesses" : "community_organizations"} recordId={record.id} name={record.name} image={record.image} imageUrls={record.image_urls} positionX={record.image_position_x} positionY={record.image_position_y} zoom={record.image_zoom} folder={kind === "business" ? "businesses" : "organizations"} onSaved={load}/></div>}</section></main>;
}
