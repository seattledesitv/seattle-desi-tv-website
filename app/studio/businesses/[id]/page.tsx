...
export default function BusinessEditPage() {
  ...
  async function loadBusiness(id: string) {
    if (!id || !UUID_RE.test(id)) {
      setActionMessage("Could not load business: missing or invalid business ID. Please go back to Businesses and open Edit again.");
      return;
    }
    const { data, error } = await forSite(supabase.from("local_businesses").select("id,name,address,website,category,discount,offer,poc_name,poc_email,poc_phone,image,status,approved"), site.id).eq("id", id).maybeSingle();
  ...
  async function init() {
    ...
    await loadBusiness(String(businessId || "")); setMessage(""); setLoading(false);
  }
  ...
  async function saveBusiness() {
    if (!businessId || !UUID_RE.test(String(businessId))) { setActionMessage("Save failed: missing or invalid business ID. Please go back to Businesses and open Edit again."); return; }
    ...
    const { error } = await forSite(supabase.from("local_businesses").update(payload), site.id).eq("id", String(businessId));
  ...
  useEffect(() => { init(); }, [businessId, site.id]);
