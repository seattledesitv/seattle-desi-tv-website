...
export default function BusinessEditPage({ params }: { params: { id: string } }) {
  ...
  async function loadBusiness() {
    const { data, error } = await supabase
      ...
      .select("id,name,address,website,category,discount,offer,poc_name,poc_email,poc_phone,image,status,approved")
      .eq("id", params.id)
      .maybeSingle();

    if (error) {
      setActionMessage(`Could not load business: ${error.message}`);
      return;
    }

    if (!data) {
      setActionMessage("Business not found.");
      return;
    }
  ...
  async function init() {
    ...
    if (!currentUser) {
      setRole("");
      setMessage("Please login to edit businesses.");
      setLoading(false);
      return;
    }
  ...
  async function saveBusiness() {
    if (!form.name.trim()) {
      setActionMessage("Business name is required.");
      return;
    }

    setSaving(true);
    setActionMessage("Saving business...");
    ...
    setActionMessage("Business saved.");
    setSaving(false);
  }
  ...
  return (
    <main className="min-h-screen bg-slate-950 text-white px-6 py-10">
      <div className="max-w-5xl mx-auto">
        <a href="/studio/businesses" className="text-pink-300 font-bold">← Back to Businesses</a>
        <h1 className="text-4xl md:text-5xl font-black mt-3 mb-2">Edit Business</h1>
        <p className="text-slate-300 mb-8">{user?.email ? `Logged in as ${user.email} · Role: ${role || "none"}` : "Business editor"}</p>
        ...
        {!loading && canAccess && (
          <section className="bg-white text-slate-950 rounded-2xl p-6 space-y-5">
            ...
            <div className="grid gap-2 text-sm font-bold">
              ...
              {form.image && <img src={form.image} alt="Business preview" className="w-40 h-40 object-cover rounded-xl border" />}
              {!cloudinaryReady && <p className="text-xs text-orange-600">Cloudinary upload is not configured; image URL still works.</p>}
            </div>

            <button onClick={saveBusiness} disabled={saving || uploadingImage} className="bg-pink-600 text-white px-6 py-3 rounded-xl font-black disabled:opacity-60">{saving ? "Saving..." : "Save Business"}</button>
          </section>
        )}
      </div>
    </main>
