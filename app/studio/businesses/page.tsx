...
export default function StudioBusinessesPage() {
  ...
  async function saveWebsite(business: any) {
    ...
    if (!website) return setActionMessage(`Enter an official website for ${business.name}.`);
    try { new URL(website); } catch { return setActionMessage("Enter a valid website address."); }
    ...
    setSavingWebsite("");
    if (error) return setActionMessage(`Could not save website: ${error.message}`);
    setActionMessage(`Official website saved for ${business.name}.`); await loadBusinesses();
  }

  async function saveContactEmail(business: any) {
    const email = String(emailDrafts[business.id] || "").trim().toLowerCase();
    if (!email || !email.includes("@")) return setActionMessage("Enter a valid business contact email.");
    setSavingEmail(business.id);
    const { error } = await supabase.from("local_businesses").update({ contact_email: email }).eq("id", business.id);
    setSavingEmail("");
    if (error) return setActionMessage(`Could not save contact email: ${error.message}`);
    ...
    setActionMessage(`Contact email saved for ${business.name}.`); await loadBusinesses();
  }
  ...
  async function sendListingNotice(business: any) {
    ...
    if (!recipient || !recipient.includes("@")) return setActionMessage("Add and save a valid contact email first.");
    ...
    setSendingNotice(business.id); setActionMessage(`${forceResend ? "Resending" : "Sending"} listing notice to ${recipient}...`);
    ...
    setSendingNotice("");
    if (!response.ok) return setActionMessage(result.error || "Could not send listing notice.");
    setActionMessage(`Listing notice sent to ${result.recipient}. Response due ${formatDate(result.responseDueAt)}.`); await loadBusinesses();
  }

  async function updateLogoRights(business: any, value: string) {
    setSavingRights(business.id);
    ...
    setSavingRights("");
    if (error) return setActionMessage(`Could not update logo rights: ${error.message}`);
    ...
    setActionMessage(`Logo rights status updated for ${business.name}.`); await loadBusinesses();
  }

  async function discoverImages(business: any) {
    if (!business.website) return setActionMessage(`Save an official website for ${business.name} first.`);
    ...
    if (!response.ok) return setActionMessage(result.error || "Could not discover images.");
    setCandidates((current) => ({ ...current, [business.id]: result.candidates || [] }));
    setActionMessage(result.candidates?.length ? `Found ${result.candidates.length} candidate image(s) for ${business.name}.` : `No suitable official-site images were found for ${business.name}.`);
  }
  ...
  async function applyImage(business: any, candidate: Candidate) {
    ...
    if (error) return setActionMessage(`Could not save image: ${error.message}`);
    ...
    setCandidates((current) => ({ ...current, [business.id]: [] })); setActionMessage(`Image saved for ${business.name}; rights review is still required.`); await loadBusinesses();
  }
  ...
  async function updateBusinessStatus(id: string, status: string) {
    ...
    if (error) return setActionMessage(`Business update failed: ${error.message}`);
    setActionMessage(`Business marked ${status}.`); await loadBusinesses();
  }

  async function deleteBusiness(id: string, name: string) { if (!window.confirm(`Delete business: ${name}? This cannot be undone.`)) return; const { error } = await supabase.from("local_businesses").delete().eq("id", id); if (error) return setActionMessage(`Business delete failed: ${error.message}`); setActionMessage("Business deleted."); await loadBusinesses(); }
  ...
  const visibleBusinesses = useMemo(() => {
    ...
    return businesses.filter((business) => {
      ...
      const textMatches = !query || [business.name, business.address, business.category, business.source_name, business.import_batch, business.contact_email].some((value) => String(value || "").toLowerCase().includes(query));
  ...
  return <main className="min-h-screen bg-slate-950 text-white"><StudioHeader/><div className="mx-auto max-w-7xl px-6 py-10">
    <div className="mb-8 flex flex-col gap-4 md:flex-row md:items-center md:justify-between"><div><h1 className="text-4xl font-black md:text-5xl">Businesses Management</h1><p className="mt-2 text-slate-300">Research, enrich, contact, and review each business from one workspace.</p><p className="mt-1 text-sm text-slate-400">{user?.email ? `Logged in as ${user.email} · Role: ${role || "none"}` : "Studio businesses"}</p></div><div className="flex gap-3"><button onClick={init} className="rounded-xl bg-white px-5 py-3 font-bold text-slate-950">Refresh</button>{user&&<button onClick={logout} className="rounded-xl border border-red-400 px-5 py-3 font-bold text-red-300">Logout</button>}</div></div>
