...
function roleContainsAdmin(role?: string | null) {
...
export default function StudioTeamPage() {
  ...
  async function enrichMembers(rows: TeamMember[]) {
    ...
    return rows.map((member) => {
      ...
      return { ...member, user_role: roleByUserId[member.user_id || ""] || roleByEmail[emailKey] || "", profile_photo_url: profile.photo_url || member.image || "", profile_created_at: profile.created_at || null };
    });
  }
  ...
  async function loadData() {
    ...
    if (teamResult.error) {
      setActionMessage(`Could not load team members: ${teamResult.error.message}`);
      return;
    }
    ...
    setApprovedUsers(adminRows.filter((item: any) => {
      ...
      return { user_id: item.user_id || profile.user_id || null, email: item.email, role: item.role || "team_member", full_name: profile.full_name || null, photo_url: profile.photo_url || null, profile_created_at: profile.created_at || null, already_linked: linkedEmails.has(item.email) || linkedUserIds.has(item.user_id) };
    }));
  }
  ...
  function startEdit(member: TeamMember) {
    ...
    setForm({ name: member.name || "", title: member.title || "", image: member.image || "", show_on_public_team: member.show_on_public_team !== false });
  ...
  function startFromApprovedUser(item: ApprovedUser) {
    ...
    setForm({ name: item.full_name || item.email, title: "Team Member", image: item.photo_url || "", show_on_public_team: true });
    setActionMessage(`Publishing ${item.full_name || item.email} to Team page`);
    window.scrollTo({ top: 0, behavior: "smooth" });
  }
  ...
  async function saveMember() {
    ...
    setSaving(true);
    setActionMessage(editingId ? "Updating team member..." : "Publishing team member...");
    ...
    setActionMessage(editingId ? "Team member updated." : "Team member published to Team page.");
    resetForm();
    await loadData();
    setSaving(false);
  }
  ...
  async function linkMemberToUser() {
    if (!editingId || !selectedApprovedUser) {
      setActionMessage("Select a team member and approved user first.");
      return;
    }
    await saveMember();
  }
  ...
  async function toggleVisibility(member: TeamMember) {
    setActionMessage("Updating public Team visibility...");
    const nextVisible = member.show_on_public_team === false;
    const { error } = await supabase.from("team_members").update({ show_on_public_team: nextVisible, updated_at: new Date().toISOString() }).eq("id", member.id);
    if (error) {
      setActionMessage(`Visibility update failed: ${error.message}`);
      return;
    }
    setActionMessage(nextVisible ? "Team member is now visible publicly." : "Team member is now hidden publicly.");
    await loadData();
  }

  async function deleteMember(member: TeamMember) {
    const ok = window.confirm(`Remove team member: ${member.name}? This only removes the public Team page row. It does not remove login or role.`);
    if (!ok) return;
    setActionMessage("Removing team member...");
    const { error } = await supabase.from("team_members").delete().eq("id", member.id);
    if (error) {
      setActionMessage(`Remove failed: ${error.message}`);
      return;
    }
    ...
    <main className="min-h-screen bg-slate-950 text-white">
      ...
      <div className="max-w-7xl mx-auto px-6 py-10">
        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 mb-8">
          ...
          <div className="flex flex-wrap gap-3"><button onClick={init} className="bg-white text-slate-950 px-5 py-3 rounded-xl font-bold">Refresh</button>{user && <button onClick={logout} className="border border-red-400 text-red-300 px-5 py-3 rounded-xl font-bold">Logout</button>}</div>
        </div>
        ...
        {!loading && canAccess && (
          <div className="space-y-8">
            ...
            <section className="bg-white text-slate-950 rounded-2xl p-6">
              <h2 className="text-2xl font-black mb-4">{editingId ? "Edit Team Member" : "Add / Publish Team Member"}</h2>
              {!editingMember?.user_id && (
                <div className="bg-yellow-50 border border-yellow-200 rounded-2xl p-4 mb-5">
                  ...
                  <p className="text-xs text-yellow-800 mt-1">This link option only appears for new/unconnected team profiles.</p>
                  <select className="border rounded-xl p-3 w-full bg-white mt-3" value={linkEmail} onChange={(event) => setLinkEmail(event.target.value)}>
                    ...
                    {approvedUsers.map((item) => <option key={`${item.email}-${item.user_id || "no-id"}`} value={item.email}>{item.full_name ? `${item.full_name} · ` : ""}{item.email} · {roleLabel(item.role)}{item.already_linked ? " · already on team" : ""}</option>)}
                  </select>
                  {selectedApprovedUser && <div className="mt-3 bg-white rounded-xl p-3 text-xs text-gray-700 flex gap-3 items-center"><ImageThumb src={selectedApprovedUser.photo_url || undefined} label={selectedApprovedUser.full_name || selectedApprovedUser.email} /><div><p><b>Selected:</b> {selectedApprovedUser.full_name || selectedApprovedUser.email}</p><p><b>Email:</b> {selectedApprovedUser.email}</p><p><b>User ID:</b> {shortId(selectedApprovedUser.user_id)}</p><p><b>Role:</b> {roleLabel(selectedApprovedUser.role)}</p><p><b>Onboarding profile:</b> {formatDate(selectedApprovedUser.profile_created_at)}</p></div></div>}
                </div>
              )}

              <div className="grid md:grid-cols-[1fr_1fr_1.4fr] gap-4">
                <label className="grid gap-2 text-sm font-bold">Name<input className="border rounded-lg p-3 font-normal" value={form.name} onChange={(event) => setForm({ ...form, name: event.target.value })} placeholder="Team member name" /></label>
                <label className="grid gap-2 text-sm font-bold">Title / Role Display<input className="border rounded-lg p-3 font-normal" value={form.title} onChange={(event) => setForm({ ...form, title: event.target.value })} placeholder="Founder, Host, Volunteer..." /></label>
                <div className="grid gap-2 text-sm font-bold">Team Page / ID Image<input type="file" accept="image/*" onChange={uploadImage} disabled={uploadingImage} className="border rounded-lg p-3 font-normal" /><input className="border rounded-lg p-3 font-normal" value={form.image} onChange={(event) => setForm({ ...form, image: event.target.value })} placeholder="Or paste image URL" />{form.image && <img src={form.image} alt="Preview" className="w-24 h-24 object-cover rounded-xl border" />}{!cloudinaryReady && <p className="text-xs text-orange-600">Cloudinary upload is not configured; image URL still works.</p>}</div>
              </div>
              <label className="mt-4 flex items-center gap-3 text-sm font-bold"><input type="checkbox" checked={form.show_on_public_team} onChange={(event) => setForm({ ...form, show_on_public_team: event.target.checked })} /> Show on public Team page</label>
