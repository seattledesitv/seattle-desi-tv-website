...
export default function StudioVolunteersPage() {
  ...
  async function publishToTeamPage(request: any, onboardingOverride?: any) {
    ...
    const payload: any = { user_id: request.user_id || onboarding.user_id || null, linked_user: request.user_id || onboarding.user_id || null, email, name, title, image, show_on_public_team: true, updated_at: now };
  ...
  async function saveProfileChanges() {
    ...
    setSelected({ ...selected, onboarding: updatedOnboarding });
    if (selected.request?.status === "approved") await publishToTeamPage(selected.request, updatedOnboarding);
    setActionMessage("Onboarding profile changes saved and Team page profile synced if already approved.");
    await loadVolunteers();
  }

  useEffect(() => { init(); }, []);

  return <main className="min-h-screen bg-slate-950 text-white"><StudioHeader /><div className="mx-auto max-w-7xl px-6 py-10"><div className="mb-8 flex flex-col gap-4 md:flex-row md:items-center md:justify-between"><div><h1 className="text-4xl font-black md:text-5xl">Volunteer Management</h1><p className="mt-2 text-slate-300">{user?.email ? `Logged in as ${user.email} · Role: ${role}` : "Studio volunteers"}</p></div><button onClick={init} className="rounded-xl bg-white px-5 py-3 font-black text-slate-950">Refresh</button></div>{loading && <div className="rounded-2xl bg-white/10 p-6">{message}</div>}{!loading && !canAccess && <div className="rounded-2xl bg-white p-8 text-slate-950">{message}</div>}{!loading && canAccess && <div className="grid gap-6 lg:grid-cols-[430px_1fr]">{actionMessage && <div className="lg:col-span-2 rounded-2xl bg-yellow-100 p-4 font-bold text-yellow-900">{actionMessage}</div>}<section className="rounded-3xl bg-white p-5 text-slate-950"><h2 className="text-2xl font-black">Volunteer Requests</h2><div className="mt-4 grid max-h-[760px] gap-3 overflow-y-auto pr-1">{requests.map((request) => { const onboarding = submissionByRequestId[request.id]; return <button key={request.id} onClick={() => openReview(request, onboarding)} className={`rounded-2xl border p-4 text-left ${selected?.request?.id === request.id ? "border-pink-500 bg-pink-50" : "bg-white"}`}><div className="flex justify-between gap-3"><div><p className="font-black">{onboarding?.full_name || request.email}</p><p className="text-sm text-slate-600">{request.email}</p><p className="mt-1 text-xs font-bold text-slate-500">{label(request.status)}</p></div>{onboarding?.photo_url && <img src={onboarding.photo_url} alt="profile" className="h-14 w-14 rounded-xl object-cover" />}</div></button>; })}</div></section><section className="rounded-3xl bg-white p-6 text-slate-950">{!selected && <div className="text-slate-500">Select a volunteer request to review onboarding and approve team access.</div>}{selected && <div><div className="flex flex-col gap-3 md:flex-row md:items-start md:justify-between"><div><p className="text-xs font-black uppercase tracking-wide text-pink-600">Selected Volunteer</p><h2 className="text-3xl font-black">{selected.onboarding?.full_name || selected.request.email}</h2><p className="text-sm font-bold text-slate-500">{selected.request.email}</p><p className="mt-2 rounded-full bg-slate-100 px-3 py-1 inline-block text-xs font-black">{label(selected.request.status)}</p></div><div className="grid gap-2"><select value={teamAreas[selected.request.id] || "General Team"} onChange={(e) => setTeamAreas({ ...teamAreas, [selected.request.id]: e.target.value })} className="rounded-xl border p-3 font-bold">{TEAM_AREAS.map((area) => <option key={area} value={area}>{area}</option>)}</select><button onClick={() => openReview(selected.request, selected.onboarding)} className="rounded-xl bg-slate-950 px-5 py-3 font-black text-white">Review / Edit Profile</button><button onClick={() => selected.onboarding && publishToTeamPage(selected.request)} className="rounded-xl border border-green-600 px-5 py-3 font-black text-green-700">Sync Team Page Profile</button>{selected.request.status !== "approved" && <button onClick={() => approveTeamAccess(selected.request)} className="rounded-xl bg-green-50 px-5 py-3 font-black text-green-700">Approve team member</button>}</div></div><div className="mt-5 grid gap-3 md:grid-cols-3">{readOnly("Request Created", fmt(selected.request.created_at))}{readOnly("Request Updated", fmt(selected.request.updated_at))}{readOnly("Approved At", fmt(selected.request.approved_at))}{readOnly("Approved By", selected.request.approved_by)}{readOnly("Approved Role", selected.request.approved_role)}{readOnly("User ID", selected.request.user_id)}</div><div className="mt-4 flex flex-wrap gap-2"><button onClick={() => updateStatus(selected.request, "awaiting_orientation")} className="rounded-xl bg-slate-100 px-4 py-3 text-sm font-black text-slate-700">Mark Awaiting Orientation</button><button onClick={() => updateStatus(selected.request, "awaiting_onboarding")} className="rounded-xl bg-blue-50 px-4 py-3 text-sm font-black text-blue-700">Enable Onboarding</button><button onClick={() => updateStatus(selected.request, "awaiting_team_role_access")} className="rounded-xl bg-yellow-50 px-4 py-3 text-sm font-black text-yellow-800">Awaiting Team Access</button><button onClick={() => updateStatus(selected.request, "rejected")} className="rounded-xl bg-red-50 px-4 py-3 text-sm font-black text-red-700">Reject</button></div>{selected.onboarding ? <div className="mt-6"><div className="grid gap-4 md:grid-cols-2"><label className="font-bold">Full Name<input value={editProfile?.full_name || ""} onChange={(e) => setEditProfile({ ...editProfile, full_name: e.target.value })} className="mt-1 w-full rounded-xl border p-3 font-normal" /></label><label className="font-bold">Phone<input value={editProfile?.phone || ""} onChange={(e) => setEditProfile({ ...editProfile, phone: e.target.value })} className="mt-1 w-full rounded-xl border p-3 font-normal" /></label><label className="font-bold">City<input value={editProfile?.city || ""} onChange={(e) => setEditProfile({ ...editProfile, city: e.target.value })} className="mt-1 w-full rounded-xl border p-3 font-normal" /></label><label className="font-bold">Photo<input type="file" accept="image/*" disabled={photoUploading} onChange={(e) => uploadAdminProfilePhoto(e.target.files?.[0])} className="mt-1 w-full rounded-xl border p-3 font-normal" /></label><label className="font-bold md:col-span-2">Photo URL<input value={editProfile?.photo_url || ""} onChange={(e) => setEditProfile({ ...editProfile, photo_url: e.target.value })} className="mt-1 w-full rounded-xl border p-3 font-normal" /></label>{editProfile?.photo_url && <div className="md:col-span-2 rounded-2xl bg-slate-50 p-4"><img src={editProfile.photo_url} alt="Volunteer profile" className="max-h-72 rounded-xl object-contain" /></div>}<label className="font-bold md:col-span-2">Interests<textarea value={editProfile?.interests || ""} onChange={(e) => setEditProfile({ ...editProfile, interests: e.target.value })} className="mt-1 min-h-24 w-full rounded-xl border p-3 font-normal" /></label><label className="font-bold md:col-span-2">Availability<textarea value={editProfile?.availability || ""} onChange={(e) => setEditProfile({ ...editProfile, availability: e.target.value })} className="mt-1 min-h-24 w-full rounded-xl border p-3 font-normal" /></label><label className="font-bold md:col-span-2">Experience<textarea value={editProfile?.experience || ""} onChange={(e) => setEditProfile({ ...editProfile, experience: e.target.value })} className="mt-1 min-h-24 w-full rounded-xl border p-3 font-normal" /></label></div>{photoMessage && <div className="mt-4 rounded-2xl bg-yellow-50 p-4 text-sm font-bold text-yellow-800">{photoMessage}</div>}<div className="mt-5 grid gap-3 md:grid-cols-3">{readOnly("Onboarding Created", fmt(selected.onboarding.created_at))}{readOnly("Onboarding Updated", fmt(selected.onboarding.updated_at))}{readOnly("Onboarding Status", selected.onboarding.status)}{readOnly("Agreement Acknowledged", selected.onboarding.agreement_acknowledged ? "Yes" : "No")}{readOnly("Agreement Acknowledged At", fmt(selected.onboarding.agreement_acknowledged_at))}{readOnly("Submission ID", selected.onboarding.id)}</div><div className="mt-5 rounded-2xl border bg-slate-50 p-4"><button onClick={() => setAgreementOpen(!agreementOpen)} className="rounded-xl bg-slate-950 px-4 py-3 text-sm font-black text-white">{agreementOpen ? "Hide Agreement Details" : "Expand Agreement Details"}</button>{agreementOpen && <div className="mt-4 grid gap-3 text-sm font-bold text-slate-700"><p>Agreement acknowledged: {selected.onboarding.agreement_acknowledged ? "Yes" : "No"}</p><p>Acknowledged at: {fmt(selected.onboarding.agreement_acknowledged_at)}</p><p className="rounded-xl bg-white p-4 text-slate-600">Volunteer confirmed the SDTV volunteer agreement during onboarding. This section is intentionally expandable so Studio can verify agreement status without cluttering the main review form.</p></div>}</div><div className="mt-5 flex flex-wrap gap-3"><button onClick={saveProfileChanges} disabled={savingProfile || photoUploading} className="rounded-xl bg-pink-600 px-5 py-3 font-black text-white disabled:opacity-60">Save Profile Changes</button><button onClick={() => approveTeamAccess(selected.request)} disabled={savingProfile || photoUploading} className="rounded-xl bg-green-600 px-5 py-3 font-black text-white disabled:opacity-60">Approve Team Access</button></div></div> : <div className="mt-6 rounded-2xl bg-slate-50 p-5 text-sm font-bold text-slate-600">No onboarding submission found yet for this volunteer request.</div>}</div>}</section></div>}</div></main>;
}
