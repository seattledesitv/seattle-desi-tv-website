...
function label(value?: string | null) {
  ...
  if (value === "awaiting_onboarding") return "Waiting for Onboarding";
...
export default function StudioVolunteersPage() {
  ...
  const submissionByRequestId = useMemo(() => {
    ...
    submissions.forEach((item: any) => {
      if (item.volunteer_request_id) map[item.volunteer_request_id] = item;
    });
    return map;
  }, [submissions]);
  ...
  async function loadVolunteers() {
    ...
    if (requestResult.error) {
      setActionMessage(`Could not load volunteer requests: ${requestResult.error.message}`);
      return;
    }

    const submissionResult = await supabase
      .from("volunteer_onboarding_submissions")
      .select("id,user_id,email,volunteer_request_id,full_name,phone,city,interests,availability,experience,photo_url,agreement_acknowledged,agreement_acknowledged_at,status,created_at")
      .order("created_at", { ascending: false });

    if (submissionResult.error) {
      setActionMessage(`Volunteer requests loaded, but onboarding submissions could not load: ${submissionResult.error.message}`);
    }

    const rows = requestResult.data || [];
    const areaDefaults: Record<string, string> = {};
    rows.forEach((request: any) => {
      areaDefaults[request.id] = teamAreas[request.id] || "General Team";
    });
    setRequests(rows);
  ...
  async function init() {
    ...
    if (!currentUser) {
      setMessage("Please login to access volunteer management.");
      setLoading(false);
      return;
    }
    ...
    if (!isAdminRole(nextRole)) {
      setMessage("This account does not have volunteer management access.");
      setLoading(false);
      return;
    }
  ...
  async function updateStatus(request: any, status: string) {
    ...
    if (error) {
      setActionMessage(`Status update failed: ${error.message}`);
      return;
    }

    if (status === "awaiting_onboarding") {
      await notifyVolunteer(request, "Volunteer onboarding ready", "Your SDTV orientation is complete. Please complete your onboarding form.", "/onboarding");
    }
  ...
  async function saveTeamArea(request: any, teamArea: string) {
    ...
    await supabase.from("volunteer_team_assignments").upsert({
      user_id: request.user_id,
      email: request.email,
      volunteer_request_id: request.id,
      onboarding_submission_id: onboarding?.id || null,
      team_area: teamArea,
      assigned_by: user?.email || user?.id || null,
  ...
  async function approveTeamAccess(request: any) {
    ...
    const upsertResult = await supabase
      .from("admins")
      .upsert({ user_id: request.user_id, email: request.email, role: "team_member" }, { onConflict: "email" });

    if (upsertResult.error) {
      setActionMessage(`Could not assign team member role: ${upsertResult.error.message}`);
      return;
    }
    ...
    const updateResult = await supabase
      .from("user_role_requests")
      .update({ status: "approved", approved_role: "team_member", approved_by: user?.email || user?.id || null, approved_at: new Date().toISOString() })
      .eq("id", request.id);

    if (updateResult.error) {
      setActionMessage(`Role assigned, but request update failed: ${updateResult.error.message}`);
      return;
    }
  ...
  async function uploadAdminProfilePhoto(file?: File) {
    ...
    if (!cloudinaryCloudName || !cloudinaryUploadPreset) {
      setPhotoMessage("Cloudinary is not configured. Paste a public image URL instead.");
      return;
    }
  ...
  async function saveProfileChanges() {
    ...
    if (error) {
      setActionMessage(`Could not save profile changes: ${error.message}`);
      return;
    }
  ...
  function renderActions(request: any, onboarding: any) {
    const status = request.status;
    const showTeamPicker = status === "awaiting_team_role_access" || status === "approved";

    return <div className="flex flex-wrap xl:flex-col gap-2 xl:min-w-60">
      {showTeamPicker && <label className="text-xs font-black uppercase text-gray-500">Team Area
        <select className="mt-1 w-full border rounded-xl px-3 py-2 text-sm font-bold" value={teamAreas[request.id] || "General Team"} onChange={(event) => setTeamAreas({ ...teamAreas, [request.id]: event.target.value })}>
          {TEAM_AREAS.map((area) => <option key={area} value={area}>{area}</option>)}
        </select>
      </label>}

      {status === "awaiting_orientation" && <button onClick={() => updateStatus(request, "awaiting_onboarding")} className="bg-pink-600 text-white px-4 py-2 rounded-xl font-bold text-sm">Complete Orientation</button>}

      {status === "awaiting_onboarding" && <div className="bg-slate-100 text-slate-700 px-4 py-3 rounded-xl font-bold text-sm text-center">Waiting for volunteer onboarding</div>}
      ...
      {status === "approved" && <div className="bg-green-50 text-green-700 px-4 py-3 rounded-xl font-bold text-sm text-center">Approved team member</div>}
      {status === "rejected" && <div className="bg-red-50 text-red-700 px-4 py-3 rounded-xl font-bold text-sm text-center">Rejected</div>}
    </div>;
  }
  ...
  return <main className="min-h-screen bg-slate-950 text-white">
    ...
    <div className="max-w-7xl mx-auto px-6 py-10">
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 mb-8">
        <div>
          <h1 className="text-4xl md:text-5xl font-black">Volunteer Requests</h1>
          <p className="text-slate-300 mt-2">Complete orientation, review onboarding, edit profiles, choose a team area, and approve team access.</p>
      ...
      {!loading && canAccess && <div className="space-y-5">
        ...
        <div className="grid md:grid-cols-4 gap-4">
          {["awaiting_orientation", "awaiting_onboarding", "awaiting_team_role_access", "approved"].map((status) => <div key={status} className="bg-white/10 rounded-2xl p-5">
            <p className="text-slate-300 text-sm font-bold uppercase">{label(status)}</p>
            <p className="text-4xl font-black">{requests.filter((request) => request.status === status).length}</p>
          </div>)}
        </div>
        ...
        <div className="grid gap-4">
          {requests.map((request) => {
            const onboarding = submissionByRequestId[request.id];
            return <article key={request.id} className="bg-white text-slate-950 rounded-2xl p-5 grid xl:grid-cols-[1fr_auto] gap-4">
              <div>
                <div className="flex gap-4 items-start">
                  <div className="w-16 h-16 rounded-2xl bg-slate-100 overflow-hidden grid place-items-center shrink-0">
                    {onboarding?.photo_url ? <img src={onboarding.photo_url} alt={onboarding.full_name || request.email} className="w-full h-full object-cover" /> : <span className="font-black text-pink-600">SDTV</span>}
                  </div>
                  <div>
                    <h2 className="text-xl font-black">{onboarding?.full_name || request.email}</h2>
                    <p className="text-sm text-gray-600">{request.email}</p>
                    <p className="text-sm text-gray-600 mt-1">Status: <b>{label(request.status)}</b></p>
                    {onboarding && <p className="text-sm text-green-700 font-bold mt-1">Onboarding submitted · Agreement: {onboarding.agreement_acknowledged ? "Yes" : "No"}</p>}
                  </div>
                </div>
                {onboarding && <div className="grid md:grid-cols-2 gap-3 mt-4 text-sm">
                  <p><b>Phone:</b> {onboarding.phone || "—"}</p>
                  <p><b>City:</b> {onboarding.city || "—"}</p>
                  <p><b>Interests:</b> {onboarding.interests || "—"}</p>
                  <p><b>Availability:</b> {onboarding.availability || "—"}</p>
                </div>}
              </div>
              {renderActions(request, onboarding)}
            </article>;
          })}
          {requests.length === 0 && <div className="bg-white text-slate-950 rounded-2xl p-8">No volunteer requests found.</div>}
        </div>
      </div>}

      {selected && <div className="fixed inset-0 bg-black/70 z-50 p-4 overflow-y-auto">
        <div className="bg-white text-slate-950 rounded-3xl max-w-4xl mx-auto p-6 md:p-8">
          <div className="flex justify-between gap-4">
            <div>
              <h2 className="text-3xl font-black">Volunteer Profile Review</h2>
              <p className="text-gray-600">{selected.request.email}</p>
            </div>
            <button onClick={() => { setSelected(null); setEditProfile(null); }} className="bg-slate-900 text-white px-4 py-2 rounded-xl font-bold h-fit">Close</button>
          </div>

          {!selected.onboarding && <div className="bg-yellow-50 text-yellow-900 rounded-2xl p-5 mt-5 font-bold">No onboarding submission found yet.</div>}

          {selected.onboarding && editProfile && <div className="grid gap-5 mt-5">
            <div className="flex gap-4 items-center">
              {editProfile.photo_url && <img src={editProfile.photo_url} alt={editProfile.full_name} className="w-24 h-24 rounded-2xl object-cover" />}
              <div>
                <h3 className="text-2xl font-black">{editProfile.full_name || selected.request.email}</h3>
                <p>{editProfile.phone}</p>
                <p>{editProfile.city}</p>
              </div>
            </div>
            ...
            <label className="font-bold">Interests<textarea className="w-full border rounded-xl p-3 mt-1 font-normal min-h-24" value={editProfile.interests} onChange={(e) => setEditProfile({ ...editProfile, interests: e.target.value })} /></label>
            <label className="font-bold">Availability<textarea className="w-full border rounded-xl p-3 mt-1 font-normal min-h-20" value={editProfile.availability} onChange={(e) => setEditProfile({ ...editProfile, availability: e.target.value })} /></label>
            <label className="font-bold">Experience<textarea className="w-full border rounded-xl p-3 mt-1 font-normal min-h-20" value={editProfile.experience} onChange={(e) => setEditProfile({ ...editProfile, experience: e.target.value })} /></label>
            <div className="bg-green-50 text-green-800 rounded-2xl p-4 font-bold">Agreement acknowledged: {selected.onboarding.agreement_acknowledged ? "Yes" : "No"}</div>
