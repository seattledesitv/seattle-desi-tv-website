...
export default function EventOpsMediaRecovery() {
  ...
  async function trackRequest() {
    ...
    setMessage("Organizer media request tracked. Notification was added if the POC has an SDTV login profile.");
    return true;
  }
  ...
  async function sendEmail() {
    ...
    if (!response.ok) setMessage(result?.error || "Email failed. Check Resend configuration.");
      ...
      await trackRequest();
      setMessage("Email sent to POC. Notification was added if the POC has an SDTV login profile.");
    }
    setSending(false);
  }
  ...
  return createPortal(<div className="rounded-2xl bg-red-50 p-4 text-slate-950">
    ...
    <div className="mt-3 flex flex-wrap gap-2">
      ...
      <button onClick={trackRequest} disabled={!eventId} className="rounded-xl bg-slate-900 px-4 py-2 text-sm font-black text-white disabled:opacity-60">Track + Notify</button>
