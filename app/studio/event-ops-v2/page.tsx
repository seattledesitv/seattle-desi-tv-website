...
import { DEFAULT_EVENT_TIMEZONE, formatEventTime, timeInputValue } from "../../lib/eventTime";
 ...
 async function loadData() {
  const [er, ar, wr, ir, up, admins, team, roleRequests, pocResult] =
    await Promise.all([
      supabase
        .from("events")
        .select("id,title,date,local_start_time,local_end_time,event_timezone,location,status,created_by,poc_email,poc_phone,ticket_url,description,image,image_urls,featured,created_at")
  ...
  useEffect(() => { init(); }, []);
  useEffect(() => { if (!selectedEvent) return; setEditForm({ title: selectedEvent.title || "", date: dateInput(selectedEvent.date), local_start_time: timeInputValue(selectedEvent.local_start_time), local_end_time: timeInputValue(selectedEvent.local_end_time), event_timezone: selectedEvent.event_timezone || DEFAULT_EVENT_TIMEZONE, location: selectedEvent.location || "", description: selectedEvent.description || "", poc_email: selectedEvent.poc_email || "", poc_phone: selectedEvent.poc_phone || "", ticket_url: selectedEvent.ticket_url || "", image_urls: eventImages(selectedEvent) }); const p = eventPocs[selectedEvent.id] || {}; const list = getPocs(p); setPocForm({ pocs: list.length ? list : [blankPoc()], notes: p.notes || "" }); }, [selectedEventId, eventPocs]);
  useEffect(() => {
  if (eventWorkflow) {
  ...
  async function saveEventEdits() { const urls = (editForm.image_urls || []).filter(Boolean); const payload = { title: editForm.title || null, date: editForm.date || null, local_start_time: editForm.local_start_time || null, local_end_time: editForm.local_end_time || null, event_timezone: editForm.event_timezone || DEFAULT_EVENT_TIMEZONE, location: editForm.location || null, description: editForm.description || null, poc_email: editForm.poc_email || null, poc_phone: editForm.poc_phone || null, ticket_url: editForm.ticket_url || null, image: urls[0] || null, image_urls: urls.length ? urls : null }; const { error } = await supabase.from("events").update(payload).eq("id", selectedEventId); if (error) setActionMessage(`Event save failed: ${error.message}`); else { setActionMessage("Event details and images saved."); await loadData(); } }
