"use client";
/* eslint-disable @next/next/no-img-element -- Studio previews user-uploaded public media */
    ...
    () =>
      data.releases.filter(
        (release) =>
          filter === "all" ||
          (filter === "open"
            ? ["pending", "changes_requested"].includes(release.status)
      ...
      <section className="mx-auto max-w-7xl px-6 py-10">
        ...
        <section className="mt-8 rounded-3xl bg-white p-6 shadow-sm">
          <div className="flex flex-wrap items-center justify-between gap-4">
            <div>
              ...
              <p className="text-sm text-slate-500">
                {visible.length} release{visible.length === 1 ? "" : "s"} in
                this view
              </p>
            </div>
            <select
          ...
          <div className="mt-6 grid gap-5">
            {visible.map((release) => (
              <article key={release.id} className="rounded-2xl border p-5">
                <div className="flex flex-wrap items-start justify-between gap-3">
                  <div>
                    ...
                    <p className="mt-1 font-bold text-slate-500">
                      {release.organization_name || "Community submission"}
                    </p>
                  </div>
                  {release.image_urls[0] && (
                    <img
                      src={release.image_urls[0]}
                      alt=""
                      className="h-24 w-36 rounded-xl object-cover"
                      style={{
                        objectPosition: `${release.image_position_x ?? 50}% ${release.image_position_y ?? 50}%`,
                      }}
                    />
                  )}
                </div>
                <p className="mt-4 text-slate-600">{release.summary}</p>
                  ...
                  <div className="mt-3 flex flex-wrap gap-2">
                    {release.documents.map((document) => (
                      <a
                        ...
                        target="_blank"
                        rel="noreferrer"
                        className="rounded-lg border px-3 py-2 text-sm font-black text-pink-600"
                      >
                        {document.name}
                      </a>
                    ))}
                ...
                <input
                  value={notes[release.id] ?? release.admin_notes ?? ""}
                  onChange={(e) =>
                    setNotes((current) => ({
                      ...current,
                  ...
                  placeholder="Review notes for submitter"
                ...
                <div className="mt-3 flex flex-wrap gap-2">
                  ...
                  <button
                    ...
                    className="rounded-xl bg-amber-400 px-4 py-2 font-black"
                  >
                    Request Changes
                  </button>
                  <button
