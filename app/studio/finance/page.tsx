...
export default function StudioFinancePage() {
  ...
  async function loadRows() {
    ...
    if (!response.ok) {
      setRows([]);
      setActionMessage(result.error || "Could not load expenses.");
      return;
    }
    setRows(result.rows || []);
  }
  ...
  async function init() {
    ...
    setUser(currentUser);
    if (!currentUser) {
      setMessage("Please login to access Finance Management.");
      setLoading(false);
      return;
    }
    ...
    if (!isSuperAdmin(nextRole)) {
      setMessage(`Super admin access required. Current role: ${nextRole}`);
      setLoading(false);
      return;
    }
    setMessage("");
  ...
  async function submitExpense(event: FormEvent) {
    ...
    if (file && file.size > MAX_FILE_SIZE) {
      setActionMessage("Bill file must be 5 MB or smaller.");
      return;
    }
    setSubmitting(true);
    setActionMessage("Saving finance item...");
    ...
    setSubmitting(false);
    if (!response.ok) {
      setActionMessage(result.error || "Could not save finance item.");
      return;
    }
    setActionMessage(
      editingId
        ? "Finance item updated."
        : isMileage
          ? "Mileage reimbursement submitted."
          : "Expense saved privately.",
    );
    resetEditor();
    await loadRows();
  }
  ...
  function editRow(row: FinanceRow) {
    setEditingId(String(row.id));
    setEditingBillName(String(row.bill_file_name || ""));
    setFile(null);
    setForm({
      expense_type: row.expense_type === "mileage" ? "mileage" : "expense",
      expense_date: String(row.expense_date || "").split("T")[0],
      vendor_name: String(row.vendor_name || ""),
      reimbursed_to: String(row.reimbursed_to || ""),
      category: String(row.category || "other"),
      amount: row.amount == null ? "" : String(row.amount),
      mileage_miles: row.mileage_miles == null ? "" : String(row.mileage_miles),
      mileage_rate:
    ...
    if (input) input.value = "";
    setActionMessage("Editing finance item. Update the fields and save.");
  ...
  async function openReceipt(row: FinanceRow) {
    ...
    const result = await response.json().catch(() => ({}));
    if (!response.ok) {
      setActionMessage(result.error || "Could not open receipt.");
      return;
    }
  ...
  async function updateStatus(row: FinanceRow, reimbursement_status: string) {
    setActionMessage("Updating reimbursement status...");
    const headers = await authHeader();
    const response = await fetch("/api/studio/finance/expenses", {
      method: "PATCH",
      headers: { ...headers, "Content-Type": "application/json" },
      body: JSON.stringify({ id: row.id, reimbursement_status }),
    });
    const result = await response.json().catch(() => ({}));
    if (!response.ok) {
      setActionMessage(result.error || "Could not update status.");
      return;
    }
  ...
  useEffect(() => {
    // Existing page initialization intentionally hydrates authenticated Studio state.
  ...
  const unpaidTotal = visibleRows
    .filter((row) => row.reimbursement_status !== "paid")
      ...
      <section className="mx-auto max-w-7xl px-4 py-8 md:px-6">
        <div className="mb-8 flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
          <div>
            ...
            <p className="mt-2 text-slate-300">
              Track expenses, mileage reimbursements, payment status, and
              private bills in Cloudflare R2.
            </p>
            {user?.email && (
          ...
          <button
            onClick={loadRows}
            disabled={!canAccess}
            className="rounded-xl bg-white px-5 py-3 font-bold text-slate-950 disabled:opacity-40"
          >
            Refresh
          </button>
        </div>
        ...
        {!loading && canAccess && (
          <div className="grid gap-6 lg:grid-cols-[390px_1fr]">
            {actionMessage && (
              <div className="rounded-2xl bg-yellow-100 p-4 font-bold text-yellow-900 lg:col-span-2">
                {actionMessage}
              </div>
            )}
            <section className="rounded-3xl bg-white p-6 text-slate-950">
              <h2 className="text-2xl font-black">
                {editingId ? "Edit Finance Item" : "Add Finance Item"}
              </h2>
              <p className="mt-1 text-sm text-slate-500">
                {editingId
                  ? "Update the record below. Select a new file only when replacing the existing bill."
                  : "Bills are stored privately. Mileage uses miles × rate."}
              </p>
              <form onSubmit={submitExpense} className="mt-5 grid gap-4">
                  ...
                  <button
                    ...
                    onClick={() =>
                      setForm({
                        ...
                        expense_type: "expense",
                        category:
                          form.category === "mileage" ? "event" : form.category,
                      })
                    }
                    className={`rounded-xl px-3 py-2 font-black ${!isMileage ? "bg-pink-600 text-white" : "text-slate-700"}`}
                  >
                    Expense
                  </button>
                  <button
                ...
                <label className="grid gap-1 text-sm font-black">
                  {isMileage ? "Traveler / person" : "Vendor"}
                  <input
                    required
                ...
                {isMileage && (
                  <label className="grid gap-1 text-sm font-black">
                    ...
                    <input
                      ...
                      className="rounded-xl border p-3 font-normal"
                      placeholder="Name/email paid to"
                    />
                  </label>
                )}
                {isMileage ? (
                  <div className="grid grid-cols-2 gap-3">
                    ...
                    <div className="col-span-2 rounded-xl bg-pink-50 p-3 font-black text-pink-700">
                      Calculated reimbursement: {money(mileageAmount)}
                    </div>
                  </div>
                ) : (
                  <div className="grid grid-cols-2 gap-3">
                    ...
                    <label className="grid gap-1 text-sm font-black">
                      ...
                      <select
                        ...
                        className="rounded-xl border p-3 font-normal"
                      >
                        {CATEGORIES.filter((c) => c !== "mileage").map(
                          (category) => (
                    ...
                    >
                      {PAYMENT_METHODS.map((method) => (
                        <option key={method || "blank"} value={method}>
                          {method ? label(method) : "Select method"}
                        </option>
                      ))}
                    </select>
                  </label>
                ...
                <label className="grid gap-1 text-sm font-black">
                  ...
                  <span className="text-xs font-normal text-slate-500">
                    {editingId && editingBillName
                      ? `Current: ${editingBillName}. Leave blank to keep it. `
                      : ""}
                ...
                <label className="grid gap-1 text-sm font-black">
                  ...
                  <textarea
                    ...
                    placeholder={
                      isMileage
                        ? "Trip purpose, route, event, start/end location..."
                        : "Purpose, event, reimbursement notes, etc."
                    }
                  />
                </label>
                <div className="grid gap-2 sm:grid-cols-2">
                  <button
                    disabled={submitting}
                    className="rounded-xl bg-pink-600 px-5 py-4 font-black text-white disabled:opacity-50"
                  >
                    {submitting
                      ? "Saving..."
                      : editingId
                  ...
                  {editingId && (
                    <button
                      ...
                      onClick={resetEditor}
                      disabled={submitting}
                      className="rounded-xl border-2 border-slate-300 px-5 py-4 font-black text-slate-700 disabled:opacity-50"
                    >
                      Cancel Edit
                    </button>
                  )}
                ...
                <div className="flex flex-col gap-3 lg:flex-row lg:items-end lg:justify-between">
                  <div>
                    ...
                    <p className="text-sm text-slate-500">
                      Signed receipt links expire after 10 minutes.
                    </p>
                  </div>
                  <div className="flex flex-col gap-2 md:flex-row">
                    <input
                      ...
                      placeholder="Search vendor, person, notes..."
                      className="rounded-xl border p-3 text-sm md:w-64"
                    />
                    <input
                      type="month"
                      value={monthFilter}
                ...
                {byCategory.length > 0 && (
                  <div className="mt-4 flex flex-wrap gap-2">
                    {byCategory.map((item) => (
                      ...
                      >
                        {label(item.category)} · {money(item.amount)}
                      </span>
                    ))}
                  </div>
                )}
                    ...
                    <tbody>
                      {visibleRows.map((row) => (
                        <tr key={row.id} className="border-b last:border-0">
                          <td className="py-3">{dateText(row.expense_date)}</td>
                          <td>{label(row.expense_type || "expense")}</td>
                          <td className="font-black">
                            {row.vendor_name}
                            {row.reimbursed_to ? (
                              <p className="text-xs font-normal text-slate-500">
                                To: {row.reimbursed_to}
                              </p>
                            ) : null}
                          </td>
                          ...
                          <td>
                            {row.expense_type === "mileage"
                              ? `${Number(row.mileage_miles || 0)} @ ${money(row.mileage_rate)}`
                              : "—"}
                          </td>
                          <td>
                            <select
                              value={row.reimbursement_status || "submitted"}
                              onChange={(e) =>
                                updateStatus(row, e.target.value)
                              }
                              className={`rounded-lg px-2 py-1 text-xs font-black ${statusClass(row.reimbursement_status)}`}
                            >
                              {STATUSES.map((s) => (
                                <option key={s} value={s}>
                                  {label(s)}
                          ...
                          <td>
                            <button
                              type="button"
                              onClick={() => editRow(row)}
                              className="rounded-lg border border-pink-300 px-3 py-2 text-xs font-black text-pink-700"
                            >
                              Edit
                            </button>
                          </td>
