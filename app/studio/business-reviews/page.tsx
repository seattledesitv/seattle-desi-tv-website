...
export default function BusinessReviewModerationPage() {
  ...
  async function load() {
    ...
    const result = await forSite(supabase.from("business_reviews").select("id,business_id,reviewer_name,reviewer_email,rating,comment,status,created_at,moderation_notes,local_businesses(name,address)"), site.id).order("created_at", { ascending: false });
  ...
  async function moderate(review: any, nextStatus: string) {
    ...
    const { error } = await forSite(supabase.from("business_reviews").update({ status: nextStatus, moderation_notes: notes || null, moderated_by: user.id, moderated_at: new Date().toISOString() }), site.id).eq("id", review.id);
