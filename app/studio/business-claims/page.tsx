...
type Claim = { id:string; business_id:string; requester_user_id?:string|null; requester_name:string; requester_email:string; requester_phone?:string|null; relationship?:string|null; verification_details?:string|null; status:string; admin_notes?:string|null; created_at:string; local_businesses?:{name?:string;address?:string}|null };
...
export default function BusinessClaimsPage() {
  ...
  async function load(preserveMessage=false){
    setLoading(true); const auth=await supabase.auth.getUser(); const user=auth.data.user||null; setCurrentUser(user);
  ...
  async function updateClaim(claim:Claim,status:"approved"|"rejected"|"needs_information"|"pending"){
    setWorking(claim.id);setMessage("");const now=new Date().toISOString();const adminNotes=notes[claim.id]?.trim()||claim.admin_notes?.trim()||null;
    ...
    if(status==="needs_information"){
      ...
      const body=[`Hello ${claim.requester_name},`,``,`SDTV needs additional information before approving your claim for ${claim.local_businesses?.name||"this business"}.`,``,adminNotes||"Please provide more verification details.",``,`Return to your claim: ${window.location.origin}/businesses/claim?business=${claim.business_id}`].join("\n");
      window.open(`mailto:${claim.requester_email}?subject=${encodeURIComponent(subject)}&body=${encodeURIComponent(body)}`,"_blank");
      setStatusFilter("needs_information");setMessage("The claim was marked Needs information and an email window was opened for the claimant.");
    } else {setMessage(status==="approved"?`${claim.local_businesses?.name||"Business"} is now owner verified and available in My Businesses.`:`Claim marked ${status.replace("_"," ")}.`);}
    await load(true);
  }
