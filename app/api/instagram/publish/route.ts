...
function parseCollaborators(value: unknown) {
  if (Array.isArray(value))
    return value.map((item) => normalizeHandle(String(item))).filter(Boolean);
...
async function graphRequest(url: URL, method: "GET" | "POST" = "GET") {
  ...
  if (!response.ok) {
    ...
    const details = [
      ...
      .join(" · ");
    throw new Error(
      details || `Instagram API request failed with status ${response.status}.`,
    );
  }
  return result;
}
  ...
  for (let attempt = 1; attempt <= MAX_POLL_ATTEMPTS; attempt += 1) {
    ...
    if (status === "FINISHED" || status === "PUBLISHED") {
      return {
        status,
        attempts: attempt,
        elapsedMs: (attempt - 1) * POLL_INTERVAL_MS,
      };
    }
  ...
  try {
    ...
    if (publicationId) {
      ...
      if (
        ...
        return NextResponse.json(
          { error: "Approve this publication before posting it to Instagram." },
          { status: 409 },
        );
    }
    if (pressReleaseId) {
      const { data: pressRelease, error: pressReleaseError } =
        await sessionClient
          .from("press_releases")
          .select("status")
          ...
          .eq("site_id", site.id)
          .single();
      if (pressReleaseError)
        return NextResponse.json(
          { error: pressReleaseError.message },
          { status: 400 },
        );
      if (String(pressRelease?.status) !== "approved")
        return NextResponse.json(
          {
            error: "Approve this press release before posting it to Instagram.",
          },
          { status: 409 },
        );
    }
    ...
    const collaboratorText = collaborators.length
      ...
      : "";
    const caption =
      `${String(body.caption || "").trim()}${collaboratorText}`.trim();

    if (
      !imageUrls.length ||
      imageUrls.some((url: string) => !/^https:\/\//i.test(url))
    ) {
      return NextResponse.json(
        {
          error:
    ...
    if (!mediaId)
      throw new Error(
        "Instagram completed processing but did not return a published media ID.",
      );
