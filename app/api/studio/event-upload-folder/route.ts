...
export async function POST(request: Request) {
  try {
    ...
    const existing = await db.from("event_video_workflows").select("id,status").eq("event_id", eventId).eq("site_id", site.id).maybeSingle();
    if (existing.error) throw existing.error;
    if (existing.data?.id) {
      const result = await db.from("event_video_workflows").update({ upload_destination_url: uploadDestinationUrl || null, updated_by: user.id, updated_at: new Date().toISOString() }).eq("id", existing.data.id).eq("site_id", site.id);
      if (result.error) throw result.error;
    } else {
      const result = await db.from("event_video_workflows").insert({ site_id: site.id, event_id: eventId, status: "ready_for_editing", upload_destination_url: uploadDestinationUrl || null, created_by: user.id, updated_by: user.id });
      if (result.error) throw result.error;
    }
