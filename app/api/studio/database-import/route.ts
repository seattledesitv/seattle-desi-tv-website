...
const TABLE_PRIMARY_KEYS: Record<string, string[]> = {
  ...
  contact_requests: ["id"],
  crew_availability: ["id"],
  event_coverage_sources: ["id"],
  event_crew_assignments: ["id"],
  event_crew_media_submissions: ["id"],
  event_deliverables: ["id"],
  event_influencer_intents: ["id"],
  event_video_notifications: ["id"],
  event_video_revisions: ["id"],
  event_video_workflows: ["id"],
  events: ["id"],
  featured_social_content: ["id"],
  festival_hero_assets: ["id"],
  hero_analytics: ["id"],
  homepage_hero_banners: ["id"],
  homepage_settings: ["section_key"],
  homepage_sponsors: ["id"],
  homepage_testimonials: ["id"],
  influencer_profiles: ["id"],
  local_businesses: ["id"],
  newsletter_campaigns: ["id"],
  newsletter_settings: ["section_key"],
  newsletter_subscribers: ["id"],
  notifications: ["id"],
  public_content_requests: ["id"],
  public_visibility_controls: ["email"],
  radio_team_members: ["id"],
  social_media_stats: ["platform"],
  sponsors: ["id"],
  team_members: ["id"],
  user_profiles: ["user_id"],
  user_role_requests: ["id"],
  volunteer_onboarding_submissions: ["id"],
};
...
async function importTable(client: LooseSupabaseClient, table: string, rows: any[], mode: ImportMode) {
  ...
  if (mode === "dry_run") {
    return { table, ok: true, requested: rows.length, valid: validRows.length, inserted: 0, would_insert: missingRows.length, skipped: rows.length - missingRows.length, error: null };
  }
  ...
  try {
    ...
    if (!isStagingEnvironment()) {
      return NextResponse.json({
        success: false,
        blocked: true,
        error: "Database import is blocked because this deployment is not marked as staging. Import is only allowed for staging.",
    ...
    return NextResponse.json({
      ...
      failed_table_count: failed.length,
      results,
    });
  } catch (error: any) {
