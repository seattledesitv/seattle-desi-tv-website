...
function uniqueRecipients(recipients: Recipient[]) {
  ...
  return recipients
    .map((item) => ({ ...item, email: cleanEmail(item.email) }))
    .filter((item) => {
...
export async function POST(request: Request) {
  try {
    ...
    const { data: admin } = await service.from("admins").select("role,email").eq("user_id", user.id).maybeSingle();
    const adminRole = String(admin?.role || "").toLowerCase();
    if (!adminRole.includes("admin")) return NextResponse.json({ error: "Studio admin access required." }, { status: 403 });
    ...
    if (resendKey) {
      ...
      const sendResult = await resend.emails.send({
        from: fromEmail,
        to: recipients.map((item) => item.email as string),
        subject,
        text: message,
      });
