...
export async function POST(request: Request) {
  try {
    ...
    if (!postalAddress) {
      return NextResponse.json({ error: "SDTV_POSTAL_ADDRESS must be configured before directory notices can be sent." }, { status: 500 });
    }
    ...
    const { data: business, error: businessError } = await db
      ...
      .select("id,name,address,website,poc_name,poc_email,status,outreach_status,outreach_sent_at,claim_token,opted_out_at")
      .eq("id", businessId)
      .maybeSingle();

    if (businessError || !business) return NextResponse.json({ error: businessError?.message || "Business not found." }, { status: 404 });
    if (business.opted_out_at || business.outreach_status === "opted_out") {
      return NextResponse.json({ error: "This business opted out and cannot be contacted from this workflow." }, { status: 409 });
    }

    const recipient = cleanEmail(body.recipient || business.poc_email);
    if (!recipient || !recipient.includes("@")) {
      return NextResponse.json({ error: "Add a valid business contact email before sending the notice." }, { status: 400 });
    }
    if (business.outreach_sent_at && !forceResend) {
      return NextResponse.json({ error: "A listing notice has already been sent. Use the explicit resend option if another notice is needed." }, { status: 409 });
    }

    const token = business.claim_token;
    if (!token) return NextResponse.json({ error: "Claim token is missing. Apply the business outreach database migration first." }, { status: 500 });

    const responseDue = new Date(Date.now() + 14 * 24 * 60 * 60 * 1000);
    const responseBase = `${siteUrl}/business-response?token=${encodeURIComponent(token)}`;
    ...
    const html = `<div style="font-family:Arial,sans-serif;line-height:1.6;color:#172033;max-width:680px;margin:auto">
      <h2 style="color:#be185d">Seattle Desi TV Community Directory</h2>
      <p>${escapeHtml(greeting)}</p>
      <p>Seattle Desi TV is building a community directory to help residents discover local businesses. We prepared a complimentary draft listing for <strong>${escapeHtml(businessName)}</strong> using publicly available information.</p>
      <div style="background:#fff7ed;border:1px solid #fed7aa;padding:16px;border-radius:12px">
        <strong>Planned publication:</strong> We plan to publish the basic listing after ${escapeHtml(dueText)} unless you ask us not to include it. You can still claim, correct, or request removal at any time after publication.
      </div>
      <p>This listing does not imply sponsorship, partnership, or endorsement.</p>
      <p><a href="${claimUrl}" style="display:inline-block;background:#be185d;color:white;padding:11px 16px;border-radius:8px;text-decoration:none;font-weight:bold">Claim and manage listing</a></p>
      <p><a href="${approveUrl}">Confirm the listing is accurate</a><br/>
      <a href="${correctionUrl}">Request a correction</a><br/>
      <a href="${optOutUrl}">Do not include my business</a></p>
      <p>There is no charge to claim or correct the basic listing.</p>
      <hr style="border:0;border-top:1px solid #ddd;margin:24px 0"/>
      <p style="font-size:12px;color:#667085">Seattle Desi TV · ${escapeHtml(siteUrl)}<br/>${escapeHtml(postalAddress)}</p>
    </div>`;

    const resend = new Resend(resendKey);
    const sendResult = await resend.emails.send({ from: fromEmail, to: recipient, subject, text, html });
    if (sendResult.error) {
      await db.from("local_businesses").update({ outreach_status: "send_failed", outreach_recipient: recipient }).eq("id", businessId);
      await db.from("business_activity_log").insert({
        business_id: businessId,
        activity_type: "listing_notice_failed",
        activity_label: "Listing notice send failed",
        actor_email: user.email || null,
        details: { recipient, error: sendResult.error.message },
      });
    ...
    await db.from("business_activity_log").insert({
      business_id: businessId,
      activity_type: forceResend ? "listing_notice_resent" : "listing_notice_sent",
      activity_label: forceResend ? "Listing notice resent" : "Listing notice sent",
      actor_email: user.email || null,
      details: { recipient, response_due_at: responseDue.toISOString(), message_id: sendResult.data?.id || null },
    });
