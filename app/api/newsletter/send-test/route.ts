...
export async function POST(request: Request) {
  try {
    ...
    const response = await fetch("https://api.resend.com/emails", { method: "POST", headers: { Authorization: `Bearer ${apiKey}`, "Content-Type": "application/json" }, body: JSON.stringify({ from, to, subject: draft.subject || `${site.name} Community Update`, html: renderHtml(draft, site.name) }) });
