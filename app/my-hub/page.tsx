...
type Counts = { events: number; businesses: number; coverage: number; contacts: number; roles: number; assignments: number; availability: number; notifications: number; editing: number; influencer: number; };
const emptyCounts: Counts = { events: 0, businesses: 0, coverage: 0, contacts: 0, roles: 0, assignments: 0, availability: 0, notifications: 0, editing: 0, influencer: 0 };
...
export default function MyHubPage() {
  ...
  async function loadHub() {
    ...
    setRole(nextRole);
    if (!user?.id) { setCounts(emptyCounts); setMessage("Please login to access your personalized hub."); setLoading(false); return; }
  ...
  return <main className="min-h-screen bg-slate-950 text-white"><MyHubHeader /><div className="max-w-7xl mx-auto px-6 py-10"><div className="bg-white/10 border border-white/10 rounded-3xl p-8 md:p-10 mb-8"><p className="text-pink-300 font-black uppercase tracking-wide">Seattle Desi TV Workspace</p><h1 className="text-4xl md:text-6xl font-black mt-3">My Hub</h1><p className="text-slate-300 max-w-3xl mt-3">One place for your SDTV portal, assignments, availability, notifications, submissions, influencer profile, and account tools.</p><div className="flex flex-wrap gap-3 items-center mt-4"><p className="text-slate-400 text-sm">{loading ? "Loading..." : email ? `${email} · ${role}` : message}</p><button onClick={loadHub} className="bg-white text-slate-950 px-4 py-2 rounded-xl font-bold text-sm">Refresh</button></div></div><div className="mb-8"><InstallPWAButton /></div>{!loading && team && <section className="mb-8"><div className="flex flex-col md:flex-row md:items-end md:justify-between gap-3 mb-4"><div><p className="text-pink-300 font-black uppercase tracking-wide">My SDTV Stats</p><h2 className="text-3xl font-black mt-2">Coverage Scoreboard</h2><p className="text-slate-300 mt-1">Your SDTV assignments, availability, and alerts at a glance.</p></div><div className="flex flex-wrap gap-2"><a href="/recognition" className="bg-white text-slate-950 px-4 py-2 rounded-xl font-bold w-fit">View Recognition</a><a href="/my-assignments" className="bg-pink-600 text-white px-4 py-2 rounded-xl font-bold w-fit">Open Assignments</a></div></div><div className="grid md:grid-cols-4 gap-4">{stats.map((stat) => <div key={stat.title} className="bg-white text-slate-950 rounded-2xl p-5"><p className="text-sm text-gray-500 font-black uppercase">{stat.title}</p><p className="text-4xl font-black text-pink-600 mt-2">{stat.value}</p><p className="text-gray-600 text-sm mt-2">{stat.note}</p></div>)}</div></section>}{!loading && videoEditor && <section className="mb-8 bg-white text-slate-950 rounded-3xl p-6"><div className="flex flex-col md:flex-row md:items-center md:justify-between gap-3"><div><p className="text-pink-600 font-black uppercase tracking-wide">Editor Workspace</p><h2 className="text-3xl font-black mt-1">My Editing Queue</h2><p className="text-gray-600 mt-1">Open videos assigned to you, submit drafts, review feedback, and mark publishing complete.</p></div><a href="/my-editing" className="bg-slate-950 text-white px-5 py-3 rounded-xl font-black w-fit">Open Editing Queue</a></div></section>}<div className="grid md:grid-cols-2 xl:grid-cols-3 gap-6">{cards.filter((card) => card.show).map((card) => <a key={card.href + card.title} href={card.href} className="bg-white text-slate-950 rounded-3xl p-6 shadow-xl border hover:scale-[1.01] transition block"><div className="flex items-start justify-between gap-4"><div className="min-w-0"><h2 className="text-2xl font-black">{card.title}</h2><p className="text-gray-600 mt-2">{card.note}</p></div><span className="bg-pink-50 text-pink-600 rounded-full px-4 py-1 text-sm font-black whitespace-nowrap shrink-0 text-center min-w-10">{card.value}</span></div></a>)}</div>{!loading && !email && <div className="bg-white text-slate-950 rounded-3xl p-8 mt-8"><h2 className="text-2xl font-black">Login required</h2><p className="text-gray-600 mt-2">Login to see your submissions, requests, assignments, influencer profile, and notifications.</p><a href="/login" className="inline-block bg-pink-600 text-white px-5 py-3 rounded-xl font-bold mt-5">Go to Login</a></div>}</div><SiteFooter /></main>;
}
