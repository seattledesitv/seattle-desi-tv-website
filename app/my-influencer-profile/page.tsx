...
function Field({
  ...
  return (
    <label className="block text-sm font-black text-slate-900">
      ...
      {children}
      {help && (
        <p className="mt-1 text-xs font-normal text-slate-500">{help}</p>
      )}
    </label>
  );
}
...
export default function MyInfluencerProfilePage() {
  ...
  async function load() {
    ...
    if (profile) {
      ...
      setStatus(
        "Create your influencer profile. Admin approval is required before it appears publicly.",
      );
    }
  }
  ...
  async function save() {
    ...
    const payload = {
      ...
      status:
        existingId && approvalStatus === "approved" ? "approved" : "pending",
    ...
    if (error) {
      ...
      setStatus(
        "Influencer profile saved. Admin approval is required before public listing.",
      );
    }
    setSaving(false);
  }
      ...
      <div className="mx-auto max-w-5xl px-6 py-10">
        <div className="mb-8 rounded-3xl border border-white/10 bg-white/10 p-8">
          ...
          <p className="mt-3 max-w-3xl text-slate-300">
            Create your influencer badge and opt in to the public {site.city}{" "}
            influencer directory. This can be used by general public members or{" "}
            {site.shortName} team members.
          </p>
        ...
        {!user ? (
          <div className="rounded-3xl bg-white p-8 text-slate-950">
            ...
            <p className="mt-2 text-slate-600">
              Login to create your influencer profile.
            </p>
            <a
              href="/login"
              className="mt-5 inline-block rounded-xl bg-pink-600 px-5 py-3 font-black text-white"
            >
              Login
            </a>
          </div>
            ...
            <section className="rounded-3xl bg-white p-6 text-slate-950 shadow-xl">
              <div className="grid gap-4 md:grid-cols-2">
                ...
                <Field label="Niche">
                  <input
                    className="mt-1 w-full rounded-xl border p-3"
                    placeholder="Food, Events, Fashion, Community..."
                    value={form.niche}
                    onChange={(e) =>
              ...
              <label className="mt-5 flex items-start gap-3 rounded-2xl border bg-slate-50 p-4 text-sm font-bold">
                <input
                  type="checkbox"
                  className="mt-1"
                  checked={form.public_listing}
                  onChange={(e) =>
                    setForm({ ...form, public_listing: e.target.checked })
                  }
                />
                <span>
                  Show me on the public “Influencers in {site.city}” page after
                  admin approval.
                </span>
              </label>
              <button
                ...
                <div className="mx-auto h-28 w-28 overflow-hidden rounded-full bg-pink-50">
                  {form.photo_url ? (
                    <SafeImage
                      ...
                      fallbackClassName="grid h-full w-full place-items-center text-2xl font-black text-pink-600"
                      fallbackLabel={form.full_name.charAt(0) || "I"}
                      widthHint={420}
                    />
                  ) : (
                    <div className="grid h-full w-full place-items-center text-2xl font-black text-pink-600">
                      {form.full_name.charAt(0) || "I"}
                    </div>
                  )}
                </div>
                <h3 className="mt-4 text-xl font-black">
                  {form.full_name || "Influencer Name"}
                </h3>
                <p className="text-sm text-slate-500">
                  {form.city || site.city}
                </p>
                <p className="mt-3 text-sm text-slate-600">
                  {form.bio || "Short bio appears here."}
                </p>
              </div>
            </aside>
          </div>
