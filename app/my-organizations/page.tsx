...
export default function MyOrganizationsPage() {
  ...
  async function loadRows() {
    ...
    const columns = "id,name,organization_type,category,location,website,description,contact_name,contact_email,contact_phone,image,image_position_x,image_position_y,image_zoom,image_display_mode,status,submitted_by,manager_verified_at";
    ...
    setMessage(managed.error ? `Submitted organizations loaded, but manager access could not be loaded: ${managed.error.message}` : "Submitted and managed organization profiles.");
  }
  ...
  function startEdit(row: OrganizationRow) {
    setSelectedId(row.id); setEditing(true); setTab("general"); setFile(null);
  ...
  async function saveMedia(row: OrganizationRow) {
    ...
    try {
      ...
      setFile(null); setMessage("Organization image presentation saved."); await loadRows();
  ...
  return <main className="min-h-screen bg-slate-950 text-white"><MyHubHeader/><section className="mx-auto max-w-7xl px-6 py-10">
    <div className="mb-8 flex flex-col gap-4 md:flex-row md:items-end md:justify-between"><div><p className="font-black uppercase tracking-wide text-pink-300">My Hub</p><h1 className="mt-2 text-4xl font-black md:text-5xl">My Organizations</h1><p className="mt-2 text-slate-300">{message}</p></div><a href="/community-organizations" className="rounded-xl bg-pink-600 px-5 py-3 text-center font-black">Organization Directory</a></div>
