...
function cleanRole(role: string) {
  return String(role || "general_public")
    .toLowerCase()
    .trim();
}
...
function isAdminRole(role: string) {
  return cleanRole(role).includes("admin");
}
function getImages(event: any) {
  const urls = Array.isArray(event?.image_urls) ? event.image_urls.filter(Boolean) : [];
  return urls.length ? (Array.from(new Set(urls)) as string[]) : event?.image ? [event.image] : [];
}
...
function deriveTags(event: any) {
  ...
  const rules = [
    ...
    ["Community", ["community", "cultural", "association"]],
    ["Family", ["family", "all ages"]],
    ["Kids", ["kids", "children", "youth"]],
    ["Food", ["food", "dinner", "cuisine"]],
    ["Charity", ["charity", "fundraiser", "nonprofit"]],
    ["Sports", ["sports", "cricket", "soccer"]],
    ["Business", ["business", "networking", "conference"]],
    ["Festival", ["festival", "celebration", "mela"]],
  ] as const;
...
export default function EventDetailPage() {
  ...
  function previousDocument() {
    selectDocument(selectedDocument - 1);
  }
  ...
  async function loadRole(currentUser: any) {
    ...
    if (adminResult.data?.role) {
      setRole(cleanRole(adminResult.data.role));
      return;
    }
  ...
  async function loadOrganizations(id: string) {
    const { data } = await supabase.from("event_organizations").select("id,organization_id,relationship,display_order,is_primary,community_organizations(id,name,organization_type,category,location,website,description)").eq("event_id", id).order("is_primary", { ascending: false }).order("display_order", { ascending: true });
  ...
  async function requestCoverage() {
    ...
    setSaving(true);
    setActionMessage("Submitting coverage request...");
    ...
    if (error) setActionMessage(`Could not request coverage: ${formatError(error)}`);
      ...
      setActionMessage("Coverage request submitted.");
  ...
  async function requestToCover() {
    ...
    if (error) setActionMessage(`Could not request to cover: ${formatError(error)}`);
      ...
      setActionMessage("Request to cover submitted.");
      await loadRequests(user);
    }
    setSaving(false);
  }
    ...
    try {
      if (navigator.share)
        await navigator.share({
          title: event.title,
          text: `Check out ${event.title} on Seattle Desi TV`,
          url: eventUrl,
        });
  ...
  async function copyLink() {
    try {
      ...
      setShareMessage("Copy the page URL from your browser.");
    }
  }
  function downloadIcs() {
    ...
    link.href = url;
    link.download = `${String(event.title || "event")
      .replace(/[^a-z0-9]+/gi, "-")
      .toLowerCase()}.ics`;
    link.click();
    ...
    <main className="min-h-screen bg-slate-50 text-slate-950">
      ...
      {loading ? (
        <section className="mx-auto max-w-5xl px-6 py-16">
          <div className="rounded-2xl border bg-white p-8">{message}</div>
        </section>
      ) : !event ? (
        <section className="mx-auto max-w-5xl px-6 py-16">
          <div className="rounded-2xl border bg-white p-8">{message}</div>
        ...
        <>
          <section className="bg-gradient-to-br from-slate-950 via-slate-900 to-pink-950 text-white">
            <div className="mx-auto max-w-7xl px-6 py-12 md:px-10 md:py-16">
              <a href="/events" className="font-bold text-pink-300">
                ← Back to Events
              </a>
              ...
              <div className="mt-5 flex flex-wrap gap-2">
                {tags.map((tag) => (
                  <span key={tag} className="rounded-full border border-white/20 bg-white/10 px-3 py-1 text-sm font-bold">
                    {tag}
                  </span>
                ))}
              </div>
                  ...
                  <div className="overflow-hidden rounded-3xl border bg-white shadow-sm">
                    <div className="border-b p-5 md:p-6">
                      <div className="flex flex-wrap items-start justify-between gap-4">
                        <div>
                          ...
                          <p className="mt-1 text-sm text-slate-500">View every flyer in full, zoom in, download, or open full screen.</p>
                        </div>
                        <span className="rounded-full bg-slate-100 px-3 py-1 text-sm font-black text-slate-600">
                          {selectedDocument + 1} / {images.length}
                        </span>
                      </div>
                      <div className="mt-4 flex flex-wrap gap-2">
                        <button onClick={() => setViewerZoom((value) => clampZoom(value - 0.25))} className="rounded-xl border px-3 py-2 text-sm font-black">
                          − Zoom
                        </button>
                        <button onClick={() => setViewerZoom(1)} className="rounded-xl border px-3 py-2 text-sm font-black">
                          Reset
                        </button>
                        <button onClick={() => setViewerZoom((value) => clampZoom(value + 0.25))} className="rounded-xl border px-3 py-2 text-sm font-black">
                          + Zoom
                        ...
                        <a href={activeImage} download target="_blank" rel="noreferrer" className="rounded-xl border border-pink-300 px-4 py-2 text-sm font-black text-pink-700">
                          Download / Open
                        </a>
                      </div>
                    </div>
                    <div className="relative overflow-auto bg-slate-100 p-3 md:p-6" style={{ minHeight: "520px", maxHeight: "1100px" }}>
                      <button
                        ...
                        className="mx-auto block min-w-full cursor-zoom-in"
                      >
                        <img
                          src={activeImage}
                          alt={`${event.title} flyer ${selectedDocument + 1}`}
                          className="mx-auto max-h-[1000px] max-w-full object-contain transition-transform"
                          style={{
                    ...
                    <div className="border-t p-5 md:p-6">
                      <div className="flex items-center justify-between gap-3">
                        <button onClick={previousDocument} disabled={images.length < 2} className="rounded-xl border px-4 py-2 text-sm font-black disabled:opacity-40">
                          ← Previous
                        </button>
                        <p className="text-center text-sm font-bold text-slate-500">
                          Flyer {selectedDocument + 1} of {images.length}
                        </p>
                        <button onClick={nextDocument} disabled={images.length < 2} className="rounded-xl border px-4 py-2 text-sm font-black disabled:opacity-40">
                          Next →
                        </button>
                      </div>
                      <div className="mt-5 grid gap-4 rounded-2xl bg-slate-50 p-4 md:grid-cols-2">
                        <div>
                          <p className="text-xs font-black uppercase tracking-wide text-slate-400">Share</p>
                          <div className="mt-3 flex flex-wrap gap-2">
                            <a href={`https://wa.me/?text=${encodeURIComponent(`${event.title} ${eventUrl}`)}`} target="_blank" rel="noreferrer" className="rounded-xl bg-emerald-600 px-3 py-2 text-sm font-black text-white">
                              WhatsApp
                            </a>
                            <a href={`https://www.facebook.com/sharer/sharer.php?u=${encodeURIComponent(eventUrl)}`} target="_blank" rel="noreferrer" className="rounded-xl bg-blue-700 px-3 py-2 text-sm font-black text-white">
                              Facebook
                            </a>
                            <button onClick={copyLink} className="rounded-xl border bg-white px-3 py-2 text-sm font-black">
                              Copy Link
                            </button>
                          </div>
                          {shareMessage && <p className="mt-2 text-sm font-bold text-emerald-700">{shareMessage}</p>}
                        </div>
                        <div>
                          <p className="text-xs font-black uppercase tracking-wide text-slate-400">Add to Calendar</p>
                          <div className="mt-3 flex flex-wrap gap-2">
                            <a href={googleCalendarUrl} target="_blank" rel="noreferrer" className="rounded-xl border bg-white px-3 py-2 text-sm font-black">
                              Google
                            </a>
                            <a href={outlookCalendarUrl} target="_blank" rel="noreferrer" className="rounded-xl border bg-white px-3 py-2 text-sm font-black">
                              Outlook
                            </a>
                            <button onClick={downloadIcs} className="rounded-xl border bg-white px-3 py-2 text-sm font-black">
                              Apple / .ics
                            </button>
                          </div>
                        </div>
                      ...
                      {images.length > 1 && (
                        <div className="mt-5 flex gap-3 overflow-x-auto pb-2">
                          {images.map((image, index) => (
                            <button key={`${image}-${index}`} onClick={() => selectDocument(index)} className={`min-w-[112px] rounded-xl border-2 bg-slate-100 p-1 ${index === selectedDocument ? "border-pink-600" : "border-transparent"}`}>
                              <img src={image} alt={`${event.title} flyer thumbnail ${index + 1}`} className="h-32 w-full rounded-lg object-contain" />
                              <span className="block py-1 text-xs font-black text-slate-600">Flyer {index + 1}</span>
                            </button>
                          ))}
                        </div>
                      )}
              ...
              <aside className="space-y-5 lg:sticky lg:top-24">
                <div className="overflow-hidden rounded-3xl border bg-white shadow-sm">
                  <div className="border-b p-6">
                    <p className="text-xs font-black uppercase tracking-[0.2em] text-pink-600">Event Information</p>
                    <h2 className="mt-2 text-2xl font-black">Event Details</h2>
                  </div>
                  <div className="space-y-4 p-6 text-slate-700">
                    ...
                    <div>
                      ...
                      <p className="mt-1 font-bold">{formatEventTime(event.local_start_time, event.local_end_time, event.event_timezone)}</p>
                    </div>
                    <div>
                    ...
                    <div>
                      ...
                      <p className="mt-1 font-bold">{countdown}</p>
                    </div>
                  </div>
                </div>

                <div className="rounded-3xl border border-pink-100 bg-pink-50 p-6 shadow-sm">
                  <h2 className="text-xl font-black">Are you responsible for this event?</h2>
                  <p className="mt-2 text-sm text-slate-600">Claim organizer access, suggest a correction, or request removal.</p>
                  <a href={`/manage-listing?type=event&id=${event.id}&name=${encodeURIComponent(event.title)}`} className="mt-4 inline-flex rounded-xl bg-pink-600 px-4 py-3 text-sm font-black text-white">
                    Manage this event listing
                  </a>
                </div>

                {eventOrganizations.length > 0 && (
                  <div className="rounded-3xl border bg-white p-6 shadow-sm">
                    <p className="text-xs font-black uppercase tracking-[0.2em] text-pink-600">Community Connections</p>
                    <h2 className="mt-2 text-xl font-black">Organized By & Partners</h2>
                    <div className="mt-4 space-y-3">
                      {eventOrganizations.map((link) => {
                        const organization = link.community_organizations;
                        return (
                          <a key={link.id} href={`/community-organizations/${link.organization_id}`} className="block rounded-2xl border bg-slate-50 p-4 transition hover:border-pink-300 hover:bg-pink-50">
                            <div className="flex items-start justify-between gap-3">
                              <div>
                                <p className="text-xs font-black uppercase tracking-wide text-pink-600">{link.relationship}</p>
                                <h3 className="mt-1 font-black">{organization?.name || "Community Organization"}</h3>
                                <p className="mt-1 text-xs text-slate-500">{organization?.organization_type || organization?.category || "Community organization"}</p>
                              </div>
                              {link.is_primary && <span className="rounded-full bg-pink-100 px-2 py-1 text-[10px] font-black uppercase text-pink-700">Primary</span>}
                            </div>
                          </a>
                        );
                      })}
                ...
                <div className="overflow-hidden rounded-3xl border bg-white shadow-sm">
                  <div className="p-6">
                    ...
                    <a href={`https://www.google.com/maps?q=${encodeURIComponent(event.location || "")}`} target="_blank" rel="noreferrer" className="mt-4 inline-flex rounded-xl bg-slate-950 px-4 py-2 text-sm font-black text-white">
                      Open Directions
                    </a>
                  </div>
                  <iframe title={`Map for ${event.title}`} src={`https://www.google.com/maps?q=${encodeURIComponent(event.location || "")}&output=embed`} className="h-64 w-full border-0" loading="lazy" />
                </div>
                ...
                {isOwner && (
                  <div className="rounded-3xl border bg-white p-6 shadow-sm">
                    <h2 className="text-xl font-black">Organizer Coverage</h2>
                    <p className="mt-3 text-gray-600">{coverageLabel(coverageRequest?.status)}</p>
                    {!coverageRequest && (
                      <button onClick={requestCoverage} disabled={saving} className="mt-4 w-full rounded-xl bg-slate-950 px-4 py-3 font-black text-white">
                        Request SDTV Coverage
                      </button>
                    )}
                  </div>
                )}
                {canRequestCrew && (
                  <div className="rounded-3xl border bg-white p-6 shadow-sm">
                    <h2 className="text-xl font-black">SDTV Team</h2>
                    <button onClick={requestToCover} disabled={saving} className="mt-4 w-full rounded-xl border border-pink-600 px-4 py-3 font-black text-pink-600">
                      Request to Cover
                    </button>
                  </div>
                )}
                {canAdmin && (
                  <div className="rounded-3xl border bg-white p-6 shadow-sm">
                    <h2 className="text-xl font-black">Admin View</h2>
                    <p className="mt-3 text-gray-600">Crew/Coverage records: {crewRequests.length}</p>
                    <a href={`/studio/events/${event.id}`} className="mt-4 block rounded-xl bg-pink-600 px-4 py-3 text-center font-black text-white">
                      Manage Event
                    </a>
                  </div>
                )}
              ...
              <div className="mt-10 rounded-3xl border bg-white p-6 shadow-sm">
                <div className="flex items-end justify-between gap-3">
                  <div>
                    ...
                    <p className="mt-1 text-sm text-gray-500">More upcoming events from Seattle Desi TV.</p>
                  </div>
                  <a href="/events" className="text-sm font-black text-pink-600">
                    View all
                  </a>
                </div>
      ...
      {lightboxOpen && activeImage && (
        <div
          ...
          onTouchStart={(touchEvent) => setTouchStartX(touchEvent.touches[0]?.clientX ?? null)}
          onTouchEnd={(touchEvent) => {
            if (touchStartX === null) return;
            const endX = touchEvent.changedTouches[0]?.clientX ?? touchStartX;
            const distance = endX - touchStartX;
            if (Math.abs(distance) > 60 && images.length > 1) distance > 0 ? previousDocument() : nextDocument();
            setTouchStartX(null);
          }}
        >
          <div className="flex flex-wrap items-center justify-between gap-3 border-b border-white/15 bg-black/70 px-4 py-3">
            ...
            <div className="flex gap-2">
              <button onClick={() => setLightboxZoom((value) => clampZoom(value - 0.25))} className="rounded-lg bg-white/10 px-3 py-2 font-black">
                −
              </button>
              <button onClick={() => setLightboxZoom(1)} className="rounded-lg bg-white/10 px-3 py-2 font-black">
                Reset
              </button>
              <button onClick={() => setLightboxZoom((value) => clampZoom(value + 0.25))} className="rounded-lg bg-white/10 px-3 py-2 font-black">
                +
              </button>
              <button onClick={() => setLightboxOpen(false)} className="rounded-lg bg-white px-4 py-2 font-black text-slate-950">
                Close
              </button>
            </div>
          </div>
          <div className="relative flex-1 overflow-auto p-4 md:p-8">
            {images.length > 1 && (
              <button onClick={previousDocument} className="fixed left-3 top-1/2 z-10 rounded-full bg-white/90 px-4 py-3 text-2xl font-black text-slate-950">
                ‹
              </button>
            )}
            <div className="flex min-h-full items-center justify-center">
              <img src={activeImage} alt={`${event?.title || "Event"} flyer`} className="max-h-[calc(100vh-120px)] max-w-[96vw] object-contain" style={{ transform: `scale(${lightboxZoom})` }} />
            </div>
            {images.length > 1 && (
              <button onClick={nextDocument} className="fixed right-3 top-1/2 z-10 rounded-full bg-white/90 px-4 py-3 text-2xl font-black text-slate-950">
                ›
              </button>
            )}
          </div>
