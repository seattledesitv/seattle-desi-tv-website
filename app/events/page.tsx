...
function firstImage(row: EventRow) { if (Array.isArray(row.image_urls) && row.image_urls.length > 0) return row.image_urls[0]; return row.image || ""; }
...
async function uploadImage(file: File) {
  const validation = validateOptionalImageFile(file, "Event image", 5);
...
export default function EventsPage() {
  ...
  async function loadEvents() { const { data, error } = await supabase.from("events").select("id,title,date,location,description,image,image_urls,ticket_url,created_by").eq("status", "approved").order("date", { ascending: true }); if (error) { setEvents([]); setMessage(`Could not load events: ${error.message}`); return; } setEvents(data || []); setMessage((data || []).length ? `Showing ${(data || []).length} approved event(s).` : "No approved events found."); }
  ...
  function validateEventForm() {
    ...
    const dateError = !eventDate ? "Enter a valid event date." : eventDate < today ? "Event date cannot be in the past." : null;
    return firstError(
      requireText(form.title, "Event title", 3),
      { ok: !dateError, message: dateError || undefined },
      requireText(form.location, "Location", 3),
      validateOptionalUrl(form.ticket_url, "Ticket / registration URL"),
      validateOptionalEmail(form.poc_email, "POC email"),
      validateOptionalPhone(form.poc_phone, "POC phone"),
      validateOptionalImageFile(imageFiles[0], "Event image", 5)
    );
  }
  ...
  function EventCard({ event }: { event: EventRow }) { const image = firstImage(event); const isOwner = Boolean(user?.id && event.created_by === user.id); const coverageRequest = coverageByEvent[event.id]; return (<article className="border rounded-2xl overflow-hidden shadow-sm bg-white">{image ? <img src={image} alt={event.title} className="w-full h-56 object-cover bg-gray-100" /> : <div className="w-full h-56 bg-pink-50 grid place-items-center text-pink-600 font-black">Seattle Desi TV</div>}<div className="p-5"><h2 className="text-xl font-black">{event.title}</h2><p className="text-gray-500 mt-1">{formatDate(event.date)} · {event.location}</p>{event.description && <p className="text-sm text-gray-600 mt-3 whitespace-pre-line">{event.description}</p>}{isOwner && <div className="mt-4 rounded-xl border bg-slate-50 p-3 text-sm"><p className="font-black text-slate-900">Private Organizer View</p><p className="text-slate-600">{coverageLabel(coverageRequest?.status)}</p></div>}<div className="flex flex-wrap gap-3 mt-5"><a href={`/events/${event.id}`} className="bg-slate-900 text-white px-4 py-2 rounded-lg font-bold text-sm">Details</a>{event.ticket_url && <CheckedExternalLink href={event.ticket_url} notFoundMessage="Page not found. This ticket/register link is not available." className="bg-pink-600 text-white px-4 py-2 rounded-lg font-bold text-sm disabled:opacity-60">Tickets / Register</CheckedExternalLink>}{event.location && <a href={`https://www.google.com/maps?q=${encodeURIComponent(event.location)}`} target="_blank" rel="noreferrer" className="border px-4 py-2 rounded-lg font-bold text-sm">Map</a>}{isOwner && !coverageRequest && <button type="button" onClick={() => requestOwnerCoverage(event)} disabled={requestingCoverageEventId === event.id} className="bg-slate-900 text-white px-4 py-2 rounded-lg font-bold text-sm disabled:opacity-60">{requestingCoverageEventId === event.id ? "Requesting..." : "Request SDTV Coverage"}</button>}{canRequestCrew && <button type="button" onClick={() => requestCrew(event)} disabled={requestingCrewEventId === event.id} className="border border-pink-600 text-pink-600 px-4 py-2 rounded-lg font-bold text-sm disabled:opacity-60">{requestingCrewEventId === event.id ? "Requesting..." : "Request to Cover"}</button>}</div></div></article>); }

  return (
    <main className="min-h-screen bg-slate-50 text-[#081024]"><SiteHeader /><section className="bg-slate-950 text-white px-6 md:px-14 py-12"><div className="max-w-7xl mx-auto"><p className="text-pink-300 font-black uppercase tracking-wide">Seattle Desi TV Events</p><h1 className="text-4xl md:text-6xl font-black mt-3">Community Events Calendar</h1><p className="text-slate-300 mt-3 max-w-3xl">Browse approved community events, submit your event, or request SDTV coverage after approval.</p></div></section><section className="px-6 md:px-14 py-10"><div className="max-w-7xl mx-auto"><div className="flex flex-col md:flex-row md:items-end md:justify-between gap-4 mb-8"><p className="text-gray-500">{message}</p><button type="button" onClick={loadEvents} className="border border-pink-600 text-pink-600 px-5 py-3 rounded-xl font-bold bg-white">Refresh Events</button></div><section className="grid lg:grid-cols-[420px_1fr] gap-8 items-start"><aside className="border rounded-2xl p-6 shadow-sm bg-white">{!authChecked ? <p className="text-gray-500">Checking login...</p> : user ? <div><div className="bg-green-50 border border-green-200 text-green-800 rounded-xl p-3 mb-5 text-sm">Logged in as <b>{user.email}</b><br />Role: <b>{userRole}</b><button type="button" onClick={signOut} className="block mt-2 text-red-600 font-bold">Logout</button></div><h2 className="text-2xl font-black mb-4">Add New Event</h2><input className="w-full border rounded-lg p-3 mb-3" placeholder="Event title" value={form.title} onChange={(event) => setForm({ ...form, title: event.target.value })} /><input className="w-full border rounded-lg p-3 mb-3" type="date" value={form.date} onChange={(event) => setForm({ ...form, date: event.target.value })} /><input className="w-full border rounded-lg p-3 mb-3" placeholder="Location" value={form.location} onChange={(event) => setForm({ ...form, location: event.target.value })} /><textarea className="w-full border rounded-lg p-3 mb-3 min-h-28" placeholder="Event description" value={form.description} onChange={(event) => setForm({ ...form, description: event.target.value })} /><input className="w-full border rounded-lg p-3 mb-3" placeholder="Ticket / registration URL" value={form.ticket_url} onChange={(event) => setForm({ ...form, ticket_url: event.target.value })} /><input className="w-full border rounded-lg p-3 mb-3" placeholder="POC email" value={form.poc_email} onChange={(event) => setForm({ ...form, poc_email: event.target.value })} /><input className="w-full border rounded-lg p-3 mb-3" placeholder="POC phone" value={form.poc_phone} onChange={(event) => setForm({ ...form, poc_phone: event.target.value })} /><input className="w-full border rounded-lg p-3 mb-3" type="file" accept="image/*" onChange={(event) => setImageFiles(Array.from(event.target.files || []))} /><p className="text-xs text-gray-500 mb-3">Image must be JPG/PNG/WebP/GIF and 5MB or smaller. URLs must be valid web links.</p>{submitMessage && <p className="text-sm text-orange-600 mb-3 whitespace-pre-line">{submitMessage}</p>}<button type="button" onClick={submitEvent} disabled={saving} className="bg-pink-600 text-white px-5 py-3 rounded-xl font-bold w-full disabled:opacity-60">{saving ? "Saving Event..." : "Submit Event for Approval"}</button></div> : <div><h2 className="text-2xl font-black mb-3">Login to Add Event</h2><p className="text-sm text-gray-500 mb-4">Public users can submit events. Approved SDTV team members can request to cover events.</p><a href="/login" className="block text-center bg-pink-600 text-white px-5 py-3 rounded-xl font-bold w-full">Login to continue</a></div>}</aside><section>{crewMessage && <div className="border border-pink-200 bg-pink-50 text-pink-800 rounded-2xl p-4 mb-5 font-bold">{crewMessage}</div>}<div className="bg-white border rounded-2xl p-5 mb-6"><div className="grid md:grid-cols-[1fr_1fr_auto] gap-3"><select className="border rounded-lg p-3" value={monthFilter} onChange={(event) => setMonthFilter(event.target.value)}>{MONTHS.map((month, index) => <option key={month} value={String(index)}>{month}</option>)}</select><select className="border rounded-lg p-3" value={yearFilter} onChange={(event) => setYearFilter(event.target.value)}>{Array.from(new Set([new Date().getFullYear(), ...years])).sort((a, b) => a - b).map((year) => <option key={year} value={String(year)}>{year}</option>)}</select><button className="bg-slate-900 text-white rounded-lg px-4 py-3 font-bold" onClick={() => setViewMode(viewMode === "cards" ? "calendar" : "cards")}>{viewMode === "cards" ? "Calendar View" : "Card View"}</button></div><p className="text-sm text-gray-500 mt-3">Showing {filteredEvents.length} event(s) for {MONTHS[Number(monthFilter)]} {yearFilter}</p></div>{viewMode === "calendar" ? <div className="bg-white border rounded-2xl p-5"><div className="grid grid-cols-7 gap-2 text-center text-xs font-black text-gray-500 mb-2">{["Sun", "Mon", "Tue", "Wed", "Thu", "Fri", "Sat"].map((day) => <div key={day}>{day}</div>)}</div><div className="grid grid-cols-7 gap-2">{calendarCells.map((cell, index) => <div key={index} className="min-h-28 border rounded-xl p-2 bg-gray-50 text-left">{cell.day && <p className="font-black text-sm mb-2">{cell.day}</p>}{cell.events.map((event) => <a href={`/events/${event.id}`} key={event.id} className="block text-xs bg-pink-50 text-pink-700 rounded-lg p-2 mb-1 font-bold">{event.title}</a>)}</div>)}</div></div> : <div>{filteredEvents.length === 0 ? <div className="border rounded-2xl p-8 text-gray-500 bg-white">No events found for this month.</div> : <div className="grid md:grid-cols-2 xl:grid-cols-3 gap-6">{filteredEvents.map((event) => <EventCard key={event.id} event={event} />)}</div>}</div>}</section></section></div></section><SiteFooter /></main>
  );
}
