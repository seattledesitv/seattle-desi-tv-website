...
export default function EventsPage() {
  ...
  async function submitEvent() {
    ...
    try {
      ...
      setForm({ title: "", date: "", location: "", description: "", ticket_url: "", poc_name: "", poc_email: "", poc_phone: "" }); setImageFiles([]); setShowSubmitForm(false); setSubmitMessage("Event submitted successfully. It will appear after admin approval."); await loadEvents();
  ...
  useEffect(() => {
    async function init() { const { data } = await supabase.auth.getUser(); const currentUser = data?.user || null; setUser(currentUser); await loadUserContext(currentUser); await loadCoverageRequests(currentUser); setAuthChecked(true); await loadEvents(); }
    init();
    const { data: listener } = supabase.auth.onAuthStateChange(async (_event, session) => { const nextUser = session?.user || null; setUser(nextUser); await loadUserContext(nextUser); await loadCoverageRequests(nextUser); });
