...
function Field({ label, required, help, children }: FieldProps) {
  return (
    <label className="block text-sm font-bold text-slate-900">
      ...
      {children}
      {help && <p className="text-xs text-gray-500 mt-1 font-normal">{help}</p>}
    </label>
  );
}
function eventImages(row: EventRow) {
...
function safeExternalUrl(value?: string | null) {
  const validated = validateOptionalUrl(value, "Ticket / registration URL");
  return validated.ok && value ? normalizeUrl(value) : "";
}
...
function profileContact(profile: any, u: any) {
  return {
    name: profile?.preferred_name || profile?.full_name || u?.user_metadata?.full_name || u?.user_metadata?.name || "",
...
async function uploadImage(file: File, label = "Event image") {
  const validation = validateOptionalImageFile(file, label, 5);
      ...
      <div className="flex h-full overflow-x-auto snap-x snap-mandatory scroll-smooth [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
        {images.map((image, index) => (
          <div key={`${image}-${index}`} className="h-full min-w-full snap-center">
            <SafeImage src={image} alt={`${event.title} image ${index + 1}`} className="w-full h-full object-cover bg-gray-100" fallbackClassName="w-full h-full bg-pink-50 grid place-items-center text-pink-600 font-black" fallbackLabel="Seattle Desi TV" widthHint={1000} />
          </div>
        ))}
      </div>
      {images.length > 1 && (
        <>
          <div className="absolute right-3 top-3 rounded-full bg-slate-950/75 px-3 py-1 text-xs font-black text-white">{images.length} photos</div>
          <div className="absolute bottom-3 left-0 right-0 flex justify-center gap-1.5">
            {images.map((_, index) => (
              <span key={index} className="h-2 w-2 rounded-full bg-white/80 shadow" />
            ))}
          </div>
        </>
      )}
...
export default function EventsPageClient({ initialEvents = [] }: { initialEvents?: EventRow[] }) {
  ...
  const [message, setMessage] = useState(
    initialEvents.length
      ? `Showing ${initialEvents.length} approved event(s).`
  ...
  const [eventPeriod, setEventPeriod] = useState<"upcoming" | "previous">("upcoming");
  ...
  async function loadCoverageRequests(currentUser: any) {
    if (!currentUser?.id) {
      setCoverageByEvent({});
      setInfluencerByEvent({});
      return;
    }
    const [coverage, influencer] = await Promise.all([supabase.from("event_crew_assignments").select("id,event_id,status,assignment_type,created_at,approved_at").eq("user_id", currentUser.id).eq("assignment_type", "owner_coverage_request").order("created_at", { ascending: false }), supabase.from("event_influencer_intents").select("id,event_id,status,created_at").or(`user_id.eq.${currentUser.id},user_email.eq.${currentUser.email}`).order("created_at", { ascending: false })]);
    const coverageNext: Record<string, any> = {};
    (coverage.data || []).forEach((row: any) => {
      if (!coverageNext[row.event_id]) coverageNext[row.event_id] = row;
    });
    setCoverageByEvent(coverageNext);
    const influencerNext: Record<string, any> = {};
    (influencer.data || []).forEach((row: any) => {
      if (!influencerNext[row.event_id]) influencerNext[row.event_id] = row;
    });
    setInfluencerByEvent(influencerNext);
  }
  ...
  async function loadUserContext(currentUser: any) {
    ...
    setForm((current) =>
      team
        ? { ...current, poc_name: "", poc_email: "", poc_phone: "" }
        : {
            ...current,
  ...
  function toggleProfilePoc(checked: boolean) {
    ...
    if (checked) {
      const contact = profileContact(baseProfile, user);
      setForm((current) => ({
        ...current,
        poc_name: contact.name,
        poc_email: contact.email,
  ...
  function validateEventForm() {
    ...
    for (let index = 0; index < imageFiles.length; index++) {
      const imageError = validateOptionalImageFile(imageFiles[index], `Event image ${index + 1}`, 5);
      if (!imageError.ok) return imageError.message;
    }
    return null;
  }
  ...
  async function createOrganization() {
    if (!user?.id) {
      setSubmitMessage("Please login before creating an organization.");
      return;
    }
    if (!newOrganization.name.trim() || !newOrganization.category.trim() || !newOrganization.location.trim()) {
      setSubmitMessage("Organization name, category, and location are required.");
      return;
    }
    setSaving(true);
    try {
      const payload: any = {
        ...
        location: newOrganization.location.trim(),
      ...
      setSubmitMessage(`Could not create organization: ${formatError(error)}`);
    } finally {
      setSaving(false);
    }
  }
  ...
  async function submitEvent() {
    ...
    try {
      ...
      const eventPayload: any = {
        ...
        poc_phone: normalizePhone(form.poc_phone),
        created_by: user.id,
        status: "pending",
        approved: false,
      };
      ...
      if (insertedEvent && organizationMode === "organization" && selectedOrganization?.id) {
        const { error: linkError } = await supabase.from("event_organizations").insert({
  ...
  async function requestInfluencerCollab(event: EventRow) {
    ...
    if (!user?.id || !user?.email) {
      window.location.href = `/login?next=${encodeURIComponent("/my-influencer-profile")}`;
      return;
    }
    ...
    try {
      ...
      const payload = {
        ...
        status: "pending",
        expected_platforms: "Instagram/TikTok/YouTube/Facebook",
        collab_note: "I am attending this event and can send/tag/collab with SDTV.",
      };
      ...
      setCrewMessage(`Could not submit influencer collaboration request: ${formatError(error)}`);
    } finally {
      setRequestingInfluencerEventId("");
    }
  }
  ...
  const years = useMemo(() => Array.from(new Set(periodEvents.map((event) => parseDate(event.date)?.getFullYear()).filter(Boolean) as number[])).sort((a, b) => (eventPeriod === "previous" ? b - a : a - b)), [periodEvents, eventPeriod]);
  ...
  function selectEventPeriod(period: "upcoming" | "previous") {
    ...
    if (targetDate) {
      setMonthFilter(String(targetDate.getMonth()));
      setYearFilter(String(targetDate.getFullYear()));
    }
  }
      ...
      <article className="flex h-full flex-col overflow-hidden rounded-2xl border bg-white shadow-sm">
        ...
        <div className="flex flex-1 flex-col p-5">
          ...
          <div className="mt-auto flex flex-wrap gap-3 pt-5">
            ...
            {!isPast && isOwner && !coverageRequest && (
              <button type="button" onClick={() => requestOwnerCoverage(event)} disabled={requestingCoverageEventId === event.id} className="bg-slate-900 text-white px-4 py-2 rounded-lg font-bold text-sm disabled:opacity-60">
                {requestingCoverageEventId === event.id ? "Requesting..." : "Request SDTV Coverage"}
              </button>
            )}
            {!isPast && canRequestCrew && (
              <button type="button" onClick={() => requestCrew(event)} disabled={requestingCrewEventId === event.id} className="border border-pink-600 text-pink-600 px-4 py-2 rounded-lg font-bold text-sm disabled:opacity-60">
                {requestingCrewEventId === event.id ? "Requesting..." : "Request to Cover"}
              </button>
            )}
            {!isPast && (
              <button type="button" onClick={() => requestInfluencerCollab(event)} disabled={influencerDisabled} className={`${influencerButtonClass(influencerRequest)} px-4 py-2 rounded-lg font-bold text-sm disabled:opacity-70`}>
                {requestingInfluencerEventId === event.id ? "Requesting..." : influencerButtonLabel(influencerRequest)}
              </button>
            )}
          </div>
        </div>
      ...
      <section className="px-6 md:px-14 py-10">
        <div className="max-w-7xl mx-auto">
          <div className="flex flex-col md:flex-row md:items-end md:justify-between gap-4 mb-8">
            <div>
              <p className="text-pink-600 font-black uppercase tracking-wide">Seattle Desi TV Events</p>
              <h1 className="text-4xl md:text-5xl font-black mt-2">Community Events</h1>
              <p className="text-gray-500 mt-2">Browse approved community events. Add an event only when you need to submit something for review.</p>
            ...
            <div className="flex flex-wrap gap-3">
              ...
              <button type="button" onClick={() => setShowSubmitForm((value) => !value)} className="bg-pink-600 text-white px-5 py-3 rounded-xl font-bold">
                {showSubmitForm ? "Hide Add Event" : "Add Event"}
              </button>
              <button type="button" onClick={loadEvents} className="border border-pink-600 text-pink-600 px-5 py-3 rounded-xl font-bold bg-white">
                Refresh Events
              </button>
            </div>
          </div>
          ...
          {showSubmitForm && (
            <section className="mb-8 border rounded-3xl p-6 shadow-sm bg-white">
              <div className="flex items-start justify-between gap-4 mb-5">
                <div>
                  <h2 className="text-2xl font-black">Add Event</h2>
                  <p className="text-sm text-gray-500 mt-1">Submitted events appear after admin approval.</p>
                </div>
                <button type="button" onClick={() => setShowSubmitForm(false)} className="text-sm font-black text-gray-500">
                  Collapse
                </button>
              </div>
              {!authChecked ? (
                ...
                <div>
                  <div className="grid md:grid-cols-2 gap-4">
                    <Field label="Event title" required>
                      <input className="w-full border rounded-lg p-3 mt-1" placeholder="Event name" value={form.title} onChange={(e) => setForm({ ...form, title: e.target.value })} />
                    </Field>
                    ...
                    <Field label="End time">
                      <input className="w-full border rounded-lg p-3 mt-1" type="time" value={form.local_end_time} min={form.local_start_time || undefined} onChange={(e) => setForm({ ...form, local_end_time: e.target.value })} />
                      <span className="mt-1 block text-xs font-normal text-gray-500">Times are shown in Pacific Time.</span>
                    </Field>
                    <Field label="Venue / location" required>
                      <input className="w-full border rounded-lg p-3 mt-1" placeholder="Venue, city, state" value={form.location} onChange={(e) => setForm({ ...form, location: e.target.value })} />
                    </Field>
                    <Field label="Ticket / registration URL">
                      <input className="w-full border rounded-lg p-3 mt-1" placeholder="https://example.com/tickets" value={form.ticket_url} onChange={(e) => setForm({ ...form, ticket_url: e.target.value })} />
                    </Field>
                    <div className="md:col-span-2 rounded-2xl border bg-slate-50 p-4">
                      <p className="text-sm font-black text-slate-900">Who is organizing this event?</p>
                      <div className="mt-3 flex flex-wrap gap-5 text-sm font-bold">
                        <label className="flex items-center gap-2">
                          <input
                            type="radio"
                            name="organizationMode"
                            checked={organizationMode === "individual"}
                            onChange={() => {
                              setOrganizationMode("individual");
                              setSelectedOrganization(null);
                              setOrganizationSearch("");
                        ...
                        <label className="flex items-center gap-2">
                          <input type="radio" name="organizationMode" checked={organizationMode === "organization"} onChange={() => setOrganizationMode("organization")} />
                          This event belongs to an organization
                        </label>
                      </div>
                        ...
                        <div className="mt-4 space-y-3">
                          {selectedOrganization ? (
                            <div className="flex flex-wrap items-center justify-between gap-3 rounded-xl border border-emerald-300 bg-emerald-50 p-3">
                              <div>
                                <p className="font-black text-emerald-900">{selectedOrganization.name}</p>
                                <p className="text-xs text-emerald-700">{selectedOrganization.status === "pending" ? "Pending organization approval" : selectedOrganization.organization_type || selectedOrganization.category || "Approved organization"}</p>
                              </div>
                              <button type="button" onClick={() => setSelectedOrganization(null)} className="rounded-lg border border-emerald-500 px-3 py-2 text-xs font-black text-emerald-800">
                                Change
                              </button>
                            </div>
                          ) : (
                            <>
                              <input className="w-full rounded-xl border bg-white p-3" placeholder="Search approved organizations..." value={organizationSearch} onChange={(e) => setOrganizationSearch(e.target.value)} />
                              {organizationSearch && (
                                <div className="max-h-56 overflow-auto rounded-xl border bg-white">
                                  {filteredOrganizations.map((organization) => (
                                    <button
                                      ...
                                      className="flex w-full items-start justify-between border-b p-3 text-left hover:bg-slate-50"
                                    >
                                      <span>
                                        <b>{organization.name}</b>
                                        <span className="block text-xs text-gray-500">
                                          {organization.organization_type || organization.category || "Community organization"}
                                          {organization.location ? ` · ${organization.location}` : ""}
                                        </span>
                                      </span>
                                      <span className="font-black text-pink-600">Select</span>
                                    </button>
                                  ))}
                                  {filteredOrganizations.length === 0 && <p className="p-4 text-sm text-gray-500">No matching organization found.</p>}
                                </div>
                              )}
                            </>
                          )}
                          <button type="button" onClick={() => setShowCreateOrganization((value) => !value)} className="text-sm font-black text-pink-600">
                            {showCreateOrganization ? "Cancel new organization" : "+ Can't find it? Create Organization"}
                          </button>
                          {showCreateOrganization && (
                            <div className="grid gap-3 rounded-xl border bg-white p-4 md:grid-cols-2">
                              ...
                              />
                              <button type="button" onClick={createOrganization} disabled={saving} className="rounded-xl bg-slate-950 px-4 py-3 font-black text-white md:col-span-2 disabled:opacity-60">
                                Create and Select Organization
                              </button>
                            </div>
                          )}
                        </div>
                    ...
                    <div className="md:col-span-2 rounded-2xl border bg-slate-50 p-4">
                      <label className="flex items-start gap-3 text-sm font-black">
                        <input type="checkbox" checked={useProfilePoc} onChange={(e) => toggleProfilePoc(e.target.checked)} className="mt-1" />
                        <span>{isTeamMember ? "Use my SDTV Base Profile as the Event POC" : "Use my contact information as the Event POC"}</span>
                      </label>
                      <p className="mt-2 text-xs text-slate-500">{useProfilePoc ? "✓ Using your SDTV Base Profile. You can edit below if needed." : "Using custom Event POC details. Team members are left blank by default so you can enter the actual organizer."}</p>
                    </div>
                    <Field label="Organizer contact name" required>
                      <input className="w-full border rounded-lg p-3 mt-1" placeholder="Point of contact" value={form.poc_name} onChange={(e) => setForm({ ...form, poc_name: e.target.value })} />
                    </Field>
                    <Field label="Organizer contact email" required>
                      <input className="w-full border rounded-lg p-3 mt-1" placeholder="organizer@example.com" type="email" value={form.poc_email} onChange={(e) => setForm({ ...form, poc_email: e.target.value })} />
                    </Field>
                    <Field label="Organizer contact phone" required>
                      <input
                    ...
                    <div className="md:col-span-2">
                      <Field label="Event description">
                        <textarea className="w-full border rounded-lg p-3 mt-1 min-h-28" placeholder="Tell viewers about the event." value={form.description} onChange={(e) => setForm({ ...form, description: e.target.value })} />
                      </Field>
                    </div>
                    <div className="md:col-span-2">
                      <Field label="Event images" help="Optional. Select one or more images. Each image must be JPG/PNG/WebP/GIF and 5MB or smaller.">
                        <input className="w-full border rounded-lg p-3 mt-1" type="file" accept="image/*" multiple onChange={(e) => setImageFiles(Array.from(e.target.files || []))} />
                      </Field>
                      {imageFiles.length > 0 && <p className="text-xs text-gray-500 mt-2">Selected {imageFiles.length} image(s).</p>}
                    </div>
                  </div>
                  {submitMessage && <p className="text-sm text-orange-600 my-4 whitespace-pre-line">{submitMessage}</p>}
                  <button type="button" onClick={submitEvent} disabled={saving} className="bg-pink-600 text-white px-5 py-3 rounded-xl font-bold w-full disabled:opacity-60 mt-5">
                    {saving ? `Uploading ${imageFiles.length || ""} image(s)...` : "Submit Event for Approval"}
                  </button>
                </div>
              ) : (
                <div>
                  <h3 className="text-xl font-black mb-3">Login to Add Event</h3>
                  ...
                  <a href="/login" className="inline-block bg-pink-600 text-white px-5 py-3 rounded-xl font-bold">
                    Login to continue
                  </a>
                </div>
              )}
          ...
          <section className="mb-6 bg-white border rounded-2xl p-4">
            <div className="flex flex-col md:flex-row md:items-end gap-4">
              <div>
                <label htmlFor="event-month" className="block text-sm font-black text-slate-900">
                  Month
                </label>
                <select id="event-month" aria-label="Filter events by month" value={monthFilter} onChange={(e) => setMonthFilter(e.target.value)} className="border rounded-xl p-3 mt-1">
              ...
              <div>
                <label htmlFor="event-year" className="block text-sm font-black text-slate-900">
                  Year
                </label>
                <select id="event-year" aria-label="Filter events by year" value={yearFilter} onChange={(e) => setYearFilter(e.target.value)} className="border rounded-xl p-3 mt-1">
                  {(years.length ? years : [new Date().getFullYear()]).map((year) => (
              ...
              <div>
                <p className="block text-sm font-black text-slate-900 mb-1">View</p>
                <div className="flex gap-2">
                  <button type="button" onClick={() => setViewMode("cards")} className={`px-4 py-3 rounded-xl font-bold ${viewMode === "cards" ? "bg-pink-600 text-white" : "bg-slate-100"}`}>
                    Cards
                  </button>
                  <button type="button" onClick={() => setViewMode("calendar")} className={`px-4 py-3 rounded-xl font-bold ${viewMode === "calendar" ? "bg-pink-600 text-white" : "bg-slate-100"}`}>
                    Calendar
                  </button>
                </div>
              </div>
              <p className="text-sm text-gray-500 md:ml-auto">
                Showing {filteredEvents.length} {eventPeriod === "previous" ? "previous" : "upcoming"} event(s) for the selected month.
              </p>
            </div>
          </section>
          {viewMode === "cards" ? (
            <section>
              {filteredEvents.length === 0 ? (
                <div className="border rounded-2xl p-8 text-gray-500 bg-white">
                  No {eventPeriod === "previous" ? "previous" : "upcoming"}
                  events found for this month.
                </div>
              ) : (
