...
export default function HomePageClient({ initialData = {} }: { initialData?: InitialHomepageData }) {
  const site = useCurrentSite();
  const [events, setEvents] = useState<EventRow[]>(initialData.events || []);
  const [businesses, setBusinesses] = useState<BusinessRow[]>(initialData.businesses || []);
  const [team, setTeam] = useState<TeamRow[]>([]);
  const [people, setPeople] = useState<PersonRow[]>([]);
  const [videos, setVideos] = useState<VideoRow[]>([]);
  const [videoMessage, setVideoMessage] = useState("Community stories and recent SDTV video highlights.");
  const [socialRows, setSocialRows] =
    useState<SocialRow[]>(initialData.socialRows?.length ? initialData.socialRows : fallbackSocialStats);
  const [featuredSocial, setFeaturedSocial] = useState<FeaturedSocialItem[]>(
    [],
  );
  const [sponsors, setSponsors] = useState<SponsorRow[]>([]);
  const [counts, setCounts] = useState<Counts>(initialData.counts || emptyCounts);
  ...
  const [loadingDynamic, setLoadingDynamic] = useState(!initialData.counts);
