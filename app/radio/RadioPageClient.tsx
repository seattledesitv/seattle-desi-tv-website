  ...
  return (
    <article className="bg-white border rounded-2xl p-5 text-center shadow-sm hover:shadow-xl transition">
      <SafeImage
        ...
        fallbackClassName="w-28 h-28 rounded-full bg-pink-50 text-pink-600 grid place-items-center mx-auto font-black"
        fallbackLabel="SDTV"
      />
...
export default function RadioPageClient() {
  ...
  async function loadHosts() {
    ...
    const modernResult = await supabase
      ...
      .select(
        "id,name,title,segment_name,image,email,user_id,show_on_public_radio",
      )
      .eq("site_id", site.id)
    ...
    if (!modernResult.error) {
      setHosts(
        (modernResult.data || []).filter(
          (row: any) => !isPubliclyHidden(row, hidden),
        ),
      );
      setLoading(false);
      return;
    ...
    const { data } = await supabase
      .from("radio_team_members")
      .select("id,name,title,segment_name,image,email,user_id")
      .eq("site_id", site.id)
      .order("created_at", { ascending: false });
    ...
    <main className="min-h-screen w-full max-w-full overflow-x-hidden bg-gradient-to-b from-white to-slate-100 text-slate-950">
      ...
      <section className="relative overflow-hidden bg-slate-950 text-white w-full max-w-full">
        <div className="relative max-w-7xl mx-auto px-4 sm:px-6 md:px-10 py-8 sm:py-14 md:py-18 overflow-hidden">
          <p className="text-pink-300 font-black uppercase tracking-wide text-sm sm:text-base">
            {site.city} Desi Radio
          </p>
          <h1 className="text-3xl sm:text-5xl md:text-6xl font-black max-w-4xl leading-tight mt-2 break-words">
            Listen Live
          </h1>
          <p className="text-base sm:text-lg md:text-xl text-slate-200 max-w-3xl mt-4">
            Community voices, music, interviews, culture, and Desi weekend vibes
            from the Pacific Northwest.
          </p>
          <div className="flex flex-col sm:flex-row gap-3 sm:gap-4 mt-6 sm:mt-8 w-full max-w-xs sm:max-w-none">
            <a
              href="#listen"
              className="bg-pink-600 text-white px-5 py-3 sm:px-6 sm:py-4 rounded-xl font-black text-center"
            >
              Start Listening
            </a>
            <a
              href="#hosts"
              className="bg-white text-slate-950 px-5 py-3 sm:px-6 sm:py-4 rounded-xl font-black text-center"
            >
              Meet Hosts
            </a>
          </div>
      ...
      <section
        id="listen"
        className="max-w-7xl mx-auto px-4 sm:px-6 md:px-10 py-7 sm:py-10 md:py-12 overflow-hidden"
      >
        <div className="bg-white border rounded-3xl p-5 sm:p-7 md:p-8 shadow-sm grid lg:grid-cols-[minmax(0,0.85fr)_minmax(0,1.15fr)] gap-7 md:gap-10 items-center overflow-hidden min-w-0">
          <div className="min-w-0">
            <p className="text-pink-600 font-black uppercase tracking-wide text-sm sm:text-base">
              On Air
            </p>
            ...
            <p className="text-gray-600 mt-4 text-base sm:text-lg">
              Tune in for Desi TV Radio programming, community interviews,
              cultural conversations, and music curated for the {site.city} Desi
              community.
            </p>
            <p className="mt-5 text-sm font-bold text-slate-500">
              Press Play to listen directly on {site.name}.
            </p>
          </div>
      ...
      <section
        id="schedule"
        className="max-w-7xl mx-auto px-4 sm:px-6 md:px-10 py-8 sm:py-10 md:py-12 overflow-hidden"
      >
        <div className="mb-6">
          <p className="text-pink-600 font-black uppercase tracking-wide">
            Programming
          </p>
          <h2 className="text-3xl md:text-4xl font-black">Radio Schedule</h2>
          <p className="text-gray-600 mt-1">
            Upcoming broadcasts and regular {site.city} Desi Radio programming.
          </p>
        </div>
        <RadioSchedule />
      </section>
      ...
      >
        <div className="flex items-end justify-between gap-4 mb-6">
          <div>
            ...
            <p className="text-gray-600 mt-1">
              Meet the voices behind {site.city} Desi Radio.
            </p>
          </div>
          {canManageStudio && (
            <a
              href="/studio/radio-team"
              className="hidden md:inline-block text-pink-600 font-black"
            >
              Studio Radio Team →
            </a>
          )}
        ...
        {loading ? (
          ...
          <div className="space-y-10">
            {productionTeam.length > 0 && (
              <div>
                <h3 className="text-2xl font-black mb-4">Production Team</h3>
                <div className="grid md:grid-cols-2 xl:grid-cols-4 gap-5">
                  {productionTeam.map((host) => (
                    <RadioCard key={host.id} host={host} />
                  ))}
                </div>
              </div>
            )}
            {radioHosts.length > 0 && (
              <div>
                <h3 className="text-2xl font-black mb-4">Hosts</h3>
                <div className="grid md:grid-cols-2 xl:grid-cols-4 gap-5">
                  {radioHosts.map((host) => (
                    <RadioCard key={host.id} host={host} />
                  ))}
