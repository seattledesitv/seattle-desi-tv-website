...
export default function MyEventsPage() {
  ...
  async function loadRows() {
    ...
    setRows(data || []); setMessage(error ? error.message : "Event listings submitted from your profile."); setLoading(false);
  }
  ...
  const filteredRows = useMemo(() => {
    ...
    return rows.filter((row) => {
      ...
      if (monthFilter && monthInput(row.date) !== monthFilter) return false;
      if (!q) return true;
  ...
  function startEdit(row: any) { setViewingId(row.id); setEditingId(row.id); setImageFiles([]); setEditForm({ title: row.title || "", date: dateInput(row.date), location: row.location || "", description: row.description || "", ticket_url: row.ticket_url || "", poc_email: row.poc_email || "", poc_phone: row.poc_phone || "", image_urls: eventImages(row), image_url_input: "" }); }
