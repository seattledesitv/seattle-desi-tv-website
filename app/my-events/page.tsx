...
export default function MyEventsPage() {
  ...
  async function loadRows() {
    ...
    if (!user?.id) { setRows([]); setMessage("Please login to view event listings submitted from your profile."); setLoading(false); return; }
    const { data, error } = await supabase.from("events").select("id,title,date,location,description,ticket_url,poc_email,poc_phone,image,image_urls,status,created_at,created_by").eq("created_by", user.id).order("created_at", { ascending: false });
    setRows(data || []);
  ...
  const filteredRows = useMemo(() => {
    ...
    return rows.filter((row) => {
      const rowDate = dateInput(row.date);
      if (statusFilter !== "all" && String(row.status || "pending").toLowerCase() !== statusFilter) return false;
      if (dateFrom && rowDate && rowDate < dateFrom) return false;
      if (dateTo && rowDate && rowDate > dateTo) return false;
      if (!q) return true;
      return [row.title, row.location, row.description, row.poc_email, row.poc_phone, row.status].some((value) => String(value || "").toLowerCase().includes(q));
    });
  }, [rows, searchText, dateFrom, dateTo, statusFilter]);

  function startEdit(row: any) {
    setViewingId(row.id); setEditingId(row.id); setImageFiles([]);
    setEditForm({ title: row.title || "", date: dateInput(row.date), location: row.location || "", description: row.description || "", ticket_url: row.ticket_url || "", poc_email: row.poc_email || "", poc_phone: row.poc_phone || "", image_urls: eventImages(row), image_url_input: "" });
  }
  ...
  async function saveEvent(row: any) {
    setMessage("");
    if (!editForm.title?.trim() || !editForm.date || !editForm.location?.trim()) { setMessage("Please enter event title, date, and location."); return; }
    setSaving(true);
    const { data: auth } = await supabase.auth.getUser(); const user = auth?.user || null;
    if (!user?.id || row.created_by !== user.id) { setSaving(false); setMessage("You can only edit event listings submitted from your own profile."); return; }
    try {
